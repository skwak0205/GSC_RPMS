<%-- ApplicabilityDefinitionDialog.jsp
...
<%@page import = "com.dassault_systemes.plm.config.expression.itf.IApplicabilityExpression"%>
<%@page import = "com.dassault_systemes.plm.config.expression.itf.ConfigExpressionFactory"%>
<%@page import = "com.dassault_systemes.plm.config.expression.itf.EnumTypeExpr"%>
...
/* x56 */
String strFormName = "emxTableForm";
...
String lng = emxGetParameter(request,"lang");
if(lng != null && lng != "undefined"){
context.getSession().setLanguage(lng);
acceptLanguage = lng;
}
...
String strInvockedFrom  = "fromTable";
...
// IVU
strParentObjectId  = emxGetParameter(request,"contextID");
String strCAPhysicalID = strParentObjectId;
	 if(strCAPhysicalID != null)
     {
         try
         {
        	
			 IApplicabilityExpression exp = ConfigExpressionFactory.initApplicabilityExpression(context, new BusinessObject(strCAPhysicalID));
			 String strExpression = exp.getExpression(context, EnumTypeExpr.TypeXML);
		     String XMLExpression = strExpression;
             String expression_xml = EFF.convertXMLtoActualExpression(context,XMLExpression);
             currActualExpression = expression_xml;
...
String dialogTitle = "Effectivity.Label.EditApplicabilityDefinitionHeading";
...
<title><emxUtil:i18n localize="i18nId">Effectivity.Label.EditApplicabilityDefinitionHeading</emxUtil:i18n></title>
...
<script language="javascript" type="text/javascript" src="../effectivity/scripts/UIApplicabilityFramework.js"></script>
<script Language="Javascript">
<%
...
/*         if(strExpandProgram != null && !("").equalsIgnoreCase(strExpandProgram)){
        ...
        /* x56 */
        String effName = "";
        Map effCriteria = null;
        String effTypes = "ContextDate,Milestone,ProductRevision,ManufacturingPlans,Unit";
        StringTokenizer st = new StringTokenizer(effTypes, ",");
        while (st.hasMoreTokens())
        {
            effName = st.nextToken();
            effCriteria = new HashMap();
            effCriteria.put("actualValue", effName);
            String strlabel = EffectivitySettingsManager.getEffectivityLabel(context, effName);
            effCriteria.put("displayValue", strlabel);
            relEffTypes.add(effCriteria);
        }
        ...
        <tr>
            ...
            <td class="buttons">
              <table border="0" cellspacing="0" cellpadding="0">
                  <tr>
                    <td><a title="<emxUtil:i18n localize="i18nId">Effectivity.Button.Help</emxUtil:i18n>" href='javascript:openHelp("emxhelpapplicabilityedit","effectivity","<%=XSSUtil.encodeForJavaScript(context,lStr)%>","<%=XSSUtil.encodeForJavaScript(context,langOnlineStr)%>")'><img src="../common/images/iconActionHelp.gif" width="16" height="16" border="0"  alt="<emxUtil:i18n localize="i18nId">Effectivity.Button.Help</emxUtil:i18n>" /></a></td>
                    <%-- x56 --%><%-- <td><button class="btn-primary" type="button" name="validate" class="mx_btn-validate" onClick="fnValidate(false,true);" ><emxUtil:i18n localize='i18nId'>Effectivity.Button.Validate</emxUtil:i18n></button></td> --%>
                    ...
                    <td><button class="btn-primary" type="button" name="cancel" class="mx_btn-cancel" onclick="fnCancel();"> <emxUtil:i18n localize="i18nId">Effectivity.Button.Cancel</emxUtil:i18n></button></td>
                  </tr>
              </table>
            </td>
        </tr>
        ...
        <div id="mx_divExpression">
           <div class="panel-head">
            <div class="function" id="mx_builderToggle" value="openStatus"><a href="#" onclick="toggleBuilderPanel()" id="builderTagId"   title="<emxUtil:i18n localize="i18nId">Effectivity.Dialog.ClickToExpand</emxUtil:i18n>"><img src="../common/images/iconActionClosePanel.gif" id="builderCollId" /></a></div>
               <div class="title" style='left:17px'><emxUtil:i18n localize="i18nId">Effectivity.Form.Label.ApplicabilityExpression</emxUtil:i18n></div>
               <div class="toolbar">
                    <div class="functions">
                        <ul>
                            <li>
                    ...
                    <div class="actions">
                        <ul id="operatorsTable" class="mx_operators">
                         ...
                         <li id="li-or"><input type="button" id="or" name="or" value="OR" class="off" onclick='togglestyle(this)' /></li>
						  ...
						  <li id="li-infinity"><input type="button" id="infinity" name="infinity" value=<%=XSSUtil.encodeForHTMLAttribute(context,infinitySymbolDisp)%> onclick='insertInfinity(),computedRule(")","left")' /></li>
                        </ul>
                    </div>
               </div>
           </div><!-- /.panel-head -->
           ...
           <div class="panel-body">
                ...
                <div id="divSelect" oncontextmenu="return false" onscroll="OnDivScroll();">
                    <select name="LeftExpression" size="10" id="IncExp" multiple oncontextmenu="return false" onfocus="OnSelectFocus();" onchange="resetKeyInDiv();" onkeydown="resetReadMode();" > </select>
...
function finalSubmit()
{
    ...
    var caPID = "<xss:encodeForJavaScript><%=strCAPhysicalID %></xss:encodeForJavaScript>";
    
    
    var url="../effectivity/ApplicabilityUtil.jsp?mode=CAApplicability&CAPID="+caPID;
    emxUICore.getDataPost(url, "actualExp=" + actualExpression);

            if(getTopWindow().getWindowOpener())
            {
            	//window.opener.document.getElementById("applicabilityContainer").innerHTML=vRes;
