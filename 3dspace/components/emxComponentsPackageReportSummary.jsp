...
<%
  ...
  String strFile = EnoviaResourceBundle.getProperty(context, "emxComponentsStringResource", context.getLocale(), "emxComponents.Common.File");
 ...
 selectStmts.addElement(DomainObject.SELECT_FILE_SIZE);
    ...
    /* To support files checked into the Part. st */
	MapList DummyList = new MapList();
    /* To support files checked into the Part. en */

    MapList fileList = new MapList();
    MapList displaymapList = new MapList();
    MapList tempMapList1 = new MapList();
...
/* To support files checked into the Part. st */
         ...
         DummyList = partObj.getRelatedObjects(context,specrelationshiptrn,spectypetrn,selectStmts,selectRelStmts,false,true,sPLevel,"","");
		 specList.add (partObj.getInfo(context, selectStmts));
		 specList.addAll(DummyList);
...
/* To support files checked into the Part. st */
					   if (parentSpecFilesizeList.size() == 1) {
						parentSpecFilesize = (String) parentSpecFilesizeList.get(0);
					   }
/* To support files checked into the Part. en */
                          ...
                          displayparentspecmap.put("size","");
                               ...
                               //Added for IR-553441 - Starts
                                 tempMapList1 = new MapList();
                                 formatMap = new HashMap();
                                 Iterator itrFiles = fileList.iterator();
                                 while (itrFiles.hasNext()){
                                    formatMap = (Map)itrFiles.next();
                                    sFormat = (String)formatMap.get(domainObj.KEY_FORMAT);
                                      if(!DomainObject.FORMAT_MX_MEDIUM_IMAGE.equalsIgnoreCase(sFormat)){
                                          tempMapList1.add(formatMap);
                                    } 
                                 }
                                 fileList = tempMapList1;
                               //Added for IR-553441 - Ends
                                 strVCIndex = "0";
                                 strVCVersion = "0";
                               }
                               else
                     ...
                     checkBoxValue = combFolderAdd+delimiter+ partId + "|" + selectparentspecobjId +delimiter+sFmtName  + delimiter + sFileName + delimiter + strVCIndex +delimiter+"versionid@" + strVCVersion;
                                      ...
                                      displayparentfilemap.put("type",strFile);
...
if(!(selectedlevelno.equals("0")&&tokenalllevels.equals("")))  {
     ...
     // getRelatedObjects(matrix.db.Context context, java.lang.String relationshipPattern, java.lang.String typePattern, matrix.util.StringList objectSelects, matrix.util.StringList relationshipSelects, boolean getTo, boolean getFrom, short recurseToLevel, java.lang.String objectWhere, java.lang.String relationshipWhere)
  	selectStmts.addElement("from.to."+DomainObject.SELECT_ID);
	selectStmts.addElement("from.to."+DomainObject.SELECT_TYPE);
	selectStmts.addElement("from.to."+DomainObject.SELECT_NAME);
	selectStmts.addElement("from.to."+DomainObject.SELECT_REVISION);
 	selectStmts.addElement("from.to."+DomainObject.SELECT_DESCRIPTION);
 	selectStmts.addElement("from.to."+DomainObject.SELECT_CURRENT);
 	selectStmts.addElement("from.to."+DomainObject.SELECT_POLICY);
 	selectStmts.addElement("from.to."+DomainObject.SELECT_ATTRIBUTE_QUANTITY);
 	selectStmts.addElement("from.to."+DomainObject.SELECT_ATTRIBUTE_UNITOFMEASURE);
 	selectStmts.addElement("from.to."+DomainObject.SELECT_FILE_SIZE);
 	selectStmts.addElement("from."+DomainObject.SELECT_ID);
 	selectStmts.addElement("from."+DomainRelationship.SELECT_RELATIONSHIP_NAME);
     ...
     if(sSupplierId!=null && !"null".equals(sSupplierId) && !"".equals(sSupplierId))
     {
      ...
      if( ebomlistSize > 0){
            ...
            while (ibom.hasNext()) {
            	specList = new MapList();
            	Map tempMap=new HashMap();
                Map mbom = (Map) ibom.next();
                
                if(mbom.containsKey("from["+DomainConstants.RELATIONSHIP_PART_SPECIFICATION+"]."+DomainObject.SELECT_ID)){
               		StringList toIdSL = getStringList(mbom.get("from["+DomainConstants.RELATIONSHIP_PART_SPECIFICATION+"].to."+DomainObject.SELECT_ID));
            		StringList currentSL = getStringList(mbom.get("from["+DomainConstants.RELATIONSHIP_PART_SPECIFICATION+"].to."+DomainObject.SELECT_CURRENT));
            		StringList toNameSL = getStringList(mbom.get("from["+DomainConstants.RELATIONSHIP_PART_SPECIFICATION+"].to."+DomainObject.SELECT_NAME));
            		StringList descSL = getStringList(mbom.get("from["+DomainConstants.RELATIONSHIP_PART_SPECIFICATION+"].to."+DomainObject.SELECT_DESCRIPTION));
            		StringList uomSL = getStringList(mbom.get("from["+DomainConstants.RELATIONSHIP_PART_SPECIFICATION+"].to."+DomainObject.SELECT_ATTRIBUTE_UNITOFMEASURE));
            		StringList qtySL = getStringList(mbom.get("from["+DomainConstants.RELATIONSHIP_PART_SPECIFICATION+"].to."+DomainObject.SELECT_ATTRIBUTE_QUANTITY));
            		StringList idSL = getStringList(mbom.get("from["+DomainConstants.RELATIONSHIP_PART_SPECIFICATION+"]."+DomainObject.SELECT_ID));
            		StringList typeSL = getStringList(mbom.get("from["+DomainConstants.RELATIONSHIP_PART_SPECIFICATION+"].to."+DomainObject.SELECT_TYPE));
            		StringList policySL = getStringList(mbom.get("from["+DomainConstants.RELATIONSHIP_PART_SPECIFICATION+"].to."+DomainObject.SELECT_POLICY));
            		StringList nameSL = getStringList(mbom.get("from["+DomainConstants.RELATIONSHIP_PART_SPECIFICATION+"]."+DomainRelationship.SELECT_RELATIONSHIP_NAME));
            		StringList revisionSL = getStringList(mbom.get("from["+DomainConstants.RELATIONSHIP_PART_SPECIFICATION+"].to."+DomainObject.SELECT_REVISION));
                	for (int i=0;i<toIdSL.size();i++){
                       	//tempMap=new HashMap();
        	           	tempMap.put(DomainObject.SELECT_ID, toIdSL.get(i));
                	   	tempMap.put(DomainObject.SELECT_CURRENT, currentSL.get(i));
                    	tempMap.put(DomainObject.SELECT_NAME, toNameSL.get(i));
                    	tempMap.put("relationship", DomainConstants.RELATIONSHIP_PART_SPECIFICATION);
                    	tempMap.put(DomainObject.SELECT_DESCRIPTION, descSL.get(i));
                    	tempMap.put(DomainObject.SELECT_ATTRIBUTE_UNITOFMEASURE, uomSL.get(i));
                    	tempMap.put(DomainObject.SELECT_ATTRIBUTE_QUANTITY, qtySL.get(i));
                    	tempMap.put("level", mbom.get("level"));
                    	tempMap.put(DomainRelationship.SELECT_RELATIONSHIP_ID, idSL.get(i));
        	           	tempMap.put(DomainObject.SELECT_TYPE, typeSL.get(i));
        	           	tempMap.put(DomainObject.SELECT_POLICY, policySL.get(i));
        	           	tempMap.put(DomainRelationship.SELECT_RELATIONSHIP_NAME, nameSL.get(i));
        	           	tempMap.put(DomainObject.SELECT_REVISION, revisionSL.get(i));
                    	specList.add(tempMap);
                	}
                }
                
                if(mbom.containsKey("from["+DomainConstants.RELATIONSHIP_REFERENCE_DOCUMENT+"].id")){
               		StringList toIdSL = getStringList(mbom.get("from["+DomainConstants.RELATIONSHIP_REFERENCE_DOCUMENT+"].to.id"));
            		StringList currentSL = getStringList(mbom.get("from["+DomainConstants.RELATIONSHIP_REFERENCE_DOCUMENT+"].to.current"));
            		StringList toNameSL = getStringList(mbom.get("from["+DomainConstants.RELATIONSHIP_REFERENCE_DOCUMENT+"].to.name"));
            		StringList descSL = getStringList(mbom.get("from["+DomainConstants.RELATIONSHIP_REFERENCE_DOCUMENT+"].to.description"));
            		StringList uomSL = getStringList(mbom.get("from["+DomainConstants.RELATIONSHIP_REFERENCE_DOCUMENT+"].to."+DomainObject.SELECT_ATTRIBUTE_UNITOFMEASURE));
            		StringList qtySL = getStringList(mbom.get("from["+DomainConstants.RELATIONSHIP_REFERENCE_DOCUMENT+"].to."+DomainObject.SELECT_ATTRIBUTE_QUANTITY));
            		StringList idSL = getStringList(mbom.get("from["+DomainConstants.RELATIONSHIP_REFERENCE_DOCUMENT+"].id"));
            		StringList typeSL = getStringList(mbom.get("from["+DomainConstants.RELATIONSHIP_REFERENCE_DOCUMENT+"].to.type"));
            		StringList policySL = getStringList(mbom.get("from["+DomainConstants.RELATIONSHIP_REFERENCE_DOCUMENT+"].to.policy"));
            		StringList nameSL = getStringList(mbom.get("from["+DomainConstants.RELATIONSHIP_REFERENCE_DOCUMENT+"]."+DomainRelationship.SELECT_RELATIONSHIP_NAME));
            		StringList revisionSL = getStringList(mbom.get("from["+DomainConstants.RELATIONSHIP_REFERENCE_DOCUMENT+"].to.revision"));
                	for (int i=0;i<toIdSL.size();i++){
                       	tempMap=new HashMap();
        	           	tempMap.put(DomainObject.SELECT_ID, toIdSL.get(i));
                	   	tempMap.put(DomainObject.SELECT_CURRENT, currentSL.get(i));
                    	tempMap.put(DomainObject.SELECT_NAME, toNameSL.get(i));
                    	tempMap.put("relationship", DomainConstants.RELATIONSHIP_REFERENCE_DOCUMENT);
                    	tempMap.put(DomainObject.SELECT_DESCRIPTION, descSL.get(i));
                    	tempMap.put(DomainObject.SELECT_ATTRIBUTE_UNITOFMEASURE, uomSL.get(i));
                    	tempMap.put(DomainObject.SELECT_ATTRIBUTE_QUANTITY, qtySL.get(i));
                    	tempMap.put("level", mbom.get("level"));
                ...
                if ( alias1 == null || alias1.equals("")){
                 ...
                 displayIcon = typeIcon1;
					...
					//Fix for IR-575425-3DEXPERIENCER2016x Starts
					if(tempMap.isEmpty() || tempMap.containsKey(DomainObject.SELECT_TYPE)){
...
/* To support files checked into the Part. st */
DummyList = subpartObj.getRelatedObjects(context,specrelationshiptrn,spectypetrn,selectStmts,selectRelStmts,false,true,SpLevel2,"","");
					specList = new MapList();
					specList.add(subpartObj.getInfo(context,selectStmts));
					specList.addAll(DummyList);
...
/* To support files checked into the Part. en */
		...
		/* To support files checked into the Part. st */
							 if (specFilesizeList.size() == 1) {
								specFilesize = (String)specFilesizeList.get(0);
							 }
                 ...
                 /* To support files checked into the Part. st */
                              ...
                              displayspecmap.put("size","");
                               ...
                               versionSelectList.add("to["+Document.RELATIONSHIP_ACTIVE_VERSION+"].from."+DomainObject.SELECT_FILE_SIZE);
                               String isVCDoc = domainObj.getInfo(context, com.matrixone.apps.common.CommonDocument.SELECT_IS_KIND_OF_VC_DOCUMENT);
                               if ((isVCDoc == null) || ("".equals(isVCDoc)) || ("null".equals(isVCDoc)))
                                 isVCDoc = "false"; 
                               String strVCIndex = "";
 ...
 /* To support files checked into the Part. st */
							   if (domainObj.isKindOf(context, typePart))
                               {
                               	fileList = domainObj.getAllFormatFiles(context);
                               	
                               	StringList selectFileStmt = new StringList();
                               	selectFileStmt.add(DomainObject.SELECT_FILE_NAME);
                               	selectFileStmt.add(DomainObject.SELECT_FILE_SIZE);
                               	selectFileStmt.add(DomainObject.SELECT_FILE_FORMAT);
                               	
                               	Map pSFileList = domainObj.getInfo(context, selectFileStmt);
                               	
                               	StringList fileListName = (StringList)pSFileList.get(DomainObject.SELECT_FILE_NAME);
                               	StringList fileSize = (StringList)pSFileList.get(DomainObject.SELECT_FILE_SIZE);
                               	Object obj  = (Object)pSFileList.get(DomainObject.SELECT_FILE_SIZE);
                               	
                               	StringList selFileFormat = (StringList)pSFileList.get(DomainObject.SELECT_FILE_FORMAT);
                               	
                               	int j = fileSize.size();
                               	
                                fileList  = new MapList();
                                String sFileSize ; String strFileName; String sFileFormat;
                               	for(int i = 0 ; i < j; i++) {
                               		Map  mp = new HashMap();
                               		
                               		sFileSize = (String)fileSize.get(i).trim();
                               		if(null != sFileSize && sFileSize.length() > 0 )
                               			mp.put(DomainObject.SELECT_FILE_SIZE, sFileSize);
                                 
                               		strFileName = (String)fileListName.get(i).trim();
                               		if(null != strFileName && strFileName.length() > 0 )
                               			mp.put("filename",strFileName);
                               		
                               		sFileFormat = (String)selFileFormat.get(i).trim();
                               		if(null != sFileFormat && sFileFormat.length() > 0 )
                                 		mp.put("format",sFileFormat);
                                 ...
                                 strVCVersion = "0";
							   //if (isVCDoc.equalsIgnoreCase("false"))
                               } else if (isVCDoc.equalsIgnoreCase("false"))
                               ...
                               //Added for IR-553441 - Ends
                                 ...
                                 if(itr.hasNext()){
                                 Map m=(Map)itr.next();
                                 //Map m=(Map)versionList.get(0);
                                 try{
                                     specFilesizeList = (StringList)m.get("to["+Document.RELATIONSHIP_ACTIVE_VERSION+"].from."+DomainObject.SELECT_FILE_SIZE);
                                  }catch(ClassCastException exp){
                                     specFilesize = (String)m.get("to["+Document.RELATIONSHIP_ACTIVE_VERSION+"].from."+DomainObject.SELECT_FILE_SIZE);
                                  }}
                               }
                               else
                               {
                                  ...
                                  versionSelectList.remove("to["+Document.RELATIONSHIP_ACTIVE_VERSION+"].from."+DomainObject.SELECT_FILE_SIZE);
                                  versionSelectList.add(DomainObject.SELECT_FILE_SIZE);
	                              Map vcMap = domainObj.getInfo(context, versionSelectList);
	                                 try{
	                                     specFilesizeList = (StringList)vcMap.get(DomainObject.SELECT_FILE_SIZE);
	                                  }catch(ClassCastException exp){
	                                     specFilesize = (String)vcMap.get(DomainObject.SELECT_FILE_SIZE);
	                                  }
                               ...
                               if(filelistsize > 0) {
                                 ...
                                 /* To support files checked into the Part. st */
									...
									//displaymapList.add(displayspecmap); //add spec obj info
			/* added to support Files checked into the Part. en */
                                    ...
                                    while (ifile.hasNext()) {
                                       ...
                                       specFilesize = (String)fileMap.get("format.file.size");
                                      
                                      // specFilesize	= (String)mmp.getElement(0);
				   ...
				   if (versionList != null) {
							...
							fileDescription = "";
                                      ...
                                      displayfilemap.put("type",strFile);
			...
			{
				String [] objIds = new String[specIdFamId.size()];
     ...
     //Additional objects to be downloaded
       ...
       if(UIUtil.isNotNullAndNotEmpty(strDownloadProgram) && UIUtil.isNotNullAndNotEmpty(strDownloadMethod)){
    	   FrameworkUtil.validateMethodBeforeInvoke(context, strDownloadProgram, strDownloadMethod, "Program");
  ...
  <fw:mapListItr mapList="<%= displaymapList %>" mapName="displaymap">
    ...
    <tr class='<fw:swap id="1"/>'>
     ...
     <%
      ...
      String objType 	= (String) displaymap.get("type");
...
%>
	   ...
	   <td>
	   		...
	   		%>
	   			&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
	   		<% }  %>
	   			<img src="../common/images/<%=icondisplay%>" border="0"/>&nbsp;<%= XSSUtil.encodeForHTML(context,(String)displaymap.get("name")) %>
	   </td>
<%
      } else {
%>
		...
		<td><% 
			...
			&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
			<% } %>
		<b><a href="javascript:emxShowModalDialog('<%=opennextURL%>',700,600,false)"><img src="../common/images/<%=icondisplay%>" border="0"/>&nbsp;<%= XSSUtil.encodeForHTML(context,(String)displaymap.get("name")) %></a></td>
<%
      }
%>
      <!--//XSSOK -->
...
public StringList getStringList(Object obj){
	
	return obj instanceof StringList ? (StringList)obj:new StringList((String) obj);
}
