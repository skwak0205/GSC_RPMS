...
<%
  ...
  String strPrivatePolicy = PropertyUtil.getSchemaProperty(context,"policy_PrivateMessage");
...
<%
  ...
  // get the logged-in user's name
   ...
   String orgId = person.getInfo(context, "relationship["+DomainConstants.RELATIONSHIP_EMPLOYEE+"].from.id");
   String strHostCompanyId= Company.getHostCompany(context);
   if(orgId.equals(strHostCompanyId))
	{
	   isPrivateUser = true;
	}
...
<script language="javascript">
  ...
  function loadTree() {
	...
	var discussion = null;
	var isPrivateUser = null;
	var privatePolicy = null;
	var currentObjectPolicy = null;
	...
	privatePolicy = "<%=strPrivatePolicy%>";
...
/* Removal of displaytime='true' by Yukthesh,Infosys for Bug # 293306*/
    discussion.createRoot("<%=tempSubject%>", "<%=tempMessage%>", "<%=strOwner%>", "<emxUtil:lzDate displaytime='true' localize='i18nId' tz='<%=sTimeZone%>' format='<%=DateFrm %>' ><%=strDate%></emxUtil:lzDate>","<%=messageId%>","<%=strPolicy1%>","<%=showSubscription%>");
//discussion.createRoot("<%=tempSubject%>", "<%=tempMessage%>", "<%=strOwner%>", "<emxUtil:lzDate displaydate='true' localize='i18nId' tz='<%=timeZone%>' format='<%=DateFrm %>' ><%=strDate%></emxUtil:lzDate>","<%=messageId%>","<%=showSubscription%>");
...
if(privatePolicy != currentObjectPolicy || (privatePolicy==currentObjectPolicy && isPrivateUser == "true")) {
  discussion.root.addChild("<%=rSubject%>","<%=rMessage%>","<%=sreplyOwner%>","<emxUtil:lzDate displaytime='true' localize='i18nId' tz='<%=sTimeZone%>' format='<%=DateFrm %>' ><%=sSelectOriginateddate%></emxUtil:lzDate>","<%=sSelectId%>","<%=showDelete%>","<%=sObjectIds%>","<%=streeIds%>","<%=strPolicy%>","<%=showSubscription%>");
} //now add the data
