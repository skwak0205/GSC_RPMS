...
function callCheckout(objectId, action, fileName, format, refresh, closeWindow, appName, appDir, partId, version, SB, customSortColumns, customSortDirections, uiType, table, getCheckoutMapFromSession, fromDataSessionKey, parentOID)
{
    ...
    if (newForm == null) {
        newForm = getTopWindow().getWindowOpener().getTopWindow().document.getElementById(formName);
    }
    //IR-186383V6R2014x: step up once more to find the form
    if (newForm == null) {
        newForm = getTopWindow().getWindowOpener().getTopWindow().getWindowOpener().getTopWindow().document.getElementById(formName);
    }
...
function validateCurrencyValue(value)
{
    if (value == "" || isNaN(value) || ((value)<=0) ) {
        ...
        if(value.contains(decimalQtySetting)){
            value=value.replace(decimalQtySetting,".");
...
function validatePersonIfEmployee() {
		
		var topWinFrame= findFrame(getTopWindow(),"detailsDisplay");
		var contextComp = topWinFrame.parentOID;
		var personsSelected = topWinFrame.getSelectedRowObjects();
		var personsSelectedIds="";
		for(var i=0;i<personsSelected.length;i++) {
			if(personsSelectedIds =="") {	
				personsSelectedIds=personsSelected[i].getAttribute("o");
			} else {
				personsSelectedIds = personsSelectedIds+","+personsSelected[i].getAttribute("o");
			}
			
		}
		var url = "../components/emxComponentsConfigurableTablePeopleMoveProcess.jsp"	
		var strdata= "isValidation=true&contCompany="+encodeURIComponent(contextComp)+"&persSelected="+encodeURIComponent(personsSelectedIds);
		
		var result = emxUICore.getDataPost(url, strdata);
		if(result != "Error") {
			alert(result);	
		} else {
			var strMovePersonUrl="emxForm.jsp?form=APPMovePersonCompany&mode=edit&emxSuiteDirectory=components&suiteKey=Components&formHeader=emxComponents.MovePerson.Heading&postProcessURL=../components/emxComponentsConfigurableTablePeopleMoveProcess.jsp?rowIds="+encodeURIComponent(personsSelectedIds)+"&parentOID="+encodeURIComponent(contextComp);
			getTopWindow().showSlideInDialog(strMovePersonUrl,"true");
		}
	}	
