<%-- emxEngineeringChangeImpactAnalysisSearchDialog.jsp
   ...
   static const char RCSID[] = $Id: emxEngineeringChangeImpactAnalysisSearchDialog.jsp.rca 1.8 Wed Oct 22 16:18:11 2008 przemek Experimental przemek $
--%>
<!-- Include directives -->
...
<%
try {
    String strStar = FrameworkProperties.getProperty(context,"emxComponents.Search.Star");
    ...
    String strAllStates = i18nNowInstance.GetString("emxComponentsStringResource",strLocale,"emxComponents.Form.Radio.AllStates");
    ...
    String strTypeDisplay = emxGetParameter(request, "TypeDisplayImpactAnalysis");
    String strTypeActual = emxGetParameter(request, "TypeImpactAnalysis");
    ...
    if ( strTypeDisplay!=null && !strTypeDisplay.equals("") ) {
        ...
        strType =DomainConstants.TYPE_IMPACT_ANALYSIS;
      ...
      <table border="0" width="99%" cellpadding="5" cellspacing="2">
        <tr>
          ...
          <td class="inputField" nowrap="nowrap" colspan="1">
             <input type="text" READONLY name="TypeDisplayImpactAnalysis" value="<%=strTypeToDisplay%>" maxlength="" size="20" readonly="readonly"/><input type="hidden" name="TypeImpactAnalysis"  value="<%=strType%>"/><input type="button" name="button1" value="..." onclick="javascript:showTypeChooser();"/>
          </td>
        </tr>
        ...
        <tr>
          ...
          <td class="inputField" nowrap="nowrap" colspan="1">
             <input type="text" name="Name" maxlength="" size="20" value="<xss:encodeForHTMLAttribute><%=strStar%></xss:encodeForHTMLAttribute>"/>
          </td>
        </tr>
        ...
        <tr>
          ...
          <td class="inputField" colspan="1">
            <input type="text" name="Description" maxlength="" size="20" value="<xss:encodeForHTMLAttribute><%=strStar%></xss:encodeForHTMLAttribute>"/>
          </td>
        </tr>
        ...
        <tr>
          ...
          <td class="inputField" colspan="1">
            <input type="text" READONLY name="OwnerDisplay" value="<%=strStar%>" maxlength="" size="20" readonly="readonly"/><input type="hidden" name="Owner"  value="<%=strStar%>"/><input type="button" name="button2" value="..." onclick="javascript:showOwnerChooser();"/>&nbsp;<a class="dialogClear" href="#" onclick="document.editDataForm.OwnerDisplay.value='<%=strStar%>'"><%=strClear%></a>
          </td>
        </tr>
...
turnOffProgress();
 function showTypeChooser()
  {
    showChooser('../common/emxTypeChooser.jsp?frameName=searchPane&formName=editDataForm&SelectType=single&SelectAbstractTypes=true&InclusionList=type_ImpactAnalysis&ObserveHidden=true&ShowIcons=true&fieldNameActual=TypeImpactAnalysis&fieldNameDisplay=TypeDisplayImpactAnalysis&ReloadOpener=true',500,400);
  }

 function showOwnerChooser()
   {

	 //Modified by Infosys for Bug#304580 on 18 May 2005
     showChooser('../components/emxCommonSearch.jsp?formName=editDataForm&frameName=formEditDisplay&searchmode=PersonChooser&suiteKey=Components&searchmenu=APPECSearchAddExistingChooser&searchcommand=SearchECOwnerLink&fieldNameActual=Owner&fieldNameDisplay=OwnerDisplay',700,500);
   }

 function reload()
    {
      document.editDataForm.method= "post";
      document.editDataForm.action="../components/emxEngineeringChangeImpactAnalysisSearchDialog.jsp?program=emxCommonEngineeringChange:getAllImpactAnalysis&table=APPImpactAnalysisList&CommandName=APPSearchECImpactAnalysisCommand&sortColumnName=Name&header=emxComponents.Heading.Search&Style=dialog&HelpMarker=emxhelpsearchresults&suiteKey=Components&StringResourceFileId=emxComponentsStringResource&SuiteDirectory=components&defaultSearch=APPSearchECImpactAnalysisCommand&Policy=null";
