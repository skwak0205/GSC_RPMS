  ...
  if( oids != null)
  {
    ...
    try
    {
	  ...
	  CommonDocument commonDocument = (CommonDocument)DomainObject.newInstance(context,oid);
	      ...
	      DocumentFlexiRevision docflexi = new DocumentFlexiRevision();
	      objectId = docflexi.reviseDocument(context, oid, copyFiles);
          }
          isRevised = true;
     }
    } catch (Exception ex)
...
<script language="Javascript" >
  ...
  if(contentFrame && contentFrame.document.location.href.indexOf("emxTree.jsp") >= 0){
	   ...
	   var strHref = contentFrame.document.location.href;
	   if (strHref.indexOf("&treeMenu=type_TaskTemplate") >= 0) {
			strHref = strHref.replace("&treeMenu=type_TaskTemplate", "");
		}
	   contentFrame.document.location.href = replaceObjectId(strHref,'<%=XSSUtil.encodeForJavaScript(context, objectId)%>');
   } else {   
