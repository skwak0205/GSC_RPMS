...
function _jsDiscussion_drawReply(d, objReply) {
  ...
  if (this.viewMode != DISCUSSION_FLAT)
  {
      if (msgType != PRIVATE_MESSAGE)
      {
          ...
          d.write(objReply.nodeID + "','" + objReply.messageId + "', 'Public");
 ...
 if(this.boolVal == 'true'){
      d.write("<a href=\"javascript:parent.createDiscussionReply('");
      d.write(objReply.nodeID + "','" + objReply.messageId + "', 'Private");
      d.write("')\"> <img align=\"absmiddle\" border=\"0\" src=\"");
...
//  (nothing)
//-----------------------------------------------------------------
function _jsDiscussion_createRoot(strSubject, strMessage, strAuthor, strDate,strMessageID, strMessageType,showSubscription) {

  //set the root (NCZ, 8/1/02)
  this.root = new jsReply(strSubject, strMessage, strAuthor, strDate,strMessageID, strMessageType);
...
function jsReply (strSubject, strMessage, strAuthor, strDate, strMessageID, showDelete, strExpandURL, strID, strMessageType) {
...
//-----------------------------------------------------------------
   function _jsReply_addChild(strSubject, strMessage, strAuthor, strDate, strMessageID, showDelete, strExpandURL, strID, strMessageType) {

  //check for duplicates (NCZ, 8/1/02)
  //if (this.loaded && this != this.tree.root) return;
  
  //create the new node (NCZ, 8/1/02)
  var objReply = new jsReply(strSubject, strMessage, strAuthor, strDate,strMessageID, showDelete, strExpandURL, strID, strMessageType);
