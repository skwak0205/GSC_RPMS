...
<xsl:template match="/">
	...
	<xs:schema attributeFormDefault="unqualified" version="1.0" targetNamespace="http://www.3ds.com/xsd/3DXML/{$ModelerName}/{$CustoName}"  xmlns:xs3d="http://www.3ds.com/xsd/3DXML" xmlns:xs="http://www.w3.org/2001/XMLSchema" elementFormDefault="qualified">
	  ...
	  <!-- For each Class we define a Complex Type for the resutling XSD -->
	  <xsl:for-each select="metadata:Package/metadata:Class">
		    ...
		    <xs:complexContent>
		      ...
		      <xs:extension base="xs3d:{attribute::SuperName}Type">
		        <xs:sequence>
				  ...
				  <!-- For Public attribute we define a new element -->
				  <xsl:for-each select="metadata:Simple[@Protection='User' or @Protection='Free' or @Protection='External' or @Protection='ExternalRO']">
