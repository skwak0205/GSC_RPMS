...
<%
//System.out.println("#########HD_EMXSECURITYCONTEXTSELECTION.jsp ####################################");
...
//System.out.println("#########redirectParam.jsp 22####"+redirectParam);

String redirectParam2 = (String)request.getHeader("redirectParam");
String redirectParam3 = (String)response.getHeader("redirectParam");

//System.out.println("#########redirectParam2###"+redirectParam2);
//System.out.println("#########redirectParam3###"+redirectParam3);
...
%>
            <script>
                ...
                var ticket="<%=XSSUtil.encodeForURL(context,request.getParameter("ticket"))%>";
				 var mode= "<%=XSSUtil.encodeForURL(context,request.getParameter("mode"))%>";
				var redirectParam= "<%=XSSUtil.encodeForURL(context,request.getParameter("redirectParam"))%>";
					
				
                submitWithCSRF("SSO_emxSecurityContextSelectionProcess.jsp?widgetId=<%=XSSUtil.encodeForJavaScript(context, emxGetParameter(request, "widgetId"))%>&SecurityContext=<%=XSSUtil.encodeForURL(context, securityContext)%>&rToken=" + rToken + "&port=" + port + "&ticket=" + ticket+"&redirectParam=" + redirectParam+"&mode=" + mode, window); 
            </script>
<%
			return;
...
%>
				...
				var redirectParam= "<%=XSSUtil.encodeForURL(context,request.getParameter("redirectParam"))%>";
                    ...
                    submitWithCSRF("SSO_emxSecurityContextSelectionProcess.jsp?widgetId=<%=XSSUtil.encodeForJavaScript(context, emxGetParameter(request, "widgetId"))%>&Organization=<%=XSSUtil.encodeForURL(context, (String)organizations.get(0))%>&Project=<%=XSSUtil.encodeForURL(context, (String)projects.get(0))%>&rToken=" + rToken + "&port=" + port + "&ticket=" + ticket+"&redirectParam=" + redirectParam+"&mode=" + mode, window); 
                </script>
<%
            }
        }
%>
           ...
           <form id="loginForm" name="loginForm" method="post" action="SSO_emxSecurityContextSelectionProcess.jsp?rToken=<%=XSSUtil.encodeForURL(context,request.getParameter("rToken"))%>&port=<%=XSSUtil.encodeForURL(context,request.getParameter("port"))%>&ticket=<%=XSSUtil.encodeForURL(context,request.getParameter("ticket"))%>&redirectParam=<%=XSSUtil.encodeForURL(context,request.getParameter("redirectParam"))%>&mode=<%=XSSUtil.encodeForURL(context,request.getParameter("mode"))%>">
...
<%
    ...
    loginurl = "/SSO_" + loginurl.split("/")[1];
