...
%>
    ...
    if (forwardURL != null && disableAutoLogin != null && !"".equals(disableAutoLogin)) {
      forwardURL = forwardURL + ((forwardURL.indexOf("?"))>=0?"&":"?") + "disableAutoLogin="+XSSUtil.encodeForURL(disableAutoLogin);
    }
    if (forwardURL != null && SecurityContext != null && !"".equals(SecurityContext) ) {
      forwardURL = forwardURL + ((forwardURL.indexOf("?"))>=0?"&":"?") + "SecurityContext="+XSSUtil.encodeForURL(SecurityContext);
    }
    if ((FrameworkTarget != null) && (!"".equals(FrameworkTarget)))
    {
        if( FrameworkTarget.contains("emxNavigator.jsp") && forwardURL != null )
        {
            ...
            if (disableAutoLogin != null && !"".equals(disableAutoLogin)) {
              FrameworkTarget = FrameworkTarget + ((FrameworkTarget.indexOf("?"))>=0?"&":"?") + "disableAutoLogin="+XSSUtil.encodeForURL(disableAutoLogin);
            }
            if (SecurityContext != null && !"".equals(SecurityContext) ) {
              FrameworkTarget = FrameworkTarget + ((FrameworkTarget.indexOf("?"))>=0?"&":"?") + "SecurityContext="+XSSUtil.encodeForURL(SecurityContext);
            }
...
<%
      ...
      if (sbLicenseErrors != null && !"".equals(sbLicenseErrors.toString())){
          %>
				<!-- //XSSOK -->
				<emxUtil:i18n localize="i18nId"><%=errorKey%></emxUtil:i18n><%=error%>
          <%
			} else {
          %>
				<!-- //XSSOK -->
