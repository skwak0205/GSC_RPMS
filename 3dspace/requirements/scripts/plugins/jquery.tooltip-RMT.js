...
var dw_Tooltip = {
    ...
    getDefaultsList: function () {
        return [['offX', 'number', 12], ['offY', 'number', 12], ['showDelay', 'number', 100], ['hideDelay', 'number', 100], ['hoverDelay', 'number', 500], ['tipID', 'string', 'type-ahead-body'], ['actuatorClass', 'string', 'showTip'], ['maxLoops', 'number', 2], ['supportTouch', 'boolean', false], ['activateOnClick', 'boolean', false], ['showCloseBox', 'boolean', false], ['closeBoxImage', 'string', ''], ['activateOnFocus', 'boolean', true], ['content_source', 'string', 'content_vars'], ['on_activate', 'function', function () {}], ['on_show', 'function', function () {}], ['on_position', 'function', function () {}], ['on_deactivate', 'function', function () {}], ['on_hide', 'function', function () {}]];
    },
    ...
    activate: function (e, tgt, msg, id) {
        ...
        // Customization to set showDelay
        var varsForShowDelay = _this.content_vars || {};
        var objForShowDelay = varsForShowDelay[id] ? varsForShowDelay[id] : false;
        var originalShowDelay = _this.showDelay;
        
        if (typeof objForShowDelay == 'object') {
            _this.checkForProps(objForShowDelay);
            if (objForShowDelay['showDelay'] && typeof objForShowDelay['showDelay'] == 'number')
                _this.showDelay = objForShowDelay['showDelay'];
        }

        // Customization to set onActivate
        var varsForOnActivate = _this.content_vars || {};
        var objForOnActivate = varsForOnActivate[id] ? varsForOnActivate[id] : false;
        var originalOnActivate = _this.on_activate;
        
        if (typeof objForOnActivate == 'object') {
            _this.checkForProps(objForOnActivate);
            if (objForOnActivate['on_activate'] && typeof objForOnActivate['on_activate'] == 'function')
                _this.on_activate = objForOnActivate['on_activate'];
        }
        ...
        // Customization to set showDelay
        _this.showDelay = originalShowDelay;
        
        // Customization to set onActivate
        _this.on_activate = originalOnActivate;
    },
    ...
    getActuatorInfo: function (tgt) {
        ...
        if (!cls) return '';
        var vars = _this.content_vars || {}, msg = '',
            id = '',
            el;
...
dw_Tooltip.handleWrapOpts = function (str, obj) {
    ...
    if (_this.sticky || _this.activateOnClick || _this.forTouch) {
        str = _this.showCloseBox ? _this.wrapWithCloseBox(str, caption) : str; // NOP _this.wrapWithCloseLink(str, caption);
    } else if (caption) {
        str = '<div class="caption">' + caption + '</div>' + str;
    }
...
dw_Event.add(window, 'load', function () {
    dw_Util.makeClickable('span.showTip');
});

dw_Tooltip.defaultProps = {
    klass: 'type-ahead type-ahead-body',
    showCloseBox: false, // when sticky true (below),
    closeBoxImage: '../common/images/iconActionDelete.gif',
    followMouse: false
};
