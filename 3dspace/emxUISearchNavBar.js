...
//
//   static const char RCSID[] = $Id: emxUISearchNavBar.js.rca 1.27 Wed Oct 22 16:09:35 2008 przemek Experimental przemek $
//   ================================================================
...
if (!objMgr)
  objMgr = getTopWindow().getWindowOpener().getTopWindow().objMgr;
...
//IR-144467V6R2013 ; IE9
var path = document.location.pathname;
var root = path.substring(0, path.indexOf("/",1));
DIR_SEARCHNAVBAR = root + "/common/images/";    
DIR_STYLES = root + "/common/styles/";
...
function _jsNavBar_draw() {
  ...
  var d = new (getTopWindow().jsDocument);
...
function clickMenuItem(strName, iMenuID, iMenuItemID) {
  ...
  if (objMenuItem.isPopup) {
    getTopWindow().showModalDialog(objMenuItem.url, 600, 600, true);   
    //getTopWindow().showDialog(objMenuItem.url)
...
function hideNavBar(bNavBarMulti) {
  ...
  if (frameTreeDisplay) {
    //build the URL
    emxcontenturl = getTopWindow().addURLParam(emxcontenturl, "tree=true"); 
  
    //save the tree
    //objMgr.saveTree();
  } else {
    //build the URL
    emxcontenturl = getTopWindow().addURLParam(emxcontenturl, "tree=false"); 
...
function showNavBar(bNavBarMulti) {    
  ...
  //build content URL 
  emxcontenturl = getTopWindow().addURLParam(emxcontenturl, (frameTreeDisplay ? "tree=true" : "tree=false"));                  
