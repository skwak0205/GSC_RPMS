...
if ((null != selTypesId) )
{
	...
	if (null != manager)
	{
		try
		{
				...
				if (null != data)
				{
					...
					if (null != type){
						String typePublishStatus = "";
					  	...
					  	if(type.canBeModified(context) && !type.isDeprecated(context) && !typePublishStatus.equals(DAuCustoDictionary.COMPARATOR_STATUS_NO))
					  	{
							type.setDeprecated(context,true,true);
							nbDeprecatedTypeExt++;
					  	}
						...
						if(null != ext){ 
							...
							if(ext.canBeModified(context) && !ext.isDeprecated(context) && !extPublishStatus.equals(DAuCustoDictionary.COMPARATOR_STATUS_NO))
							{
							 	ext.setDeprecated(context,true,true);
							 	nbDeprecatedTypeExt++;
							}
			...
			System.out.println("Deprecated OK : " + nbDeprecatedTypeExt + " deprecated on " + nbSelectedTypeExt );
			ArrayList<String> nbTypeExt = new ArrayList<String>();
			nbTypeExt.add(Integer.toString(nbDeprecatedTypeExt));
			nbTypeExt.add(Integer.toString(nbSelectedTypeExt));
			String setTypeExtDeprecatedOK = DAuUtilities.getSpecializationStringResource(context,"emxDataSpecialization.DeprecatedTypeExtOK",nbTypeExt);
			emxNavErrorObject.addMessage(setTypeExtDeprecatedOK);
