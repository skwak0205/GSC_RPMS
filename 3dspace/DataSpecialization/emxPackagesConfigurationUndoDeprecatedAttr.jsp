...
if ((null != selAttrsIds) )
{
	...
	if (null != manager)
	{
		try
		{
			for (k = 0; k<nbSelectedAttr; k++)
			{
				...
				if (null != attr){
				  String attrPublishStatus = attr.getPublishStatus(context);
					...
					{
						attr.setDeprecated(context,false, true);
						nbUndoDeprecatedAttr++;
					}
			...
			System.out.println("Undo Deprecated OK : " + nbUndoDeprecatedAttr + " deleted on " + nbSelectedAttr );
			ArrayList<String> nbAttr = new ArrayList<String>();
			nbAttr.add(Integer.toString(nbUndoDeprecatedAttr));
			nbAttr.add(Integer.toString(nbSelectedAttr));
			String setDeprecatedAttrOK = DAuUtilities.getSpecializationStringResource(context,"emxDataSpecialization.UndoDeprecatedAttrOK",nbAttr);
