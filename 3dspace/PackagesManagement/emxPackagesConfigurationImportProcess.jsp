...
<%        
	...
	if (null != manager) {
		try {
			...
			System.out.println(st1 + ": >> After the import and indexation, we are in the JSP");
			
			/*
    		// BMN2 25/01/2021: IR-824801-3DEXPERIENCER2019x
			...
			//}
				Date t2 = new Date();
				String st2 = t2.toString();
				System.out.println(st2 + ": >> the computation for TypeLookup ended, we are in the JSP");
				System.out.println(st2 + ": >>> Time to do TypeLookup: " + DAuUtilities.ellapsedTime(t1, t2));
			emxNavErrorObject.addMessage(resMsg);			
			%>			
			<%@ include file = "emxPackagesConfigurationCommitTransaction.inc" %>		
			<%
			Date t3 = new Date();
			String st3 = t3.toString();
			System.out.println(st3 + ": >> we are quitting the JSP");
		}
		catch (Exception e) {
