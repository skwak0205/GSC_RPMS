export default function(Nls) {
	...
	CKEDITOR.dialog.add('customtableDialog', function(editor) {
		...
		return {
			...
			onShow: function() {
				...
				if (selected && selected.is('table')) table = selected;
			...
			onOk: function() {
				...
				var table = this._.selectedElement || wrapper.append(makeElement('table')),
					data = {};
									...
									children: [
										...
										},
										{
											...
											setup: function(selectedTable) {
												...
												else this.setValue(dialog.hasColumnHeaders ? 'col' : '');
											},
											commit: commitValue
										},
										{
											...
											commit: function(data, selectedTable) {
												...
												else selectedTable.removeAttribute('align');
											}
										},
										{
											id: 'caption',
											...
											label: Nls.Caption,
											setup: function(selectedTable) {
												if (selectedTable.$.caption) {
													this.setValue(selectedTable.$.createCaption().innerHTML);
												}
											},
											commit: function(data, selectedTable) {
												if (this.getValue()) {
													var table = selectedTable.$.createCaption();
													table.innerHTML = this.getValue();
												} else {
													selectedTable.$.deleteCaption();
												}
											}
										}
									]
											...
											children: [
												{
													...
													validate: CKEDITOR.dialog.validate.cssLength(
														editor.lang.common.invalidCssLength.replace(
															'%1',
															editor.lang.common.width
														)
													),
													onChange: function() {
														var styles = this.getDialog().getContentElement(
															'advanced',
															'advStyles'
														);
											...
											children: [
												{
													...
													label: Nls.DisplayBorder,
													items: [ [ Nls.Yes, 1 ], [ Nls.No, 0 ] ],
													setup: function(selectedTable) {
														this.setValue(selectedTable.getAttribute('border') || 1);
													},
													commit: function(data, selectedTable) {
														if (this.getValue())
															selectedTable.setAttribute('border', this.getValue());
														else selectedTable.removeAttribute('border');
													}
												}
											]
										}
