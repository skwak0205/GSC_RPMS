...
function showAndGetNonModalDialog(strURL, intWidth, intHeight, bScrollbars) {
	if(typeof strURL != "undefined" && strURL) {
		//Any URL append has to happen only when there is (valid ?) URL, otherwise, some browsers use the referring URL by default and some browsers use null.
		//In either case of browser operation, security may be compromised.
		if(arguments.length > 4 && arguments[4] == "openAsContent"){
			//don't add targetLocation to the URL.
...
function createRequestForm(query, fromDataGrid, fromDeleteDataGrid) {
  ...
  for (i in keypairs)
  {
	...
	if(typeof fromDeleteDataGrid != "undefined"){
		keyValue =decodeURIComponent(keyValue);
	}else if(typeof fromDataGrid != "undefined"){
		keyValue =(keyValue);
	}else{
...
function isComponentPage(strURL) {
	...
	if(strURL.indexOf("emxNavigatorDialog.jsp") < 0){
		icp = (isTablePage(strURL) || isFormPage(strURL) || isTreePage(strURL) ||  isNavigatorPopup(strURL) ||  isSelectUserGroupPopup(strURL)) ? true : false;
	}
	return icp;
}

function isSelectUserGroupPopup(strURL) {
	var ind = false;
	if(strURL.indexOf("emxRouteSelectUsers.jsp") >= 0){
		ind = true;
	}
	return ind;
}
...
function openNavigatorDialog(strURL, strFeatures, isApplyIEMask){
	//datagrid
	var dialogPage;
	if (strURL && strURL.indexOf("isGrid")>=0)
		dialogPage = "../../common/emxNavigatorDialog.jsp";
	else
		dialogPage = "../common/emxNavigatorDialog.jsp";
