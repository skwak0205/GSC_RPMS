...
function _registerDnd() {
	...
	$.ui.plugin.add("draggable", "connectToFancytree", {
		...
		drag: function(event, ui) {
			...
			if(targetNode){
				if(highlightNode && (targetNode != prevTargetNode)){
				      highlightNode.removeClass('drop-no');
				      highlightNode.removeClass('drop-yes');
			    }

			    if(targetNode!= null && targetNode.span){
			    	highlightNode = $(targetNode.span).find('span.fancytree-title');
			    }

				if(!targetNode.tree.options.dnd.dragDrop) {
					// not enabled as drop target
					//highlightNode.addClass('drop-no');
				} else if(targetNode === prevTargetNode) {
					// Moving over same node
					targetNode.tree.ext.dnd._onDragEvent("over", targetNode, sourceNode, event, ui, draggable);
					if(classificationDnDValidation(targetNode) != classificationDnDValidation(sourceNode) ){
						highlightNode.addClass('drop-no');						
					}
						
				}else{
					// Entering this node first time
					if(classificationDnDValidation(targetNode) != classificationDnDValidation(sourceNode) ){//p9y to avoid DnD form shared to personal Workspaces.  
						 highlightNode.addClass('drop-no');						 
					}else if(!dndValidation(sourceNode.data.type,targetNode.data.type, true,false)){//JUK
						//logMsg("ERROR JUk"+ sourceNode.data.type +".."+targetNode.data.type);
						highlightNode.addClass('drop-no');
					}else{
					highlightNode.addClass('drop-yes');
						//logMsg("SUCCESS JUk"+ sourceNode.data.type +".."+targetNode.data.type);
					}
...
//          helper.removeClass("fancytree-drop-hover");
	...
	_onDragEvent: function(eventName, node, otherNode, event, ui, draggable) {
		...
		case "helper":
			...
			//juk $("ul.fancytree-container", node.tree.$div).append($helper);
			$("body").append($helper);
