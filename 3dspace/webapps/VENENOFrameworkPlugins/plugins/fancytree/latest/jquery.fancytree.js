...
function FancytreeNode(parent, obj){
	if(obj){
	//obj is having null on the last call this a plugin bug
	var i, l, name, cl;
...
//                dict.children = null;
	...
	toggleExpanded: function(expandNode){
		return this.tree._callHook("nodeToggleExpanded", this, expandNode);
	},
...
//      this.tree.logDebug("ftnode.onClick(" + event.type + "): ftnode:" + this + ", button:" + event.button + ", which: " + event.which);
		...
		if( targetType === "expander" ) {
			...
			if( activate ) {
				//CUSTOM CODE to NOT to seelct node until if user clicks on the NODE it self
				if(ctx.targetType != undefined){//CUSTOM				
...
//        node.span.className = cnList.join(" ");
		...
		if(isActive === flag){
			...
			if(getTopWindow().bclist){
				...
				if(currbc && currbc.categoryObj){
					$.each(currbc.categoryObj.items,function(i, item){
						if(currbc.categoryObj.defaultObjCategoryName){
							if(item.dynamicName && item.dynamicName == currbc.categoryObj.defaultObjCategoryName){
								getTopWindow().bclist.changeCategory(node.key, {item : item});
								return false;
							}
						} else if(item.dynamicName && item.dynamicName.startsWith("type_")){
							getTopWindow().bclist.changeCategory(node.key, {item : item});
							return false;
						}
					});
	 ...
	 */
	nodeSetExpanded: function(ctx, flag, callOpts, expandMode) {
		callOpts = callOpts || {};
		...
		var expandMode = expandMode !== false;
		...
		_afterLoad = function(callback){
			...
			if( node.ul ) {
				...
				if ( isVisible === isExpanded ) {
					...
					//Custom change to fix node expand/collapse issue
					$(node.ul).toggle(effect.duration, function(){
...
//			this._fixSelectionState(node);
	 ...
	 * @param {EventData} ctx
	 */
	nodeToggleExpanded: function(ctx, expandNode) {
		return this.nodeSetExpanded(ctx, !ctx.node.expanded, null, expandNode);
	},
	/**
	 * @param {EventData} ctx
	 */
