...
var _assert = $.ui.fancytree.assert,
	...
	FOCUS = "focus",
	SELECTED = "selected";
//CUSTOM code tro support the persistence for the 2 trees in same Layout
var sName = location.search.match(/[?&]serviceName=([^&]*)?/);
sName = (sName == null?"":sName[1] || "");
if(location.href.indexOf('getPersonalFolders=true') > 0){
	sName = sName +"PF";
}
...
$.ui.fancytree._FancytreeClass.prototype.getPersistData = function(){
	...
	res[ACTIVE] = local._data("ft-" + ACTIVE);//prefix
...
$.ui.fancytree.registerExtension({
	...
	options: {
		cookieDelimiter: "~",
		cookiePrefix: sName+"-",//CUSTOM CODE undefined, // 'fancytree-<treeId>-' by default
		cookie: {
			raw: false,
			expires: "",
			path: "",
...
//									node.setSelected();
				...
				if(local.storeActive){
					cookie = local._data("ft-"+ ACTIVE);//local.cookiePrefix
					if(cookie && (opts.persist.overrideSource || !tree.activeNode)){
						node = tree.getNodeByKey(cookie);
						if(node){
							//CUSTOM CODE to open the active node's link
							emxUICore.link("javascript:fancyTreeNodeClick('"+node.data.objectId+"','"+node.data.relId+"','"+node.data.type+"')");//juk
							node.setActive();
							$("#fancyTreeMainBody")[0].scrollTop = node.span.offsetTop;//To scrollTo the active Node
						}
					}
				}
	...
	nodeSetActive: function(ctx, flag, opts) {
		...
		if(local.storeActive){
			local._data("ft-" + ACTIVE, this.activeNode ? this.activeNode.key : null);//local.cookiePrefix
		}
		return res;
	},
