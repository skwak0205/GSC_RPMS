...
$.ui.fancytree._FancytreeNodeClass.prototype.editEnd = function(applyChanges, _event){
	...
	if( eventData.save ) {
		...
		//node.setFocus();//CUSTOM CODE not to set foucs after RENAME/Create
	}else{
		if( eventData.isNew ) {
			node.remove();
	...
	//$(tree.$container).focus();//CUSTOM CODE IN IE it is failing
...
$.ui.fancytree.registerExtension({
	...
	options: {
		adjustWidthOfs: null,   //4 .. null: don't adjust input size to content
		allowEmpty: false,   // Prevent empty input
		inputCss: {minWidth: "12em"},
...
//		keypress: $.noop,    // Not yet implemented
	...
	nodeDblclick: function(ctx) {
		//CUSTOM CODE To return if user clicked on NODE expand button
