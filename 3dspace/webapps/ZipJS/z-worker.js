...
(function main(global) {
	...
	function doImportScripts(msg) {
		...
		global.postMessage({ type: 'importScripts' });
	}
	...
	function newTask(msg) {
		...
		global.postMessage({ type: 'newTask', sn: sn });
	}
	...
	function processData(msg) {
		...
		if (isAppend) {
			try {
				output = task.codec.append(input, function onprogress(loaded) {
				    global.postMessage({ type: 'progress', sn: sn, loaded: loaded });
				});
			} catch (e) {
				delete tasks[sn];
				throw e;
		...
		try {
		    global.postMessage(rmsg, transferables);
		} catch(ex) {
		    global.postMessage(rmsg); // retry without transferables
		}
	}
