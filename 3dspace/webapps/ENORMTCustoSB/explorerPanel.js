...
//quickreview KIE1  ZUD     05:07:19    IR-661920-3DEXPERIENCER2020x: Resizing an RSP view with the quickcharts active widening the screen leads to a white bar
...
define('DS/ENORMTCustoSB/explorerPanel', [
	...
	if (bShowTreeExplorerAndDecorators) {
	    ...
	    if (!bFirstShowDynaTreeSlideContainer) {
			...
			//Against IR-810533-3DEXPERIENCER2021x If toolbar already loaded customize toolbar for RMT 
			if($('#divToolbarContainer').find('.toolbar').first().find('td').length > 0)
	...
	if(!isSBEmbedded && bShowTreeExplorerAndDecorators){
		...
		win.jQuery(".categories LI:not(:first)[name!='li_RMTChapterStructureTreeCategory']").click(hideToggleIcons);
		win.jQuery(".categories LI[name='li_RMTChapterStructureTreeCategory']").click(function() {
			getTopWindow().jQuery("div#togglecat").show();
		});
		win.jQuery(".categories LI:first[name!='li_type_Chapter']").click(function() {
			getTopWindow().jQuery("div#togglecat").show();
		});
...
function createContainerForDynaTree() {
	...
	//++VMA10 ZUD : updated :   IR-720314-3DEXPERIENCER2020x
	//PLA3 updated: IR-750892-3DEXPERIENCER2021x
    sideBarSB.append('<div class="toggler_dynaTree" style="width: ' + widthContainer +
        ...
        '<div id="requirementTreeExplorer" style="height:calc(100% - 68px);"> </div>' +
        '</div>' +
        '</div>');
		//--VMA10 ZUD : updated :   IR-720314-3DEXPERIENCER2020x
    ...
    $("#dynaTreeSlideContainer").resizable({
        ...
        stop: function(event, ui) {
            ...
            var oldWidth = ui.originalSize.width;
               ...
               // ++VMA10 ZUD added IR-683076-3DEXPERIENCER2020x
            if($("#dynaTreeSlideContainer").width() < widthContainer){
            	$("#dynaTreeSlideContainer").width(widthContainer-2)
            	
            	 if (newWidth > oldWidth) {
                     toSlide = '+=' + (newWidth - oldWidth-6) + 'px';
            	 ...
            	 if (newWidth > oldWidth) {
                     ...
                     toSlide = '-=' + (oldWidth - newWidth+6) + 'px';
                 }
            ...
            var height = $("#dynaTreeSlideContainer").height();
            $("#dynaTreeSlideContainer").height(height+7);
			 // --VMA10 ZUD added IR-683076-3DEXPERIENCER2020x
        }
    });
    ...
    if(!isSBEmbedded) {
		if(getTopWindow().objStructureFancyTree != undefined) //++VMA10 ZUD : IR-703867-3DEXPERIENCER2020x : If block added
		{
...
function displayChapterNumber(){
	var objectList=$("#treeBodyTable > tbody").find('div tr');
	var rows = emxUICore.selectNodes(oXML, "/mxRoot/rows//r");
	var arrayLength=16;
	var chapIndextable =Array(arrayLength).fill(0)
	var preLevel = 0;
	for (var i = 0; i < rows.length; i++) {
		var type = rows[i].getAttribute("type");
		var rowContent = rows[i];
		if(type == "Chapter" && preLevel < arrayLength){
		    ...
		    var chapterIndex = "";
			  if(level < preLevel)
						...
						{
							if(chapIndextable[j] != 0)
							AppendIndex=AppendIndex + chapIndextable[j] + ".";
						}
						chapterIndex =  AppendIndex + chapIndextable[level] ;
				
			if(objectList.filter("[id='" + objectRowId + "']")[0] != undefined)
				{objectList.filter("[id='" + objectRowId + "']")[0].lastElementChild.prepend(chapterIndex+' - ');	}
...
function DnDForSBRMT() {
    ...
    var title_column = colMap.columns["DUP_Title"];
    if(title_column == null || title_column === undefined)
    {
    title_column = colMap.columns["Title"];
    }
    var index = title_column ? title_column.index : -1;
    if(index == 1)
    {
