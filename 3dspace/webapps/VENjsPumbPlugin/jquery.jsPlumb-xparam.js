	...
	*/
    ...
    var log = null,
		...
		_draw = function(element, ui, timestamp, clearEdits) {
			var hasSplitTask = element.hasSplitTask;
            ...
            if (!jsPlumbAdapter.headless && !_suspendDrawing) {
			    ...
			    _currentInstance.anchorManager.redraw(id, ui, timestamp, null, clearEdits,hasSplitTask);
			    if (repaintEls) {
		...
		*/
		_finaliseConnection = function(jpc, params, originalEvent) {
			...
			//Modified for user group split task functionality
			jpc.source.hasSplitTask = params.hasSplitTask;
			// force a paint
			_draw(jpc.source);
		...
		*/
		...
		this.connect = function(params, referenceParams) {
			...
			if (_p) {
				...
				jpc.hasSplitTask = _p.hasSplitTask;
        ...
        this.paint = function(params) {
            ...
            if (visible) {
                ...
                if (timestamp == null || timestamp != lastPaintedAt) {                        
					...
					//for user group split					
					//self.hasSplitTask;
                    // paint the connector.
                    connector.paint(self.paintStyleInUse, null, extents, self.hasSplitTask);  
                    // and then the overlays
                    for ( var i = 0; i < self.overlays.length; i++) {
                        var o = self.overlays[i];
                        if (o.isVisible()) {
                            o.paint(self.overlayPlacements[i], extents, self.hasSplitTask);    
                        }
                    }                  
 ...
 // ************************** / SVG utility methods ********************************************	
	...
	var SvgComponent = function(params) {
		...
		this.paint = function(style, anchor, extents) {	   			
			if (style != null) {
				...
				var nodeToNodeConnection = arguments[3];
	...
	var SvgConnector = jsPlumb.ConnectorRenderers.svg = function(params) {
		...
		_super.renderer.paint = function(style, anchor, extents) {
			...
			//
			if(5 == segments.length && arguments.length >= 4 && arguments[3] == true){
				var updatedCoOrds = segments[4].getCoordinates();
				updatedCoOrds.x2 += 65;
				segments[4].setCoordinates(updatedCoOrds);
				offset[0] -= 65;
			}
			// create path from segments.	
			for (var i = 0; i < segments.length; i++) {
	...
	var AbstractSvgArrowOverlay = function(superclass, originalArgs) {
    	...
    	this.paint = function(params, containerExtents) {
    		...
    		if (params.component.svg && containerExtents) {
	    		...
	    		var nodeToNodeConnection = arguments[2];
