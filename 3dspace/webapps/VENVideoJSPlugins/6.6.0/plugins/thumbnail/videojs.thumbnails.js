...
(function() {
  ...
  videojs.plugin('thumbnails', function(options) {
    ...
    /**handleAddTrack = function handleAddTrack(e) {
      return tracks.addTrack(e.track);
    };**/
    remoteTracks.on('addtrack', function() {
        var numtracks = player.remoteTextTracks().length;
      if (numtracks === 0) {
        return;
      }
      i = 0;
      ...
      moveListener = function(event) {
        var mouseTime, time, active, left, setting, pageX, right, width, halfWidth, pageXOffset, clientRect, offsetProgressHolder = 0;
        active = 0;
        ...
        if (progressControl.el().getElement && progressControl.el().getElement('.vjs-progress-holder')) {
          offsetProgressHolder = progressControl.el().getElement('.vjs-progress-holder').offsetLeft;
        }
        mouseTime = Math.floor((left - offsetProgressHolder) / progressControl.width() * duration);
        ...
        //do not use clip anymore this is deprecated, instead use background-position
        img.style['background-position'] = setting.x+'px '+setting.y+'px';
