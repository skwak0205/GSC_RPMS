...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:template name="tableBody">
           <xsl:apply-templates select="columns">
                  <xsl:with-param name="tblName" select="'treeHeadTable'"/>
                  <xsl:with-param name="path" select="columns/column[position() &lt;= $split]"/>
           </xsl:apply-templates>
    </xsl:template>    		 
	<xsl:template match="columns">
		<xsl:param name="tblName" select="'treeHeadTable'"/>
		<xsl:param name="path" select="column[position() &lt;= $split]"/>
		<table width="100%" id="{$tblName}" >
			...
			<xsl:if test="$uiAutomation = 'true' ">
				...
				<xsl:attribute name="colorize"><xsl:value-of select="$colorize"/></xsl:attribute>
			...
			<xsl:variable name="colName" select="//mxRoot/columns/column/@name"/>			
			<tr>
				<xsl:if test="$uiAutomation = 'true' ">
    ...
    <xsl:if test="($displayMode = 'Both' or $displayMode = $sbMode)">
		...
		<xsl:if test="$cellIndex &lt;= $split and $cellIndex &gt; 1">
			<td id="ROW_{$cellIndex}" class="mx_hidden-row">
				<xsl:attribute name="width">
					<xsl:choose>
						...
						<xsl:when test="(string-length(@label)*15) &gt; 205">
							205
						</xsl:when>
						<xsl:otherwise>
							<xsl:value-of select="(string-length(@name)*12)" />
						</xsl:otherwise>
					</xsl:choose>
				</xsl:attribute>
			</td>
      <td width="{$sizerWidth}" class="mx_hidden-row" />
		</xsl:if>		
	...
	<xsl:if test="settings/setting[@name='Row Number']='1'">
		<xsl:if test="$tblName = 'treeHeadTable' and $currentPosition = '1' and ($reportType = 'Difference_Only_Report' or $reportType = 'Unique_toLeft_Report' or $reportType = 'Unique_toRight_Report' or $reportType = 'Common_Report')" >
			<td height="19">&#160;</td>
			<td/>							
		</xsl:if>
		<xsl:if test="$tblName = 'treeHeadTable' and position() = 1 and $reportType = 'Complete_Summary_Report' and /mxRoot/tableControlMap/setting[@name='HasDiffCodeColumn']/value = 'true'" >
			<td height="19">&#160;</td>
			<td/>							
		</xsl:if>
		<xsl:choose>
			<xsl:when test="settings/setting/@name='Group Header'">	
					<xsl:if test="settings/setting/@name='Group Header' and settings/setting[@name='Col Span']/text() != '0'">
						<td class="mx_group-header">
							<xsl:if test="number((settings/setting[@name='Col Span']/text())*2-1) &gt; 1">
		                        <xsl:attribute name="colspan">
				                  <xsl:value-of select="(settings/setting[@name='Col Span']/text())*2-1"/>
		                	   </xsl:attribute>
		                     </xsl:if>
	...
	<xsl:template match="column">
        ...
        <xsl:if test="(($displayMode = 'Both' or $displayMode = $sbMode) and $displayMode != 'none')">
		...
		<xsl:if test="$tblName = 'treeHeadTable' and position() = 1 and ($reportType = 'Difference_Only_Report' or $reportType = 'Unique_toLeft_Report' or $reportType = 'Unique_toRight_Report' or $reportType = 'Common_Report')">
			<th width="50"><table><tr><td><span><xsl:value-of select="//setting[@name='Labels']//item[@name='Level']/value"/></span></td></tr></table></th>
			<th class="mx_sizer"/>
		</xsl:if>
		<xsl:if test="$tblName = 'treeHeadTable' and position() = 1 and $reportType = 'Complete_Summary_Report' and /mxRoot/tableControlMap/setting[@name='HasDiffCodeColumn']/value = 'true'">
			<th width="50"><table><tr>
			<xsl:choose>
	...
	<xsl:if test="($settingRowNumber/text() = $rowNumber)">
					<xsl:if test="$columnIndex = 1">
						<th width="1" position="-1">
					...
					<table>
						<tr>
							...
							<xsl:choose>
								...
								<td>
									<input type="checkbox" class="small" name="chkList" onclick="setCheckboxStatus(this, false)"/>
								</td>
								</xsl:when>
								...
								<xsl:when test="@type='File'">
									<td>
										<img src="images/iconSmallPaperclipVerticalR.gif" align="absmiddle" border="0"/>
									</td>
								</xsl:when>
