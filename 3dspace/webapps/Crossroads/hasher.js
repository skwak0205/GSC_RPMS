...
define('DS/Crossroads/hasher', [
        'DS/Crossroads/signals'
    ],
    ...
    function(signals) {
        ...
        var hasher = (function(window) {
            ...
            function _escapeRegExp(str) { // eslint-disable-line require-jsdoc
                return String(str || '').replace(/\W/g, "\\$&");
            }

            function _trimHash(hash) { // eslint-disable-line require-jsdoc
                if (!hash) return '';
                var regexp = new RegExp('^' + _escapeRegExp(hasher.prependHash) + '|' + _escapeRegExp(hasher.appendHash) + '$', 'g');
                return hash.replace(regexp, '');
            }

            function _getWindowHash() { // eslint-disable-line require-jsdoc
            ...
            function _getFrameHash() { // eslint-disable-line require-jsdoc
                return (_frame) ? _frame.contentWindow.frameHash : null;
            }

            function _createFrame() { // eslint-disable-line require-jsdoc
            ...
            function _updateFrame() { // eslint-disable-line require-jsdoc
            ...
            function _registerChange(newHash, isReplace, silent) { // eslint-disable-line require-jsdoc
                if (_hash !== newHash) {
                    var oldHash = _hash;
            ...
            function _addListener(elm, eType, fn) { // eslint-disable-line require-jsdoc
            ...
            function _removeListener(elm, eType, fn) { // eslint-disable-line require-jsdoc
            ...
            function _makePath(paths) { // eslint-disable-line require-jsdoc
            ...
            function _encodePath(path) { // eslint-disable-line require-jsdoc
            ...
            hasher = /** @lends hasher */ {
                 ...
                 */
                setHash: function(path, silent) {
                    path = _makePath(path);
                    if (path !== _hash) {
                        // we should store raw value
                        _registerChange(path, false, silent);
                        if (path === _hash) {
                 ...
                 */
                replaceHash: function(path, silent) {
                    path = _makePath(path);
                    if (path !== _hash) {
                        // we should store raw value
                        _registerChange(path, true, silent);
                        if (path === _hash) {
                            // we check if path is still === _hash to avoid error in
                            // case of multiple consecutive redirects [issue #39]
                            if (!hasher.raw) {
