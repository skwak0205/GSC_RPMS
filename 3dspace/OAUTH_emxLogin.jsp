...
//System.out.println("####################################  OAUTH_EMXLOGIN.jsp ####################################");
...
if (Framework.isLoggedIn(request))
{
    ...
    //JPA-
    targetPage = "oslc/OAUTH_emxSecurityContextSelection.jsp?rToken=" + XSSUtil.encodeForURL(request.getParameter("rToken")) + "&port=" + XSSUtil.encodeForURL(request.getParameter("port")) + "&ticket=" + XSSUtil.encodeForURL(request.getParameter("ticket"));
...
%>
    ...
    if ((FrameworkTarget != null) && (!"".equals(FrameworkTarget)))
    {
        if( FrameworkTarget.contains("emxNavigator.jsp") && forwardURL != null )
        {
            ...
            FrameworkTarget = "/" + FrameworkTarget.split("/")[1] + "/" + FrameworkTarget.split("/")[2] + "/OAUTH_" + FrameworkTarget.split("/")[3] + "?rToken=" + XSSUtil.encodeForURL(request.getParameter("rToken")) + "&port=" + XSSUtil.encodeForURL(request.getParameter("port")) + "&ticket=" + XSSUtil.encodeForURL(request.getParameter("ticket"));
