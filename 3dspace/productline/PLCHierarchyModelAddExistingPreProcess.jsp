<%--
  PLCHierarchyModelAddExistingPreProcess.jsp
...
<%
   ...
   try
   {
	   ...
	   String strOtherCollabSpacePL = EnoviaResourceBundle.getProperty(context,"ProductLine","emxProduct.Alert.AddExistingModelUnderOtherCSProductline", strLanguage);
...
%>
		        ...
		        var submitURL = "../common/emxFullSearch.jsp?objectId=<%=XSSUtil.encodeForURL(context,strObjectID)%>&field=TYPES=type_Model&excludeOIDprogram=emxPLCCommon:excludeConnectedModels&table=PLCSearchModelsTable&HelpMarker=emxhelpfullsearch&selection=multiple&showInitialResults=true&hideHeader=true&suiteKey=ProductLine&submitURL=../productline/PLCHierarchyModelAddExistingPostProcess.jsp?suiteKey=ProductLine&StringResourceFileId=emxProductLineStringResource&SuiteDirectory=productline&objectId=<%=XSSUtil.encodeForURL(context,strObjectID)%>&ContextParentOID=<%=XSSUtil.encodeForURL(context,strContextParentOID)%>";
