...
try {
    ...
    for( int i=0; i < vectSize; i++) {  //SECTION R
        ...
        String modifiedName = (String)hmaplist.get("modifiedName");
        ...
        for( int j=0; j < timeArray.size(); j++) {  //SECTION P
			...
			//added as a part of 831795 STARTs
            sAction = sAction.replace('(',' ');
			sAction = sAction.replace(')',' ');
			sAction = sAction.trim();
			//added as a part of 831795 ENDs
            ...
            if(ActionFilterValues.contains(sAction) || showAllHistoryItems ) {
                ...
                historyHashMap.put("sDescription", UINavigatorUtil.getHistoryDescriptionDisplayString(context, sDescription, languageStr, modifiedName));//desc
...
<%
	...
	for (Iterator iter = templateMapList.iterator(); iter.hasNext();) {
        ...
        if(sActionProp.equals(sActionI18N)){
    	   	sActionI18N = sAction;
    	}
...
<%
	...
	{
		msg=EnoviaResourceBundle.getProperty(context, "emxFrameworkStringResource", context.getLocale(), "emxFramework.PageHistory.NoResultsFound");
	}
%>
  <table border="0" cellspacing="0" cellpadding="0" width="100%">
    <tr>
...
<%
    emxNavErrorObject.addMessage("emxHistorySummary : " + msg);
}catch(Exception e) {
