...
String compPortal = emxGetParameter(request,"compPortal");
...
{
    strHeader = "emxFramework.Common.StructureCompare";
}
...
if(UIUtil.isNullOrEmpty(objSelectLimitMsg))
{
    ...
    else //Invoke the Portal
    {
        if(isBPSStructureCompareReport)
        {
            ...
            contentURL = "../common/emxPortal.jsp?SCTimeStamp="+XSSUtil.encodeForJavaScript(context,scTimeStamp)+"&portal=AEFStructureComparePortal&header="+XSSUtil.encodeForJavaScript(context,strHeader)+"&suiteKey=Framework&SuiteDirectory=common&StringResourceFileId=emxFrameworkStringResource&objectId1="+XSSUtil.encodeForJavaScript(context,strObjectId1)+"&objectId2="+XSSUtil.encodeForJavaScript(context,strObjectId2)+"&table="+XSSUtil.encodeForJavaScript(context,strTable)+"&objIDs="+objIDs+"&objectId="+XSSUtil.encodeForJavaScript(context,strObjId)+appendURL.toString()+"&selectedTable="+selectedTable+"&HelpMarker="+XSSUtil.encodeForJavaScript(context,helpMarker);
        }
        else
        {
            isPortalDispaly = true;
	    //The below fix is added in order to load the portal dynamically to support toggle between 3DLiveExamine and 3DPlay -start
            if(UIUtil.isNotNullAndNotEmpty(compPortal)){
            	contentURL = "../common/emxPortal.jsp?SCTimeStamp="+XSSUtil.encodeForJavaScript(context,scTimeStamp)+"&portal="+XSSUtil.encodeForJavaScript(context,compPortal)+"&header="+XSSUtil.encodeForJavaScript(context,strHeader)+"&AppSuiteKey="+XSSUtil.encodeForJavaScript(context,suiteKey)+"&suiteKey="+XSSUtil.encodeForJavaScript(context,suiteKey)+"&objectId1="+XSSUtil.encodeForJavaScript(context,strObjectId1)+"&objectId2="+XSSUtil.encodeForJavaScript(context,strObjectId2)+"&form="+XSSUtil.encodeForJavaScript(context,strForm)+"&submitURL="+XSSUtil.encodeForJavaScript(context,strSubmitURL)+"&HelpMarker="+XSSUtil.encodeForJavaScript(context,helpMarker);
            }
            else{
          ...
          //The below fix is added in order to load the portal dynamically to support toggle between 3DLiveExamine and 3DPlay -end    
        }
    }
