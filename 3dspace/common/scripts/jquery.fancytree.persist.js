		...
		if( foundOne && missingKeyList.length > 0 ) {
			...
			if( missingKeyList.length ) {
				...
				for( i=0, l=missingKeyList.length; i<l; i++ ) {
					key = keyList[i];
					//local._appendKey(EXPANDED, keyList[i], false); //mss1 : Commenting this line for IR-543370
				}
			}
			//.... CUSTOM CODE setTimeout()
			setTimeout(function(){
...
$.ui.fancytree._FancytreeClass.prototype.getPersistData = function(){
	...
	res[ACTIVE] = local._data(prefix + ACTIVE);//prefix
...
$.ui.fancytree.registerExtension({
	...
	options: {
		cookieDelimiter: "~",
		// cookiePrefix: "-",//CUSTOM CODE undefined, // 'fancytree-<treeId>-' by default
		cookie: {
			raw: false,
			expires: "",
			path: "",
	...
	treeInit: function(ctx){
		...
		//To enable persitence or not .... CUSTOM CODE
		//if(tree.$div[0].baseURI.indexOf('persist=false')>0){
        ...
        //mss1-Start: fixed for IR-584732
		tree.$div.bind("fancytreeinit", function(event, inputData){
			var cookie, dfd, i, keyList, node;
			if(inputData && inputData.options){
				local.cookiePrefix = inputData.options.persist.cookiePrefix;
			}
			var prevFocus = $.cookie(local.cookiePrefix + FOCUS); // record this before node.setActive() overrides it;


            // mss1-end: fixed for IR-584732
...
//									node.setSelected();
				...
				if(local.storeActive){
					cookie = local._data(local.cookiePrefix+ ACTIVE);//local.cookiePrefix
					if(cookie && (opts.persist.overrideSource || !tree.activeNode)){
						var objStructureFancyTree = getTopWindow().objStructureFancyTree;
						var isNodeFoundFlag = false;
						if(objStructureFancyTree){
							var strParentNodeId = local._data(objStructureFancyTree.getRootNode().key + "|" +cookie);
							// In case of linked folder - get the required node using the node label
							if(strParentNodeId){
								var linkedParentNode = tree.getNodeByKey(strParentNodeId);
								var childNodeList = linkedParentNode.getChildren();
								if(childNodeList){
									for(var j = 0; j < childNodeList.length; j++){
										if(cookie == childNodeList[j].key){
											var nodeTitle = $('span.fancytree-title',childNodeList[j].li)[0].textContent;
											node = objStructureFancyTree.getNodeByTitle(objStructureFancyTree.getRootNode(), nodeTitle, cookie);
						if(node){
						...
						if(node && !opts.persist.isTreeReInit){
							//CUSTOM CODE to open the active node's link : opts.persist.isTreeReInit boolean value, true if tree is reinit
							 var strTarget = "detailsDisplay";
				             emxUICore.link(node.data.url, strTarget);
				             node.setFocus();
							 //update local storage active key at the sametime as focus while creating or opening a workspace
						     node.setActive();
						}else if(node){
							node.setFocus();
						}
	...
	nodeSetActive: function(ctx, flag, opts) {
		...
		if(local.storeActive){
			local._data(local.cookiePrefix + ACTIVE, this.activeNode ? this.activeNode.key : null);//local.cookiePrefix
		}
		return res;
	},
	nodeSetExpanded: function(ctx, flag, opts, expandMode) {
		var res,
			node = ctx.node,
			local = this._local;

		flag = (flag !== false);
		res = this._super(ctx, flag, opts, expandMode);

		if(local.storeExpanded && expandMode !== false){
