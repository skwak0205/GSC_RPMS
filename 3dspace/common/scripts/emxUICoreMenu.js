	...
	var menu_section = {
			...
			template: {
		        ...
		        menu_group:function(expand){
		        		...
		        		try{
		        			if(!jsonObj.menuItems[i].displayMode.contains(displayView) && (jsonObj.isRMB=="false" || jsonObj.isRMB=="") && jsonObj.menuItems[i].displayMode!=""){
		       		...
		       		var li;
		    	   		if (menuGroupObj.items[j].items) {
		    	   			if(menuGroupObj.items[j].icon && gtb != "true"){
								...
								if(dynamicName != "My Desk"){
									li=toggleMenuGroup(menuGroupObj.items[j],label);
								}else{
					      ...
					      error: function(error, status) {
							...
							if(obj.icon && gtb != "true"){
								if(menuGroupObj.actualName=="APPRelatedFiles"){
									li = jQuery('<li class="document-file"><a href="javascript:void(0)"><span><img HEIGHT="16" WIDTH="16" src='+obj.icon+'></span><label>'+label+'</label></a></li>');
								}else{
									...
									try{
			    		  			...
			    		  			if(dynamicName == "My Desk"){
                                    	...
                                    	if(this.target != 'popup' && this.target != 'slidein'){
											jQuery('#navBar').empty();
											getTopWindow().jQuery("div#mx_divGrabber").css('display',"none");
											/*
												Adding shortcut icon to every page as a part of function 78403
											*/
											var shortcutTooltip =getTopWindow().emxUIConstants.STR_SHORTCUTS;
											var appMenuDiv = getTopWindow().jQuery("<div id='appMenu' class='tab-bar button-bar'></div>");
											...
											getTopWindow().jQuery('#navBar').append(appMenuDiv);												
	                ...
	                if(obj.formFieldName != "emxExpandFilter" && obj.htmlType != "textbox" && obj.htmlType != "checkbox" && obj.htmlType != "combobox"){
			  		...
			  		li.click(function(e){
		    			...
		    			if(dynamicName != "AEFPersonMenu" &&
		    					...
		    					dynamicName != "AEFHelpMenu"){
			  			showMarkOnMenuItem(this,mPanel[0]);
		    			}
			  			if(menuItemClassName == "emxUICalendarYearMenuItem"){
			  				obj.parent.calendar.setYear(parseInt(obj.value));
			  			}
...
emxUICorePopupMenu.prototype.createDOM = function _emxUICorePopupMenu_createDOM(gtb) {
	...
	for(var i=0; i<menuItems.length; i++ ){
		if(menuItems[i].itemObj){
			menuItems[i].itemObj.index = i;
		}
	}
				...
				if((this.isRMB && this.isRMB == "true")){
					...
					if((emxUICore.getWinHeight() - intY) < (this.layer.offsetHeight)) {
		         	...
		         	intY = navy;
  				    ...
  				    if(x && x.id=="displayModeMenu"){
  				    	var menuPanel=jQuery('.menu-panel.display-mode');
					...
					var allLISize = allLI.size()-1;
  					invokeShowMarkOnMenuItem(allLISize,menuPanel,allLI[allLISize]);
  					  }
  		    	  } 
  		 		  if((this.isRMB && this.isRMB == "true" && intY < 0)){
					  intY = 0;
				  }
            	  emxUICore.moveTo(this.layer, intX, intY);
            	  // to add scroll to page menu, if height exceeds
				  if(!(this.isRMB && this.isRMB == "true")) {
					  adjustMenuHeight(jQuery(this.layer).children(".menu-content"), false, strDir);
				  }
            	  ...
            	  this.fnTemp = fnTemp = function (e) {
            		  if(e){
            		  		...
            		  		if(target && jQuery(target).closest(".menu-panel").length ==0) {
            		  			if(!(e.type == "resize" && (getTopWindow().isMobile || getTopWindow().isPCTouch))){
            		  				objThis.hide();
            		  				}
            		  		}
            		  }else{
...
function emxUICoreMenuLink (strIcon, strText, strURL, strTarget,value) {
        this.superclass = emxUICoreMenuItem;
        this.superclass();
        delete this.superclass;
...
emxUICorePopupMenu.prototype.show1 = function _emxUICorePopupMenu_show1(objRef, strDir) {
	   ...
	   function toggleMenuGroup(menuGrpItem,label){
		   if(menuGrpItem.displayMode!=""){  
				var toolbarVisibleMode=menuGrpItem.displayMode;
				toolbarVisibleMode=toolbarVisibleMode.split(",");
				displayView= displayView.toLowerCase();
			if((toolbarVisibleMode=="" || toolbarVisibleMode.find(displayView)==-1) && menuGrpItem.icon){
				return jQuery('<li class="menu collapsed disabled"><a href="javascript:void(0)"><span><img HEIGHT="16" WIDTH="16" src='+menuGrpItem.icon+'></span><label>'+label+'</label></a></li>');}
			else if(!(toolbarVisibleMode=="" || toolbarVisibleMode.find(displayView)==-1) && menuGrpItem.icon){
				return jQuery('<li class="menu collapsed icon"><a href="javascript:void(0)"><span><img HEIGHT="16" WIDTH="16" src='+menuGrpItem.icon+'></span><label>'+label+'</label></a></li>');
			}else if((toolbarVisibleMode=="" || toolbarVisibleMode.find(displayView)==-1)){
				return  jQuery('<li class="menu collapsed disabled"><a href="javascript:void(0)"><label>'+label+'</label></a></li>');
			}else{
				return jQuery('<li class="menu collapsed"><a href="javascript:void(0)"><label>'+label+'</label></a></li>');
			}
	   }else if(menuGrpItem.icon){
			return jQuery('<li class="menu collapsed icon"><a href="javascript:void(0)"><span><img HEIGHT="16" WIDTH="16" src='+menuGrpItem.icon+'></span><label>'+label+'</label></a></li>');
	   }else{
			return jQuery('<li class="menu collapsed"><a href="javascript:void(0)"><label>'+label+'</label></a></li>');
	   }
 ...
 * To Caculate the Height of menu and show teh scroll bar if needed.*/
   function adjustMenuHeight(menuElem, isMyDesk, strDir){
	   ...
	   if ((jQuery(menuElem.context.offsetParent).hasClass("expanded"))){
			hasSubMenu= true;
	   }
	   ...
	   // in case of RMB click
	   if(strDir != 'right'){
		   if(menuElem[0].clientHeight > tempHt && !getTopWindow().isMobile){
