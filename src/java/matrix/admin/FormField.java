...
public class FormField{
	...
	public FormField(Context context,Form form,String name)
	{
		try
		{
	...
	throws Exception
	{
        ...
        String command = "print form "+form.getName() + " select field['"+getName() + "'].user dump |";
	...
	public String toMqlString()
	{
		...
		try
		{
			buf.append("field ").append("#").append(getNumber()).append("\n");
			buf.append("\t").append("name\t").append(quote(getName())).append("\n");
			if(!"".equals(getExpression()))
				buf.append("\t").append("businessobject\t").append(quote(getExpression())).append("\n");
			if(!"".equals(getLabel()))
...
//			buf.append("\t").append("hidden\t").append(quote(getHidden())).append("\n");
			//setting
			System.out.println("getSetting()=" + getSetting());
	...
	throws Exception
	{
        MQLCommand mqlCommand = new MQLCommand();
        String system = "";
        String command = "print form " + form.getName() + " select field[" + getName() + "].property dump |";
        boolean executeOk = mqlCommand.executeCommand(context, command);
        String result;
	...
	throws Exception
	{
        MQLCommand mqlCommand = new MQLCommand();
        String system = "";
       String command = "print form " + form.getName() + " select field[" + getName() + "].setting dump |";
        boolean executeOk = mqlCommand.executeCommand(context, command);
        String result;
        ...
        String command2 = "print form " + form.getName() + " select field[" + getName() + "].setting.*";
        ...
        for(int i=0 ; i < propertyList.size(); i++)
        {
        	...
        	if(settingValue == null || "".equals(settingValue))
        		settingValue = (String)settingMap.get("field[" + getName() + "].setting["+settingName+"].value");
	...
	{
        MQLCommand mqlCommand = new MQLCommand();
        String command = "print form " + form.getName() + " select field[" + getName() + "]."+ attr +" dump";
