...
public class UM5WdgConfig extends RestService {
	...
	@GET
	public Response getConfig(@Context HttpServletRequest request, @DefaultValue("") @QueryParam("action") String action, @QueryParam("wdg") String wdg, @QueryParam("configName") String configName, @QueryParam("configContent") String configContent) throws UM5RestServiceException{
		return config(request, action, wdg, configName, configContent);
	}
	@POST
	public Response postConfig(@Context HttpServletRequest request, @DefaultValue("") @FormParam("action") String action, @FormParam("wdg") String wdg, @FormParam("configName") String configName, @FormParam("configContent") String configContent) throws UM5RestServiceException{
		return config(request, action, wdg, configName, configContent);
	}
	
	public Response config(HttpServletRequest request, String action, String wdg, String configName, String configContent) throws UM5RestServiceException{
			...
			output.add("msg", "KO");
				...
				if(action.equals("getConfigsList")){
					...
					for (String keyParam : keySet) {
						//System.out.println("UM5WdgConfig - keyParam="+keyParam);
