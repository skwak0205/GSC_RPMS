...
public class Task implements ServiceConstants {
    ...
    public static String createTask(Context var0, String var1, String var2, Map<String, String> var3, String var4, String var5) throws FoundationException {
        ...
        String var7 = ObjectEditUtil.create(var0, var6, var2, (String)null, var4, (String)null, (String)null, false, false, (Map)null);
        if (var5 != null && !var5.isEmpty()) {
            ObjectEditUtil.createObjectOwnership(var0, var7, var5, "Multiple Ownership For Object");
        }

        Object var8 = var3 != null ? var3 : new HashMap(1);
    ...
    public static Datacollection getUserTasks(Context context, List<Selectable> selectableList) throws FoundationException {
        return getUserTasks(context, selectableList, "all", false);
    }

    public static Datacollection getUserTasks(Context context, List<Selectable> selectableList, String var2, boolean var3) throws FoundationException {
        return getUserTasks(context, selectableList, var2, var3, (String)null,"");
    }

    public static Datacollection getUserTasks(Context context, List<Selectable> selectableList, String currentTaskFilter, boolean isShowProjectTasks, String completionDate, String state) throws FoundationException {
        checkLicenseCollaborativeTasks(context);
        if (selectableList != null && !selectableList.isEmpty()) {
            boolean isOwned = true;
            boolean isAssigned = true;
            if ("owned".equals(currentTaskFilter)) {
                isAssigned = false;
            } else if ("assigned".equals(currentTaskFilter)) {
                isOwned = false;
            }

            String where = "((!{to[Project Access Key]})";
            if (isShowProjectTasks) {
                checkLicenseProjectTasks(context, (HttpServletRequest)null);
                where = where + " || ((!{from[Subtask]})  &&";
                where = where + " !(to[Project Access Key].from.from[Project Access List].to.type matchlist 'Project Template,Project Baseline,Project Snapshot,Experiment', ',' ||";
                where = where + "   to[Project Access Key].from.from[Project Access List].to.current matchlist 'Hold,Cancel', ',')))";
            } else {
                where = where + ")";
            }

            if (completionDate != null && !completionDate.isEmpty()) {
                where = where + " && (attribute[Task Actual Finish Date]=='' || attribute[Task Actual Finish Date]>'" + completionDate + "' )";
            }
            if (state != null && !state.isEmpty()) {
                where = where + " && (current=='" + state + "')";
            }
            String exclusionList = "";

            try {
                exclusionList = EnoviaResourceBundle.getProperty(context, "emxCollaborativeTasks.TaskExclusionList");
            } catch (FrameworkException var14) {
            }

            if (exclusionList != null && !exclusionList.isEmpty()) {
                where = "!(type matchlist '" + exclusionList + "', ',') && " + where;
            }

            String ownerWhere = "(owner == context.user || ownership=='-|" + context.getUser() + "_PRJ|co-owner')";
            Datacollection datacollection = null;
            if (isOwned) {
                String var11 = ownerWhere + " && " + where;
                System.out.println("isOwned where : " + var11);
                QueryData queryData = new QueryData();
                queryData.setTypePattern("Task,Milestone,Gate,Phase");
                queryData.setWhereExpression(var11);
                datacollection = ObjectUtil.query(context, queryData, selectableList);
            }
            System.out.println("where : " + where);

            Datacollection datacollection1 = null;
            if (isAssigned) {
                Dataobject personObject = ContextUtil.getPersonObject(context);
                where = "current != 'Create' && " + where;
                if (isOwned) {
                    where = "(!" + ownerWhere + ") && " + where;
                }

                ExpandData expandData = new ExpandData();
                expandData.setRelationshipPattern("Assigned Tasks");
                expandData.setTypePattern("Task,Milestone,Gate,Phase");
                expandData.setGetFrom(true);
                expandData.setObjectWhere(where);
                System.out.println("isAssigned where : " + where);

                datacollection1 = ObjectUtil.expand(context, personObject, expandData, selectableList);
            }

            Datacollection datacollection2 = new Datacollection();
            if (datacollection != null) {
                datacollection2.getDataobjects().addAll(datacollection.getDataobjects());
            }

            if (datacollection1 != null) {
    ...
    public static boolean getTaskAssignNotificationPreference(Context var0) throws FoundationException {
        String var1 = ContextUtil.getPersonProperty(var0, "preference_AssignedTaskNotificationOn");
        if (var1.isEmpty()) {
            var1 = "TRUE";
            setTaskAssignNotificationPreference(var0, true);
        }
