...
public class gscPatentService implements ServiceConstants {

    public static Datacollection getPatents(Context context, String[] args) throws FoundationException {
...
//        com.dassault_systemes.enovia.dpm.ServiceUtil.checkLicenseDPM(context);
        ...
        if (datacollection != null && !datacollection.getDataobjects().isEmpty()) {
            if(projectId != null && !"".equals(projectId)){
                datacollection1 = gscPatent.getUserPatents(context, datacollection, false, selects, (String) null, projectId);
            }else{
                datacollection1 = gscPatent.getUserPatents(context, datacollection, false, selects, (String) null, (String) null);
            }
        } else {
            datacollection1 = gscPatent.getUserPatents(context, (Datacollection) null, isOwned, selects, state, projectId);
        }

        String var14 = String.format("%s%s (%d) ...\t", ">>> PROGRAM (SVC):  ", "getPatents", datacollection1.getDataobjects().size());
        FoundationUtil.debug(var14, var4);
        return datacollection1;
    }


    static StringList _getPatentState() {
    ...
    public static void getPatentInfo(Context var0, Datacollection var1, List var2) throws FoundationException {
//        com.dassault_systemes.enovia.dpm.ServiceUtil.checkLicenseProject(var0, (HttpServletRequest) null);
    ...
    public static Dataobject updatePatents(Context context, String[] var1) throws Exception {
        ...
        dataobject = updatePatents(context, serviceParameters, dataobject, updateAction);
        return dataobject;
    }

    private static String getPatentCode(Context context, Dataobject dataobject) throws FoundationUserException {
    ...
    public static Dataobject updatePatents(Context context, ServiceParameters serviceParameters, Dataobject dataobject, UpdateActions updateActions) throws Exception {
        if (!UpdateActions.CREATE.equals(updateActions) && !UpdateActions.MODIFY.equals(updateActions)) {
            ...
            String gscMstNo = DataelementMapAdapter.getDataelementValue(dataobject, "gscMstNo");
            String name = gscMstNo;
            ...
            if (UpdateActions.CREATE.equals(updateActions)) {
                ...
                // BUS 생성 부분
                busResRqId = com.gsc.enovia.dpm.gscPatentService.createPatent(context, type, name, attrMap, policy);

                // RelationShip 처리
                com.gsc.util.ServiceUtil.updateConnection(context, updateActions, busResRqId, "gscProjectPatent", projectId, false, false);
                ...
                gscPatentService.modifyPatent(context, busResRqId, attrMap);
...
//            RelateddataMapAdapter.addRelatedData(dataobject, "performRollup", var18);
    ...
    public static String createPatent(Context context, String type, String name, Map attrMap, String policy) throws FoundationException {
        try {
            //checkLicenseProjectLead(context, (HttpServletRequest)null);
            ObjectEditUtil.checkNameRules(context, name);
            String typeName = type != null ? type : "gscPatent";
            String policyName = policy != null && !policy.isEmpty() ? policy : "gscInterfaceDeliverable";
            gscPatent resourceRequest = new gscPatent();
    ...
    public static void modifyPatent(Context context, String objectId, Map map) throws FoundationException {
...
//            ServiceUtil.checkLicenseDPM(context);
    ...
    public static void deletePatent(Context context, String var1) throws FoundationException {
//        ServiceUtil.checkLicenseDPM(context);
