...
public class gscProjectInvest extends DomainObject {
    ...
    protected static Datacollection getAllInvests(Context context, Datacollection datacollection1, boolean isOwner, List selects, String var4) throws FoundationException {
        boolean hasData = false;
        String typeName = PropertyUtil.getSchemaName(context, "type_gscInvestIf");
        if (datacollection1 != null && !datacollection1.getDataobjects().isEmpty()) {
            hasData = true;
        }

        if (hasData && (selects == null || selects.isEmpty())) {
            throw new FoundationException("You must provide some selectables to getAllInvests");
        } else {
            ...
            if (hasData) {
                ...
                String whereClause = String.format("to[gscProjectInvestIf] == 'false'");
