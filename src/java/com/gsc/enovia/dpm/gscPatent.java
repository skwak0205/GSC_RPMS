...
public class gscPatent extends DomainObject {
    ...
    private static final String TYPE_GSCPATENT = "type_gscPatent";
    ...
    public String createPatent(Context context, String var2) throws FoundationException {
        ...
        try {
            ...
            String results = MqlUtil.mqlCommand(context, mqlStr, new String[]{TYPE_GSCPATENT, var2, physicalid, "physicalid", "|"});
            ...
            if (results != null && !results.isEmpty()) {
                ...
                this.createObject(context, TYPE_GSCPATENT, var2, physicalid, getDefaultPolicy(context), context.getVault().getName());
            ...
            strFormat = String.format("%s%s (%s) done ...\t", ">>> PROGRAM (OBJ):  ", "createPatent", var2);
            FoundationUtil.debug(strFormat, var3);
            return physicalId;
    ...
    public static String getDefaultPolicy(Context var0) {
        return com.matrixone.apps.domain.util.PropertyUtil.getSchemaProperty("policy_gscPatent");
    }

    protected static Datacollection getUserPatents(Context context, Datacollection datacollection1, boolean isOwner, List selects, String state, String projectId) throws FoundationException {
        boolean hasData = false;
        String type = PropertyUtil.getSchemaName(context, TYPE_GSCPATENT);
        if (datacollection1 != null && !datacollection1.getDataobjects().isEmpty()) {
            hasData = true;
        }

        if (hasData && (selects == null || selects.isEmpty())) {
            throw new FoundationException("You must provide some selectables to getUserPatents");
        } else {
            ...
            if (hasData) {
                ...
                if (state != null && !state.isEmpty()) {
                    whereClause = ServiceUtil.getStateWhereClause(context, "", state, _getPatentStates());
                } else {
                ...
                if (projectId != null && !"".equals(projectId)) {
                    if(whereClause != ""){
                        whereClause = String.format("%s AND to[gscProjectPatent].from.physicalid==%s", whereClause, projectId);
                    }else{
                        whereClause = String.format("%s to[gscProjectPatent].from.physicalid==%s", whereClause, projectId);
                    }
                }
//                else{
//                    if(whereClause != ""){
//                        whereClause = String.format("%s AND relationship[gscProjectPatent]==FALSE", whereClause);
//                    }else{
//                        whereClause = String.format("%s relationship[gscProjectPatent]==FALSE", whereClause);
//                    }
//                }
    ...
    private static StringList _getPatentStates() {
    ...
    public void create(Context context, String var2, String var3, String var4) throws FrameworkException {
        this.create(context, "gscPatent", var2, var3, var4);
    }
