...
public class gscProfileService implements ServiceConstants {
    public static Datacollection getProfiles(Context context, String[] args) throws FoundationException {
...
//        com.dassault_systemes.enovia.dpm.ServiceUtil.checkLicenseDPM(context);
        ...
        if(getServletPath.equals("/resources/v1/modeler/gscprofiles/myprofile")){
            datacollection1 = gscProfile.getProfiles(context,(Datacollection) null, selects, state, true, businessUnitId, gscYear);
        }else{
            if (datacollection != null && !datacollection.getDataobjects().isEmpty()) {
                datacollection1 = gscProfile.getProfiles(context,datacollection, selects, state, false, businessUnitId, gscYear);
            }else{
                datacollection1 = gscProfile.getProfiles(context,(Datacollection) null, selects, state, false, businessUnitId, gscYear);
            }
        }

        String var14 = String.format("%s%s (%d) ...\t", ">>> PROGRAM (SVC):  ", "getProfiless", datacollection1.getDataobjects().size());
