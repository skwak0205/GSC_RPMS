...
public class StringUtil {
    ...
    {
        String str = Base64.getEncoder().encodeToString((userId+"@SC@"+securityContext+":"+password).getBytes());
        return str;
    }
    public static String getContextUser() {
        String strConnectEnCode = "";
        try {
//            String strConnect = "if_user@SC@VPLMProjectLeader.Company Name.HIMSEN:if_user";
            String strConnect = "admin_platform" + "@SC@" + "VPLMProjectLeader.Company Name.RPMS" + ":" + "Qwer1234";
            strConnectEnCode = "Basic " + matrix.util.Mime64.encode(strConnect.getBytes());

        } catch (Exception ex) {
            ex.printStackTrace();
        }
        return strConnectEnCode;
    }

    public static void main(String[] args) {
        String contxtStr = getContextUser();
        System.out.println(contxtStr);
        contxtStr = Base64.getDecoder().decode(contxtStr.getBytes()).toString();
        System.out.println(contxtStr);
