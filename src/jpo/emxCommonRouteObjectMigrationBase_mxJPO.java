            ...
            for (int i = 0; i < contents.length; i++) {                
                for (int j = 0; j < grantees.length; j++) {
                    if(canRevokeAccessOnContentToGrantee(context,  contents[i], grantees[j])) {                        
                        ...
                        MqlUtil.mqlCommand(context, sCommand, contents[i].getId(context), ROUTE_ACCESS_GRANTOR, grantees[j]);
                        mqlLogRequiredInformationWriter("revokeAccessOnContent: MQL Command : "+ sCommand + "....Executed successfully."); 
                    }
                }
            }
        } finally {
