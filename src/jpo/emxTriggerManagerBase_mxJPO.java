...
//    static final protected Hashtable isJavaMap = new Hashtable();
    ...
    static protected String triggerType, progName, sequenceNo, methodName, adminVault, constructorParam, targetStates, errorType, runInBack;
    ...
    public emxTriggerManagerBase_mxJPO (Context context, String[] args) throws Exception
    {
        ...
        synchronized(_lock) {
            if (sl == null) {
				...
				s[23] = "attribute_eServiceRunInBackground";
				...
				if(adminNames.size()==24){
					runInBack = (String)adminNames.get(23);	
				}
				...
				sl.addElement("attribute[" +  runInBack + "].value");
...
/*End Of Changes */
            ...
            while (it.hasNext()) {
                ...
                if (result.compareTo("TRUE") == 0) {
					...
					String isRunInBack = bows.getSelectData("attribute[" +  runInBack + "].value");
					if ("true".equalsIgnoreCase(isRunInBack)) {					
						Job backgroundJob = new Job(prog, method, arguments);
                    ...
                    sBuffer.append("execute program ");
                    sBuffer.append(prog);
                    sBuffer.append(" ");
                    for (int k = 0; k < 15; k++) {
                        sBuffer.append("'");
