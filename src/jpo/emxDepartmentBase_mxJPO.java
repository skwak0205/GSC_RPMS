...
public class emxDepartmentBase_mxJPO extends emxOrganization_mxJPO {
    ...
    @com.matrixone.apps.framework.ui.ProgramCallable
	public String getFieldQueryForDepartment(Context context, String[] args) throws Exception {
		try {
			HashMap paramMap = (HashMap) JPO.unpackArgs(args);
			String objectId = (String) paramMap.get("objectId");
			String strRelationship = PropertyUtil.getSchemaProperty(context, "relationship_CompanyDepartment");
			strRelationship = "to["+strRelationship+"].from.name";
			String mqlCmd = "print bus $1 select $2 $3 dump $4";
			String mqlOutPut = MqlUtil.mqlCommand(context, mqlCmd, objectId, strRelationship, "name", "|");
			StringList mqlOutPutSplit = FrameworkUtil.split(mqlOutPut, "|");
			return "TYPES=type_Person:CURRENT=policy_Person.state_Active:member=" + (String) mqlOutPutSplit.get(0)
					+ ":member!=" + (String) mqlOutPutSplit.get(1);
		} catch (Exception ex) {
			ex.printStackTrace();
			throw ex;
		}
