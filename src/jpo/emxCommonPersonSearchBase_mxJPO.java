...
public class emxCommonPersonSearchBase_mxJPO extends emxDomainObject_mxJPO
{
	...
	public StringList includeRiskAssignees(Context context, String[] args)throws MatrixException
	{
		try {
			...
			MapList mlAssigneeList = DomainObject.getInfo(context, riskIdArray, busSelects, multiValueSelectables);
			...
			for(int i=0;i<mlAssigneeList.size();i++){
				...
				if(riskAssigneeList != null){
						for(int j=0;j<projectMemebers.size();j++){
							...
							if(!riskAssigneeList.contains(strMemberId) && !slMemberList.contains(strMemberId)){
...
//				slProjectNames.add(mapProject.get(SELECT_NAME));
				slProjectNames.add((String)mapProject.get(SELECT_NAME));
			}
...
//				s1Locations.add(mapLocation.get(SELECT_NAME));
				s1Locations.add((String)mapLocation.get(SELECT_NAME));
			}
...
//				slCompanies.add(mapCompany.get(SELECT_NAME));
				slCompanies.add((String)mapCompany.get(SELECT_NAME));
			}
	...
	throws Exception
	{
		try{
			...
			MapList mlPersons = DomainObject.findObjects(context, TYPE_PERSON, null, null, strList);
