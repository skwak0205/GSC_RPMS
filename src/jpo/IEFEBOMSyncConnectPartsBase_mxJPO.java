...
import java.util.Iterator;					//mkcsc
...
//import matrix.db.AttributeType;				//mkcsc
...
//import matrix.db.QueryIterator;				//mkcsc
...
private Vector updatedCADRelIdVector = null;
	...
	public Hashtable connectPartWithCADObject(Context context, String[] args) throws Exception
	{
		...
		String PART_RELEASE_PHASE = ebomConfObject.getConfigAttributeValue(IEFEBOMConfigObject.ATTR_NEW_PART_RELEASE_PHASE);
		...
		{			
			matchRevisionSequence(context, cadObjId, partObjId, isMinorType,PART_RELEASE_PHASE);
		...
		if(relatedDocsList.size() > 1)
		{
			...
			{			
				...
				Vector revisionIds = mxUtil.getRevisionBoIdsOfAllStreams(context, boId, globalConfigObject.isMajorType(busObj.getTypeName()));
				
				boolean removeObject = false;
				for (int j = revisionIds.size()-1 ; j >= 0; j--)
				{
					String revisionId = (String) revisionIds.elementAt(j);
			
					if(removeObject && relatedDocsList.contains(revisionId))
						relatedDocsList.remove(revisionId);
					else if(relatedDocsList.contains(revisionId))
						removeObject = true;
				}
			}
		}
		...
		//Fix for IR-669081-3DEXPERIENCER2018x :Start 
		Vector HandledObjectIds = new Vector();
		...
		boolean relationExists = false;
			if(partObjIdForPartSpecCreation == null)
			 relationExists = mxUtil.doesRelationExist(context, boId, partObjId, relPartSpecification);
		else
			 relationExists = mxUtil.doesRelationExist(context, boId,partObjIdForPartSpecCreation , relPartSpecification);
	
		HandledObjectIds.add(boId);
			...
			//[NDM]	if(disconnectOldEBOMPartSpecRels == true)
				...
				if (partObjIdForPartSpecCreation == null)
				{
					...
					{
						for (int k = 0; k < HandledObjectIds.size(); k++)
						{
							String objectId = (String) HandledObjectIds.elementAt(k);
							relationExists = mxUtil.doesRelationExist(context, objectId, partObjIdForPartSpecCreation, relPartSpecification);
							if(!relationExists){					
								mxUtil.connectBusObjectsWithSameContext(context, partObjIdForPartSpecCreation,objectId,relPartSpecification,true,attrNameValTable,null);
							}
							
						}
						HandledObjectIds.clear();
				...
				if(!relationExists){
					//IR-454329: create "Part Specification" relation between CAT Part & EBOM Part	
					...
					//rgh1 EBOM changes start for LATEST REV IR - 633852 / 622763
				//	if(matchingPartString.equals("LATEST_REV") /*&& isPartReleased(context,partObjId)*/)
				//	{
				//		connectBusObjectIds(context,relatedDocsList,partObjIdForPartSpecCreation,cadObjId,partObjId,boId,attrNameValTable);
				//	}
					//rgh1 EBOM changes end
					//Fix for IR-669081-3DEXPERIENCER2018x : END
				}
			...
			//	DomainObject currentInst = DomainObject.newInstance(context, cadObjId); [NDM]
				...
				while(alreadySynchedInstObjListItr.next())
				{
					...
					if(typeNameKeyIdTable.containsKey(keyString.toString()))
					{
						...
						//IR-526895 :if part is not released, then disconnect 'Part Specification' relatioship
						if(!latestVersion.equals(oldVersion) && !isPartReleased(context, partObjId))
						{
							//disconnect old P.S.
						...
						//IR-526895 : isPartReleased() condition is handled in outer if condition
	...
	try{
		...
		mxUtil					= new MCADMxUtil(context, serverResourceBundle, cache);
		serverGeneralUtil    						= new MCADServerGeneralUtil(context,globalConfigObject, serverResourceBundle, cache);
		updatedCADRelIdVector 	= new Vector();
		...
		{			
			if(relatedObjectsList.size()==0 && cadSubComponentRelationShipCount !=0)
			{
				...
				if(cadAttrTable.containsKey(ATTR_COMPONENT_LOCATION)) 
				{
					...
					//TODO
					//IR-454329: create "EBOM" relation betwwn two EBOM Parts
				...
				for(int i = 0; i < relatedObjectsList.size(); i++)
				{
					...
					else
					{
						...
						if(cadAttribTable != null && cadAttribTable.size() > 0)
						{
							...
							/* IR-821601:UPR1
							 * Only those users which have modify access for EBOM rel should update the attributes.
							 * Thus removing grant and revoke access API calls  
							 */
		...
		else if(relatedObjectsCount > cadSubComponentRelationShipCount)
		{
			...
			for(int i = 0; i < difference; i++)
			{
				...
				/*	UPR1:IR-408011
				 * 	Remove the disconnected ebom-rel Id from the ebomRelIdList as we need to accurate rel count for rel-attribute re-sync
				 * 	Ok to remove as for a CAD-assembly & CAD-component, This API is called only once. Irrespective of subComponent relationship count between them  
				 * */
				relatedObjectsList.removeElementAt(i);  
			}
			
			
			/*	UPR1:IR-408011
			 * 	After equalizing the SubComponent rel count with EBOM rel count. 
			 * 	We re-sync all Mand-rel attribute mapped from SubComponent and EBOM.
			 * */
			for (int cadRelCount = 0 ; cadRelCount < cadSubComponentRelationShipCount ; cadRelCount++) {
				
				String cadRelId		   = (String)updatedCADRelIdVector.elementAt(cadRelCount);

				String relationshipId  = (String)relatedObjectsList.elementAt(cadRelCount);
		...
		else
		{
			...
			if(difference == 0 && relatedObjectsCount > 0)
			{
				for (int j = 0; j < relatedObjectsList.size(); j++ )
				{
					...
					Relationship ebomRel   = new Relationship(relationshipId);
					String relId		   = (String)updatedCADRelIdVector.elementAt(j);			// UPR1:IR-408011 UseCase when, between particular CAD Assembly and CAD component, we exclude n relationships and create n new relationships simultaneously.   
			...
			else
			{
				if(ebomExpositionMode != null && "single".equalsIgnoreCase(ebomExpositionMode))
				{
					for(int i = 0; i < difference; i++)
					{
						...
						//TODO
							//IR-454329: create "EBOM" relation between two EBOM Parts
							mxUtil.connectBusObjectsWithSameContext(context, parentPartId,childPartId,relEBOM,true,cadAttrTable,null);
				...
				else
				{
			  	...
			  	/* IR-821601:UPR1
				 * Errors encountered within connectPartWithPart should not be grounded.
				 * Grounded Exceptions are considered to be handled and thus overall EBOM Synch is considered OK
				 * Thus re-throwing MCADEception from catch block 
				 */	
			  System.out.println("ConnectPartwithPart---Error--->");
			  ex.printStackTrace();
			  MCADServerException.createException(ex.getMessage(),null);
		  }
		return retval;
	}
			...
			if (relations.length() > 0)
			{
				...
				if (retVal.startsWith("true"))
				{
					...
					while (strTok.hasMoreTokens())
					{
						...
						//IR-622583 : if IEF-ExcludeFromBOM is set to true on Drawing then it should be exluded.
						String excludeFromBOM 	= mxUtil.getAttributeForBO(context, connectedObjId, MCADMxUtil.getActualNameForAEFData(context, "attribute_IEF-ExcludeFromBOM"));
						
						if(!retList.contains(connectedObjId) && !"TRUE".equalsIgnoreCase(excludeFromBOM))
	...
	protected float getcadSubComponentRelationShipCount(Context context, Vector relIdsVector) throws Exception
	{
		...
		for(int i = 0 ; i < relIdsVector.size(); i++)
		{
			...
			if(excludeFromBOM == null || (excludeFromBOM != null && !"true".equalsIgnoreCase(excludeFromBOM)))
			{
					...
					updatedCADRelIdVector.addElement(relId);		//UPR1:IR-408011 include only those CAD-SubComponent Rel who are inclusive in BOM and whose attributes should be mapped
				}
...
//	Returns the revision of Part Object if it is released.
	...
	protected void disconnectEBOMRelation(Context context, BusinessObjectList streamBOList, String partObjId, String sourceName, String currentModelID) throws Exception
	{
		...
		while(boItr.next())
		{
			...
			while(partObjItr.next())
			{
				...
				// IR-523120-3DEXPERIENCER2018x: Skip disconnecting relationships which are not connected to new PART object.
	...
	protected boolean disconnectPartSpecRelations(Context context, BusinessObject streamBO, BusinessObject partObj) throws Exception
	{
		...
		{
			short level = 1;
			//Visuals vis = null; //context.getDefaultVisuals();//FUN080585: Removal of deprecated function getDefaultVisuals
			...
			//call the expand method on the business object
			Expansion expansion = partObj.expand(context,level,"",relationshipWhereClause.toString(),null);//FUN080585: Last parameter is sent as null, owing to deprecation of Visuals
	...
	protected boolean disconnectEBOMRelations(Context context, BusinessObject partBO, String sourceName) throws Exception
	{
		...
		{
			short level = 1;
			//FUN080585: Deprecated function getDefaultVisuals removed
			//Visuals vis = null; //context.getDefaultVisuals();
			...
			//call the expand method on the business object
			Expansion expansion = partBO.expand(context,level,"",relationshipWhereClause.toString(),null);//FUN080585: Last parameter is sent as null, owing to deprecation of Visuals
	...
	protected void matchRevisionSequence(Context context, String busObjectID, String matchingPartID, boolean isMinorType, String PART_RELEASE_PHASE) throws MCADException
	{
		try
		{
			...
			{
				//IR-589352-3DEXPERIENCER2017x : If PART_RELEASE_PHASE is 'Development', then get Revision sequence from 'Development Part' policy
				if(PART_RELEASE_PHASE.equalsIgnoreCase("Development")){
					matchingObjSeq = MCADMxUtil.getRevisionSequenceForDevelopmentPhase(context);
				}
				else
				{
	...
	/**
	 * Method to Connect Relationship for EBOM
	 * @param context
	 * @param relatedDocsList
	 * @param partObjIdForPartSpecCreation
	 * @param cadObjId
	 * @param partObjId
	 * @param boId
	 * @param attrNameValTable
	 * @throws Exception
	 */
	private void connectBusObjectIds(Context context,Vector relatedDocsList,String partObjIdForPartSpecCreation,String cadObjId,String partObjId,String boId,Hashtable attrNameValTable) throws Exception
	{
		Iterator itrDocList = relatedDocsList.iterator();  //iterate on rest of list 
		
		while(itrDocList.hasNext())     
		{
			String busId = (String) itrDocList.next();
			
			if(!cadObjId.equals(busId))
			{
				String cadType	= mxUtil.getAttributeForBO(context, busId, attrCADType);
				
				if(("presentation".equalsIgnoreCase(cadType) || "drawing".equalsIgnoreCase(cadType)))   // if cad type is drawing or presentation
				{
					List relIds = getExactRelIdsBetweenParentAndChild(context, cadObjId, busId, relAssociateDrawing, "to");  // to check if Associated Drawing Relationship exists between the selected Drawing and CAD MODEL
					
					if(!relIds.isEmpty())
					{
						Vector majorObjectRevisionIds = mxUtil.getMajorObjectRevisionsIds(context, busId, false);    //to get associated Revision ids
						
						if(majorObjectRevisionIds != null && !majorObjectRevisionIds.isEmpty() && majorObjectRevisionIds.size()>1)    //to check if more than one revision is present
						{
							Iterator itrMajorObjectList = majorObjectRevisionIds.iterator();  //iterate on major Object ids
						
							while(itrMajorObjectList.hasNext())     
							{
								String revisionId = (String) itrMajorObjectList.next();
								
								List revisionRelIds = getExactRelIdsBetweenParentAndChild(context, partObjId, revisionId, relPartSpecification, "to");
								
								if(!revisionRelIds.isEmpty())     // if EC part is released and there is Part specification relationship
								{
									List partSpecificationRelIds = getExactRelIdsBetweenParentAndChild(context, partObjIdForPartSpecCreation, revisionId, relPartSpecification, "to");
									
									if(partSpecificationRelIds.isEmpty())
										mxUtil.connectBusObjectsWithSameContext(context, partObjIdForPartSpecCreation,revisionId,relPartSpecification,true,attrNameValTable,null);
									
									break;
								}
							}
						}
						else							// if one revision is present ,  EC part is released and there is no Part specification relationship
						{
							List partSpecRelIdsForSingleObject = getExactRelIdsBetweenParentAndChild(context, partObjIdForPartSpecCreation, busId, relPartSpecification, "to"); 
							
							if(partSpecRelIdsForSingleObject.isEmpty())
								mxUtil.connectBusObjectsWithSameContext(context, partObjIdForPartSpecCreation,busId,relPartSpecification,true,attrNameValTable,null);
						}
					}
			...
			{
				List cadObjectRelIds = getExactRelIdsBetweenParentAndChild(context, partObjIdForPartSpecCreation, cadObjId, relPartSpecification, "to"); 
				
				if(cadObjectRelIds.isEmpty())
					mxUtil.connectBusObjectsWithSameContext(context, partObjIdForPartSpecCreation,cadObjId,relPartSpecification,true,attrNameValTable,null);
	 ...
	 * @throws Exception
	 */
	public List getExactRelIdsBetweenParentAndChild(Context context, String parentId, String childId, String relName, String end) throws Exception
	{
		List relIds = new ArrayList(10);
		
		String [] objectIds = {parentId};
		Hashtable relsAndEnds = new Hashtable(1);
		relsAndEnds.put(relName, end);
		
		IEFSimpleObjectExpander expander = new IEFSimpleObjectExpander(objectIds, relsAndEnds, null, null, (short)1);
		expander.expandInputObjects(context); 
		HashMap relIdChildBusidsTable = expander.getRelidChildBusIdList(parentId);
		if(relIdChildBusidsTable != null)
		{
			Iterator relIdItr = relIdChildBusidsTable.keySet().iterator();
			while (relIdItr.hasNext())
			{
				String relId = (String) relIdItr.next();
				String childID = (String)relIdChildBusidsTable.get(relId);
