...
//                      Added if condition to check whether the person is Application user of local DB
        ...
        }catch(Exception e )
        {
            ...
            if(e.toString().indexOf("License error") != -1)
            {
                ...
                throw new MatrixException(EnoviaResourceBundle.getProperty(context,"emxPersonOrgModelStringResource",strLocale, "emxComponents.Common.LicenseError").toString());
            }
            else
                throw new Exception(e.toString());
        }
            ...
            while (i.hasNext())
            {
                ...
                if(!level.equalsIgnoreCase("3"))
                {
                    ...
                    else
                    {
                        ...
                        sbEditIcon.append("<img src=\"../common/images/iconSmallPerson.png\" border=\"0\"></img></a>");
                        sbEditIcon.append("<a href=\"JavaScript:emxTableColumnLinkClick('../common/emxTree.jsp?");
                        sbEditIcon.append("objectId=" + strId+"','800', '600', 'false', 'popup', '')\">");
                        ...
                        {
                            sbEditIcon.append("<a><img src=\"../common/images/iconSmallPerson.png\" border=\"0\"></img>");
    ...
    public  Vector getNames(Context context, String[] args) throws Exception {
        try {
            ...
            for (Iterator itrObjects = objectList.iterator(); itrObjects.hasNext();) {
				...
				if (UIUtil.isNotNullAndNotEmpty(strLicenseName) && strLicenseName.indexOf("&amp;") != -1){
                            strLicenseName = FrameworkUtil.findAndReplace(strLicenseName, "&amp;", "&");
                }
    ...
    public  Vector getLicensesAssigned(Context context, String[] args) throws Exception {
        try {
            ...
            final String RESOURCE_BUNDLE = "emxPersonOrgModelStringResource";
    ...
    public  void updateLicense(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            boolean isCRRepOrAdmin =  PersonUtil.isLoggedInUserCRorAdmin(context);
            	...
            	if(casualHours == 0){
					...
					if("RWA".equalsIgnoreCase(strLicenseTrigram) && !isProductAssingned(context,"CSV",strPersonName)){
						if(isCRRepOrAdmin ) {
							...
							strResult = MqlUtil.mqlCommand(context, strUserCommand, false, "CSV", "person", strPersonName);
					}
					}
					if(isCRRepOrAdmin ) {
            			strResult = MqlUtil.mqlCommand(context, strUserCommand, true, strLicenseTrigram, "person", strPersonName);
					}else{
            			strResult = MqlUtil.mqlCommand(context, strUserCommand, false, strLicenseTrigram, "person", strPersonName);
            		}
            	}else{
					...
					if("RWA".equalsIgnoreCase(strLicenseTrigram) && !isProductAssingned(context,"CSV",strPersonName)){
						if(isCRRepOrAdmin ) {
							...
							strResult = MqlUtil.mqlCommand(context, strUserCommand, false, "CSV", "casualhour", casualHours.toString(), "person", strPersonName);
						}
					}
            		if(isCRRepOrAdmin ) {
            			...
            			strResult = MqlUtil.mqlCommand(context, strUserCommand, false, strLicenseTrigram, "casualhour", casualHours.toString(), "person", strPersonName);
            		}
            	}
            } else {
            	if(casualHours == 0){
					...
					for(int k=0; k<consumedList.size(); k++)
					{
						...
						strlConsumedPersonList.add((String)hm.get(matrix.util.LicenseUtil.USAGE_USER_NAME));
					}            	   		
                	...
                	{                
						...
						if("RWA".equalsIgnoreCase(strLicenseTrigram) && isProductAssingned(context,"CSV",strPersonName)){
							if(isCRRepOrAdmin ) {
								...
								strResult = MqlUtil.mqlCommand(context, strUserCommand, false, "CSV", strPersonName);	
							}
						}
                     	if(isCRRepOrAdmin) {
                   			 strResult = MqlUtil.mqlCommand(context, strUserCommand, true, strLicenseTrigram, strPersonName);
                    	} else {
                    		strResult = MqlUtil.mqlCommand(context, strUserCommand, false, strLicenseTrigram, strPersonName);
                    	}
                	}
				} else {
            		String strPersonCommand = "print product $1 select $2 dump $3";
 	                String strPersonResult = MqlUtil.mqlCommand(context, strPersonCommand, true, strLicenseTrigram, "casualhour[40].person", "|");
            ...
            {   
				String msg = "";
                msg = MessageUtil.getMessage(context, null, "emxComponents.Common.LicenseError.NotReleasable", new String[] {strLicenseTrigram, strPersonName},null, MessageUtil.getLocale(context), "emxPersonOrgModelStringResource" );         
                throw new Exception(msg.toString());
            } else if(strErrMsg.contains("1500039")) {
           	 	strErrMsg = MessageUtil.getMessage(context, null, "emxFramework.Common.LicenseError.NotAuthorized", new String[] {contextUser},null, MessageUtil.getLocale(context), "emxPersonOrgModelStringResource" );
    ...
    public StringList isPersonInactive(Context context, String[] args) throws Exception{
        ...
        for (int i = 0; i<objectList.size(); i++) {
            returnStringList.add(new Boolean(hasAccess).toString());
        }       
        return returnStringList;
    }

    public boolean canModifyPersonLicense(Context context, String args[]) throws FrameworkException
    {
    	com.matrixone.apps.common.Person me = com.matrixone.apps.common.Person.getPerson(context);
    	String hostId 						= Company.getHostCompany(context);
    	Company myCompany					= me.getCompany(context);
    	String myCompanyId  				= myCompany.getId();
    	boolean isThisMyCompany 			= myCompanyId.equals(hostId);
    	boolean isCompanyRepresentative 	= me.hasRole(context, ROLE_COMPANY_REPRESENTATIVE);
    	boolean isOrganizationManager   	= me.hasRole(context, ROLE_ORGANIZATION_MANAGER);
    	if(((isCompanyRepresentative || isOrganizationManager) && isThisMyCompany) || PersonUtil.isVPLMAdmin(context, context.getRole())){
    		return true;
    	}else{
    		return false;
    	}
