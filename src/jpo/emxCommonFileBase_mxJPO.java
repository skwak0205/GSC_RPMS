...
public class emxCommonFileBase_mxJPO extends emxDomainObject_mxJPO
{
     ...
     */
     ...
     public String checkin(Context context, String[] args) throws Exception
     {
		...
		System.out.println("L48 Collab & approve CommonFileBase Entry store : " + store);
		...
		System.out.println("L48 Collab & approve CommonFileBase Entry storeFromBL : " + storeFromBL );
    ...
    */
     ...
     */
     public String mcsCheckinCreate(Context context, String[] args) throws Exception
     {
          ...
          String fileName   = (String) uploadParamsMap.get("fileName");
          String store      = (String) uploadParamsMap.get("store");
		System.out.println("L48 Collab & approve CommonFileBasemcsCheckinCreate store : " + store );
String storeFromBL = DocumentUtil.getStoreFromBL(context, "Document"); //L48 : STORE FUN113021 Hardcoding
			  if(!"".equals(storeFromBL) && !"null".equals(storeFromBL))
				{
					store = storeFromBL;
     ...
     */
     public Map multiFileCheckin(Context context, String[] args) throws Exception
     {
          try
          {
			...
			System.out.println("L48 Collab & approve CommonFileBase multiFileCheckin store : " + store);
    ...
    * This method is executed to update the meta date while using FCS.
    ...
    public boolean hasFileCheckOutAccess(Context context,String args[]) throws Exception
    {
             ...
             if (UIUtil.isNotNullAndNotEmpty(parentOID)) {
            	 if(parentOID.equals(objectId)) {
            		 ...
            		 MapList mapList = masterObject.getRelatedObjects(context,
                             ...
                             null);
            		 if( mapList.size()>0) {
            			 documentId =(String)((Map)mapList.get(0)).get(CommonDocument.SELECT_ID);
            		 }
            	 }
             }
             DomainObject fileObject = DomainObject.newInstance(context,objectId);
             StringList selectFileList = new StringList(1);
             selectFileList.add(DomainConstants.SELECT_ID);
             selectFileList.add(CommonDocument.SELECT_IS_VERSION_OBJECT);
             selectFileList.add(CommonDocument.SELECT_LOCKED);
             Map fileObjectInfo = fileObject.getInfo(context,selectFileList);
        ...
        return (CommonDocument.canCheckout(context, docObjectInfo) && (Boolean.valueOf((String)fileObjectInfo.get(CommonDocument.SELECT_IS_VERSION_OBJECT))).booleanValue()
        		&& !(Boolean.valueOf((String)fileObjectInfo.get(CommonDocument.SELECT_LOCKED))).booleanValue());
    }
