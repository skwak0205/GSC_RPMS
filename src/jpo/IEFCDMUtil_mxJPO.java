...
public class IEFCDMUtil_mxJPO
{
    ...
    {
        String xmlOutput = "";
        CommonDocument revisedObject =null;
        String lang = args[0];
        try
        {
        	...
        	String latestRevId = _util.getLatestRevisionID(context, oid);

        	if(!latestRevId.equals(oid))           // oid is current object(user selected) ID  //IR-547710-3DEXPERIENCER2018x
        	{
        		String exReviseErrorMessage = reviseErrorMessage(context, type, name, rev,lang);
				MCADServerException.createException(exReviseErrorMessage, null);				
        	}
        	else
        	revisedObject = iefRevise(context, domainObject, copyFiles);
        	
        	/*FUN104057 : TSK6842458 :: Support of comment in revise with files operation
        	 * MSF will add revisecomment|<doc_title>|<comment_value> for Revise with files option.
        	 * If IEF got argument contains 'revisecomment' , we will map comment value to  files object's 'Checkin Reason' attribute */	
        
        	String revisecommentvalue =null;
        	String docName =null;
        	
			for (int i =0; i <args.length ;i++)
			{
		  		if(args[i].startsWith("revisecomment"))
		  		{
		  			StringTokenizer stringtokenizer = new StringTokenizer(args[i], "|");
		        	String key = stringtokenizer.nextToken();
		        	docName = stringtokenizer.nextToken();
		        	if(stringtokenizer.hasMoreTokens())
		        		revisecommentvalue = stringtokenizer.nextToken();
		        	else 
		        		revisecommentvalue ="";
		  		}
			}

            if(revisecommentvalue!=null && docName!= null &&  !docName.isEmpty())
            {
            	String argsToMQL[] = new String[3];
	            argsToMQL[0] = revisedObject.getObjectId();
	            argsToMQL[1] = "from[Latest Version|(to.attribute[Title] == \"" + docName + "\")].to.id";
	            argsToMQL[2] = "|";
	            
	            
	            String minorObjectID = MCADMxUtil.executeMQL("print bus $1 select $2 dump $3", context, argsToMQL);
	            if(minorObjectID!=null && minorObjectID.startsWith("true|"))
	            {
	            	minorObjectID = minorObjectID.substring(5);
	            	if(minorObjectID != null && !minorObjectID.isEmpty())
	            	{
	            		BusinessObject activeMinorVersion = new BusinessObject(minorObjectID);
	            		activeMinorVersion.setAttributeValue(context,"Checkin Reason", revisecommentvalue);
	            	}
	            }
    ...
    private CommonDocument iefRevise(Context context, DomainObject domainObject, boolean copyFiles) throws Exception
    {
    	CommonDocument revisedObject = ((CommonDocument)domainObject).revise(context, copyFiles);
    ...
    public String getRelationshipToConnectFromGCO(Context context, String[] args) throws Exception
    {
        ...
        try
        {
            ...
            {
            	//IR-513592: IF destination folder is deleted or renamed, error message thrown
            	if(parentId == null || parentId.isEmpty()){
            		MCADServerResourceBundle serverResourceBundle=new MCADServerResourceBundle(context.getSession().getLanguage());
            		String parentFolderDeletedOrRenamed = serverResourceBundle.getString("mcadIntegration.Server.Message.IEF0033210644");
					MCADServerException.createException(parentFolderDeletedOrRenamed, null);
            	}
...
//  Throw away the following code after the CDM bean supports Non-Versioning probably in v10.6
	...
	//IR-547710-3DEXPERIENCER2018x
    private String reviseErrorMessage(Context context, String type, String name, String rev,String lang) throws Exception
    {    
    	//IR-873347 : NLS Type should add error messge. Also error should be in Client language.  
    	String nlsType 	=  MCADMxUtil.getNLSName(context, "Type", type, "", "" , lang);
    	Hashtable messageTokens = new Hashtable();
		messageTokens.put("TYPE", nlsType);
		messageTokens.put("NAME", name);
		messageTokens.put("REVISION", rev);
		MCADServerResourceBundle serverResourceBundle=new MCADServerResourceBundle(lang);
		String errMsg = serverResourceBundle.getString("mcadIntegration.Server.Message.NoReviseAccessForOlderRevision",messageTokens);
		return errMsg;
    }
}
