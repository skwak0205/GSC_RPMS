...
public class emxLibraryCentralGrantAccessMigrationFindObjectsBase_mxJPO extends emxCommonFindObjects_mxJPO
{
	String libraryType = "";
	String classType = "";
	String defaultChunkSize = "100";

	public emxLibraryCentralGrantAccessMigrationFindObjectsBase_mxJPO(Context context, String[] args) throws Exception 
	{
		super(context, args);
    ...
    public int mxMain(Context context, String[] args) throws Exception
    {
        try
        {
            ...
            if(args.length == 1)
            {
                ...
                newArgs[1] = libraryType+","+classType;
                newArgs[2] = args[0];
            }
            else
            {
     ...
     *      - Objects of type 'Libraries' which have grants (grantee : 'User Agent', grantor : 'User Agent')
     *      - Objects of type 'Classification' which have grants (grantee : 'User Agent', grantor : 'User Agent')
    ...
    public void getIds(Context context, int chunkSize) throws Exception
    {
        ...
        try
        {
            ...
            MqlUtil.mqlCommand(context, command, libraryType, name, revision, vaultList, "1", "grantee == 'User Agent' && grantor == 'User Agent' && program[" + migrationProgramName + " -method writeOID ${OBJECTID} \"${TYPE}\"] == true");
            
            MqlUtil.mqlCommand(context, command, classType, name, revision, vaultList, "1", "grantee == 'User Agent' && grantor == 'User Agent' && program[" + migrationProgramName + " -method writeOID ${OBJECTID} \"${TYPE}\"] == true");
        }
        catch(Exception me)
        {
            throw me;
