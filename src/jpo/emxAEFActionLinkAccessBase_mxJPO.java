...
public class emxAEFActionLinkAccessBase_mxJPO
{
    ...
    public boolean displayAEFFreezeExpandlevelFilterMenu(Context context,String[] args) throws Exception {
		...
		if(!("thumbnail".equalsIgnoreCase(displayView)) && "structureBrowser".equalsIgnoreCase(uiType) && !"false".equalsIgnoreCase(expandFilter)) {
    ...
    throws Exception
  {
    ...
    public boolean hasAccessOnCreateSubFolderCommand(Context context, String args[]) throws Exception {
		boolean boolChecking = true;
		
		Boolean tempBoolChecking = (Boolean) JPO.invoke(context, "emxAEFUtil", null, "canDisplayCreateCommand", args, Boolean.class);
		boolChecking = tempBoolChecking.booleanValue();
		if(!boolChecking) {
			return boolChecking;
		}
        ...
        if(DomainConstants.TYPE_WORKSPACE_VAULT.equals(objType)) {
        	boolChecking = !FrameworkUtil.isWsInArchiveState(context, obj);
        }
