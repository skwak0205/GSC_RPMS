	...
	{	
		...
		try{	
			...
			for(int j = 0; j < buslWithSelectionList.size(); j++)
			{
				...
				System.out.println("JPO_DSCShowNavigationLinks:getHtmlString : BEFORE XSSUtil.encodeForHTML | name = " + name); // IR-601553
				name = XSSUtil.encodeForHTML(context, name);
				System.out.println("JPO_DSCShowNavigationLinks:getHtmlString : AFTER XSSUtil.encodeForHTML | name = " + name);
				...
				//IR-561142 Name is not visible on export in html format as name has link, hence remnoved all the links for name in case of export.
