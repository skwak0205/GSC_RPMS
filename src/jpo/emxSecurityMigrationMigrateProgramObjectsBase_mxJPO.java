...
public class emxSecurityMigrationMigrateProgramObjectsBase_mxJPO extends
	...
	public void migrateObjects(Context context, StringList objectList) throws Exception
	{
        ...
        StringList multiValueSelectables = new StringList(10);
        multiValueSelectables.add(prgProjectMemberIds);
		multiValueSelectables.add(projectAccessListIdFromProject);
		multiValueSelectables.add(projectSpaceGranteeFromProjectAccessList);
		multiValueSelectables.add(projectSpaceGrantGranteeAccessFromProjectAccessList);


		multiValueSelectables.add(granteeFromProjectAccessList);
		multiValueSelectables.add(grantGranteeAccessFromProjectAccessList);
				
		multiValueSelectables.add(selectTaskAssignee);
		multiValueSelectables.add(selectBudgetCostItemList);

		multiValueSelectables.add(selectRiskAssignee);
		multiValueSelectables.add(selectRiskItems);
        
		String[] oidsArray = new String[objectList.size()];
		oidsArray = (String[])objectList.toArray(oidsArray);
		MapList mapList = DomainObject.getInfo(context, oidsArray, mxObjectSelects, multiValueSelectables);
   		try{
