...
//
// Copyright (c) 2006-2020 by Dassault Systemes.
// All Rights Reserved
    ...
    throws MatrixException
    {
        ...
        try
        {
                    ...
                    || imageType.equalsIgnoreCase("CGM") || imageType.equalsIgnoreCase("png"))))
            {
                ...
                MapList fullMl  =null;
				if (imageType.equalsIgnoreCase("CATData")) {
					ContextUtil.pushContext(context);
					fullMl = DomainObject.getInfo(context, objectIds, objectSelects);
                        ...
                        for (int imageIdItr = 0; imageIdItr < imageIdList.size(); imageIdItr++ ) {
                            ...
                            for (int fileItr = 0; fileItr < fileNamesList.size(); fileItr++) {
                                ...
                                if (imageId != null && !"".equals(imageId)
									...
									//Markup object Access Check and Inherit Access from parent
									if (fileName.contains(".CATData")) {
										DomainObject markupObj = new DomainObject(imageId);
										boolean accesstoMarkup = markupObj.checkAccess(context, (short)AccessConstants.cRead);
										if(!accesstoMarkup){
										ContextUtil.pushContext(context);
										String argReadability = "'Access to Markup Objects'";
										MqlUtil.mqlCommand(context, "mod bus $1 add access businessobject $2 for $3",
												imageId, objectId, argReadability);
										ContextUtil.popContext(context);
										}
										String attrCheckinReason = markupObj.getAttributeValue(context, DomainConstants.ATTRIBUTE_CHECKIN_REASON);
										if(attrCheckinReason==null || attrCheckinReason.equalsIgnoreCase("")){
											markupObj.setAttributeValue(context, DomainConstants.ATTRIBUTE_CHECKIN_REASON, "LMO::Default_InWork_Data");
										}
									}
