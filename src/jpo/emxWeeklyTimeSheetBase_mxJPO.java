	...
	public MapList getSubmittedTimesheetForApprover(Context context, String[] args) throws Exception {
		...
		if(relatedEffort.size()>0 || weeklytimeSheetSize >0) {
			...
			for(int k=0;k<weeklytimeSheetSize;k++) {
				...
				effortList=weeklyimtsheet.getEfforts(context, strList, null, null, new StringList(SELECT_ID));
	...
	public Vector getTimeSheetOwnerName(Context context, String[] args)
		...
		try {
			...
			for(int i=0;i<size;i++) {
				...
				sbownerLink.append(XSSUtil.encodeForHTML(context,ownerName));
...
//		return mpChildTasks;
	...
	public MapList getAssignedTasks(Context context, String[] args) throws Exception {
		...
		while(assignedTaskListIterator.hasNext()){
			...
			if("True".equalsIgnoreCase(isKindOfPhase)){
				taskList.add(taskInfo);
			}else if("Task".equalsIgnoreCase(parentProjectEffortSubmission) && "True".equalsIgnoreCase(isKindOfTask)){
				taskList.add(taskInfo);
			}
		}
