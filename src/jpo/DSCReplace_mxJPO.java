...
public class DSCReplace_mxJPO
{
									   ...
									   Relationship oldRelD)
	{
		try
		{
			...
			//IR-573550 Updating UUID on Relationship, so that when we checkout, Proper Structure should be checked out.
			String UUID = MCADMxUtil.getActualNameForAEFData(context, "attribute_IEF-UUID");
			String UUIDVal = getRelAttribute(context, relID,UUID,true);
			setRelAttribute(context, oldRelD,UUID,UUIDVal);
	...
	public Hashtable replace(Context context, String[] args) throws Exception
	{
		...
		//IR-520335
		if(null == localeLanguage || localeLanguage.equals(""))
...
//IR-573550 "Replace" command from "Where Used" Page, used to replace Minor only, hence repeating the same for Major too, so that the exact can be reflected on 
// "Navigate" page.
		 boolean hasLockAccessMajor 	= false;

		//Get the replaceWith object that the user searched for.
		String replaceWith = (String) paramMap.get("replaceWith");
		String replaceWithMajor = util.getMajorObjectId(context, replaceWith);
		...
		while (keys.hasMoreElements())
		{
			...
			String childMajor = util.getMajorObjectId(context, child);
			...
			for (int par = 0; par < parent.size(); par++)
			{
			  ...
			  String currentParentMajor = util.getMajorObjectId(context, currentParent);
			  BusinessObject currentParObj = new BusinessObject(currentParent);
			  BusinessObject currentParObjMajor = new BusinessObject(currentParentMajor);

			  currentParObj.open(context);

        	   hasLockAccess 	=	util.hasLockAccess(currentParObj,context);
        	   currentParObj.close(context);
        	   currentParObjMajor.open(context);

        	   hasLockAccessMajor 	=	util.hasLockAccess(currentParObjMajor,context);
        	   currentParObjMajor.close(context);
        	   
        	  if(hasLockAccess && hasLockAccessMajor)
        	  {
					...
					Vector relLstMajor = util.findRelationShip(context,childMajor, currentParentMajor,
							true, "all", MCADMxUtil.getActualNameForAEFData(context, "relationship_CADSubComponent"));					
					if(relLst.size() == 0)
					{
					...
					Vector replaceWithChildrenMajor = util.getImmParentOrImmChild(context,replaceWithMajor, MCADMxUtil.getActualNameForAEFData(context, "relationship_CADSubComponent"), "to");
					...
					Vector parentListMajor = util.getImmParentOrImmChild(context, childMajor,MCADMxUtil.getActualNameForAEFData(context, "relationship_CADSubComponent"), "from");
					...
					for (int i = 0; i < relLst.size(); i++)
					{
						...
						String relIDMajor = (String) relLstMajor.get(i);				
						for (int k = 0; k < replaceWithChildren.size(); k++)
						{
							...
							String replaceChild = (String) replaceWithChildren
									.get(k);
							String replaceChildMajor = (String) replaceWithChildrenMajor
									.get(k);
							//Check if the child matches the list of the parents
							if (parentList.contains(replaceChild) || parentListMajor.contains(replaceChildMajor))
							{
							...
							boolean outcome = util.replaceWithRelationshipEnd(context, relID, true,
																replaceWith);
							boolean outcomeMajor = util.replaceWithRelationshipEnd(context, relIDMajor, true,
									replaceWithMajor);
							...
							lockConnectAndModifyAttribtues(context, child,
										...
										true);
							lockConnectAndModifyAttribtues(context, childMajor,
									currentParentMajor, replaceWithMajor, relIDMajor, false,
									true);
							}
						}
						...
						lockConnectAndModifyAttribtues(context, childMajor, currentParentMajor,
								replaceWithMajor, relIDMajor, false, true);								
						}
