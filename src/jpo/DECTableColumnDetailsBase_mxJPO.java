...
public class DECTableColumnDetailsBase_mxJPO
{
	...
	private String IS_KIND_OF_DOCUMENT						="";
	...
	public DECTableColumnDetailsBase_mxJPO(Context context, String[] args) throws Exception
	{
		...
		IS_KIND_OF_DOCUMENT="type.kindof["+ MCADMxUtil.getActualNameForAEFData(context, "type_Document")+"]";
	}
	...
	public Object getLockerData(Context context, String[] objIds) throws Exception
	{
		...
		StringList busSelectionList = new StringList(10);
		...
		busSelectionList.addElement(SELECT_ISVERSIONOBJ);
		busSelectionList.addElement(IS_KIND_OF_DOCUMENT);
		busSelectionList.addElement(SELECT_ON_ACTIVE_MINOR+ "locker");
		...
		for(int i = 0; i < buslWithSelectionList.size(); i++)
		{
			...
			{					
				...
				String isDocumentType=busObjectWithSelect.getSelectData(IS_KIND_OF_DOCUMENT);
				StringList activeMinorLockerList =(StringList)busObjectWithSelect.getSelectDataList(SELECT_ON_ACTIVE_MINOR+ "locker");
				//support only single active minor
				if(isDocumentType.equalsIgnoreCase("TRUE")){
					if(isVersion){
						showLocker		= busObjectWithSelect.getSelectData("locker");
					}else{
						if(activeMinorLockerList!=null&&activeMinorLockerList.size()==1){
						showLocker		=(String)activeMinorLockerList.get(0);
						}
					}
				}
				else{
	...
	//FUN112588 : New column added in DECSavePageDetails 
	public Object getEnoviaFolderTitlePath(Context context, String[] args) throws Exception
	{		
		init(context, args);
		...
		try
		{
			...
			HashMap busIdFolderPathMap  = mcadFolderUtil.getAssignedFolderTitle(context,objIds);
	...
	private Object getLockUnlockData(Context context, String[] objIds)throws Exception
	{
		...
		for(int i = 0; i < buslWithSelectionList.size(); i++)
		{
			...
			{					
				...
				StringList activeMinorLockerList =(StringList)busObjectWithSelect.getSelectDataList(SELECT_ON_ACTIVE_MINOR+ "locker");
				//support only single active minor
				if(isDocumentType.equalsIgnoreCase("TRUE")&&activeMinorLockerList!=null&&activeMinorLockerList.size()==1){
//					showLocker=(String)activeMinorLockerList.get(0);
					if(isVersion){
						showLocker		= busObjectWithSelect.getSelectData("locker");
					}else{
