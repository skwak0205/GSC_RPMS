...
public class emxMemberListBase_mxJPO extends emxDomainObject_mxJPO
{
  ...
  private static final String ORG_ID_PREFIX = "ORG_ID::";
  ...
  public static StringList getAllMemberLists(Context context,String args[]) throws Exception{
	  MapList str = getAllDistributionLists(context, args);
	  Map memberMap= new HashMap();
	  StringList memberOid=new StringList();
	  for(int i=0;i<str.size();i++){
		  memberMap=(Map) str.get(i);
		  memberOid.add((String) memberMap.get("id"));
	  }
...
public static MapList getOwnedDistributionLists(Context context,String[] args) throws Exception
{
  try
  {
    ...
    MapList ownedDistList=dObj.findObjects(context,TYPE_MEMBERLIST,"*", "*", contextUser,  null, where, true, objSelects);
...
public static Vector getBusinessUnits(Context context, String[] args)throws Exception
{
  Vector businessUnitList = new Vector();
    ...
    try{
  	...
  	StringList strSelectList = new StringList();
  	String strName = "to["+RELATIONSHIP_MEMBER_LIST+"].from.name";
  	String strTitle = "to["+RELATIONSHIP_MEMBER_LIST+"].from.attribute[Title]";
  	String strId = "to["+RELATIONSHIP_MEMBER_LIST+"].from.id";
  	String strType = "to["+RELATIONSHIP_MEMBER_LIST+"].from.type";
  	strSelectList.add(strName);
  	strSelectList.add(strId);
  	strSelectList.add(strType);
  	strSelectList.add(strTitle);
  	...
  	while(iterator.hasNext())
  	  {
  		...
  		String type = (String)map.get(strType);
  		String title = (String)map.get(strTitle);
...
public static String getScope(Context context, String[] args)throws Exception
{
  ...
  try{
	...
	boolean isAdmin = PersonUtil.isEnterpriseScopeVisible(context);
    //similarly create a new string list for holding relationship selectables
    StringList relSelects = new StringList();
    short level = 1;
    DomainObject doObj=new DomainObject(objectId);
    ...
    while(scopeListItr.hasNext())
    {
      ...
      if(fromType.equals(TYPE_BUSINESSUNIT) || fromType.equals(TYPE_COMPANY) || fromType.equals(TYPE_PLANT) || ( fromType.equals(TYPE_PERSON) && isAdmin) || fromType.equals(TYPE_DEPARTMENT) )
      {
	    ...
	    organizationName=(String)companyList.get(DomainConstants.SELECT_NAME);
	    organizationId=(String)companyList.get(DomainConstants.SELECT_ID);
  ...
  try{
    ...
    while(scopeListItr.hasNext())
    {
      ...
      if(scope != null)
      {
        ...
        if(fromType.equals(TYPE_BUSINESSUNIT) || fromType.equals(TYPE_COMPANY) || fromType.equals(TYPE_PLANT) || fromType.equals(TYPE_DEPARTMENT))
        {
          scopesList.add((String)scope.get(DomainConstants.SELECT_ATTRIBUTE_TITLE));
          scopesList.add(ORG_ID_PREFIX+(String)scope.get(DomainConstants.SELECT_ID));
        }
        else if (fromType.equals(TYPE_PERSON))
        {
          scopesList.add(" ");
...
public static Object setScope(Context context, String args[]) throws Exception
{
  ...
  HashMap paramMap = (HashMap)programMap.get("paramMap");
  String objectId = (String)paramMap.get("objectId");
  String[] organizationOIDArr = (String[])requestMap.get("OrganizationOID");
  ...
  if(scopeArray != null )
    scope = scopeArray[0];
  if(organizationOIDArr != null )
    organizationOID = organizationOIDArr[0];
  ...
  try
  {
    ...
    } else if(scope.equalsIgnoreCase("Enterprise") ){
		DomainRelationship.disconnect(context,relId);
      	DomainRelationship.connect(context, organizationOID,RELATIONSHIP_MEMBER_LIST,objectId, false);
    }
  }catch(Exception e)
  {
    e.printStackTrace();
...
public static String getMemberOrganization(Context context, String[] args)throws Exception
{
    ...
    if(mode!=null && mode.equals("edit") && isAdmin)
    {
		...
		for(int i=0; i<orgLst.size(); i++){
			String elem = (String)orgLst.elementAt(i);
			if(elem.startsWith(ORG_ID_PREFIX)){
				organizationOID = elem.replace(ORG_ID_PREFIX,"");
				orgLst.remove(i);
			}
		}
        if(orgLst.size() == 1)
        ...
        retStr += "<input type='hidden' name='OrganizationOID'  value='"+organizationOID+"'>";
    ...
    else
    {
        ...
        for(int i=0; i<strLst.size(); i++){
			String elem = (String)strLst.elementAt(i);
			if(elem.startsWith(ORG_ID_PREFIX)){
				strLst.remove(i);
			}
		}
 ...
 * @throws Throwable
  ...
  public void addMembersToMemberList(Context context, String[] args) throws Throwable{
    ...
    }else if("Group".equals(memberType))
     {
        ...
        MapList groupList = DomainObject.findObjects(context,TYPE_BUSINESS_GROUP, typeGroupId, "-", "*", null, null, false, selectList);
        if(groupList.size()>0)
            isGroupExist = true;
    if(!isGroupExist){
        try {
            ...
            //this will fail in only on cloud as user dont have employee role and access to see other Collaborative space access 
			if(msg.contains("Business object 'type name revision' not unique")){
				emxContextUtil_mxJPO.mqlNotice(context,EnoviaResourceBundle.getProperty(context, "emxComponentsStringResource",context.getLocale(), "emxComponents.MemberList.GroupConnectionFailureMsg"));
			}
        }
    }
  ...
  public static HashMap getScopeForCreation(Context context, String[] args)throws Exception
  {
      ...
      boolean isAdmin = ( personAssignments.contains(PropertyUtil.getSchemaProperty(context,"role_SpecificationOfficeManager") ) ||
               ...
               || PersonUtil.hasAssignment(context, ROLE_SecuredVPLMProjectLeader));
      ...
      for(int k=0; k < objList.size(); k++) {
          ...
          if(PropertyUtil.getSchemaProperty(context, "type_BusinessGroup").equalsIgnoreCase(objType)){
              ...
              if(!isprinterFriendly) {
                  strBuffName.append("<a href=\"javascript:showModalDialog('../common/emxTree.jsp?emxSuiteDirectory=components&amp;objectId=" + strMemberId + "', '800', '575')\"><img src=\"../common/images/iconSmallPerson.png\" alt=\"\" name=\"person\" id=\"PersonId\" border=\"0\" /></a>&#160;");
            	  ...
            	  if(exportFormat!=null && "csv".equalsIgnoreCase(exportFormat)){
                	  ...
                	  strBuffName.append("<img src=\"../common/images/iconSmallPerson.png\" alt=\"\" name=\"person\" id=\"PersonId\" border=\"0\"/>");
