...
public class emxUtilBase_mxJPO
{
	...
	private static final String FORMAT_GENERIC = "generic";
	private static final String FROM_PRIMARY_IMAGE_TO_ID = "from[Primary Image].to.id";
	private static final String TO_IMAGE_HOLDER_FROM_ATTRIBUTE_PRIMARY_IMAGE = "to[Image Holder].from.attribute[Primary Image]";
	private static final String COMMON_IMAGES_ICON48X48_IMAGE_NOT_FOUND_GIF = "../common/images/icon48x48ImageNotFound.gif";
	...
	private static final String COMPONENTS_IMAGES_DEFAULT_THUMBNAIL_IMAGE = "../components/images/emxDefaultThumbnail.jpg";
   ...
   */
   ...
   public static String getMCSURL(Context context, String[] args) throws Exception {
	...
	private static boolean isFormatGeneratedForGivenFile(String givenFileName, StringList formatFilesList) {
		boolean isFormatGeneratedForgivenFile = false;

		if(formatFilesList != null && !formatFilesList.isEmpty()) {
			for(int i=0; i< formatFilesList.size(); i++) {
				if(((String)formatFilesList.get(i)).contains(givenFileName)) {
					isFormatGeneratedForgivenFile = true;
					break;
				}
			}
		}

		return isFormatGeneratedForgivenFile;
	}
   ...
   public static String getPrimaryImageURL(Context context, String[] args, String sOID, String sFormat, String sMCSURL, String sDefaultImage) throws Exception {
	   ...
	   if(null == sDefaultImage || "".equals(sDefaultImage)) { sDefaultImage = COMMON_IMAGES_ICON48X48_IMAGE_NOT_FOUND_GIF; }
	   ...
	   busSelects.add(TO_IMAGE_HOLDER_FROM_ATTRIBUTE_PRIMARY_IMAGE);
	   busSelects.add(FROM_PRIMARY_IMAGE_TO_ID);
       busSelects.add("to[Image Holder].from.format["+sFormat+"].file");
       StringList mulValSelects = new StringList();
       mulValSelects.add("to[Image Holder].from.format["+sFormat+"].file");
	   Map mData               = dObject.getInfo(context, busSelects, mulValSelects);
	   ...
	   String sFileName        = (String)mData.get(TO_IMAGE_HOLDER_FROM_ATTRIBUTE_PRIMARY_IMAGE);
       StringList sFormatFileName        = (StringList)mData.get("to[Image Holder].from.format["+sFormat+"].file");
	   // skipFor3dxml is used to check if thumbnail image is not present, when original image is a 3dxml file
       boolean skipFor3dxml = false;
       if(UIUtil.isNotNullAndNotEmpty(sFileName) && sFileName.substring(sFileName.lastIndexOf(".")).contains(".3dxml")) {
    	   skipFor3dxml = !isFormatGeneratedForGivenFile(sFileName, sFormatFileName);
       }
       boolean fromPrimaryImage = true; 
		try{
       if(skipFor3dxml) {
		   sResult = COMPONENTS_IMAGES_DEFAULT_THUMBNAIL_IMAGE;
	   } else {
	   if(null == sOIDImageHolder) {            
		   sOIDImageHolder = (String)mData.get(FROM_PRIMARY_IMAGE_TO_ID);
		   if(null != sOIDImageHolder) {    
	   ...
	   if(!sFormat.equals("generic") && sFormatFileName != null && fromPrimaryImage) { 
           ...
           sResult = tmpImageUrls[0];
			...
			sResult = COMMON_IMAGES_BROKENIMAGE_PNG;
	   }
	   return sResult;
