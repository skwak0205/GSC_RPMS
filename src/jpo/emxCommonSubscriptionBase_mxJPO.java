...
//					String title=doObject.getInfo(context, DomainConstants.SELECT_ATTRIBUTE_TITLE); 
					...
					Map contentInfo =doObject.getInfo(context, objectSelect);
					strObjectName=(String) contentInfo.get(DomainConstants.SELECT_NAME);
					strObjectType=(String) contentInfo.get(DomainConstants.SELECT_TYPE);
					String title=(String) contentInfo.get(DomainConstants.SELECT_ATTRIBUTE_TITLE);


					StringBuilder sbLink =new StringBuilder();
					if(strObjectType.equals(DomainConstants.TYPE_GROUP) || strObjectType.equals(PropertyUtil.getSchemaProperty(context, "type_GroupProxy")))
					{
						strObjectName=title;
						strObjectType=DomainConstants.TYPE_GROUP;
					} 
					...
					else if(strObjectType.equals(DomainConstants.TYPE_GROUP)){
						String strType = (strObjectType.equals(DomainConstants.TYPE_GROUP))?DomainConstants.TYPE_GROUP:PropertyUtil.getSchemaProperty(context, "type_GroupProxy"); // CHANGE
						String strTypeSymName = FrameworkUtil.getAliasForAdmin(context, "type", strType, true);
						String strTypeIcon = FrameworkProperties.getProperty(context,"emxFramework.smallIcon." + strTypeSymName);
						sbLink.append("<img src=\"images/"+strTypeIcon+"\" alt=\"\"></img>");
					}
		...
		try 
		{
			...
			for (Object object : objectList) 
			{
				...
				if(strRel!= null && strRel.equals(PropertyUtil.getSchemaProperty(context,DomainSymbolicConstants.SYMBOLIC_relationship_PushedSubscription)))
				{
					...
					sbLink.append("<img src=\"images/iconActionPushSubscribe.gif\" title=\"").append(XSSUtil.encodeForXML(context,strTitle)).append("\" />");
				}
				else if(strRel!= null && strRel.equals(PropertyUtil.getSchemaProperty(context,DomainSymbolicConstants.SYMBOLIC_relationship_SubscribedPerson)))
				{
					...
					sbLink.append("<img src=\"images/iconSmallSubscription.gif\" title=\"").append(XSSUtil.encodeForXML(context,strTitle)).append("\"/>");
				}
				else if(null!=strType && !"".equals(strType) && !"null".equals(strType) && "Event".equals(strType))
				{
					if(isLoginPersonSubscribed && hasOtherSubscribers)
					{
						...
						sbLink.append("<img src=\"images/iconStatusUserSubscribedToEvent.gif\" title=\"").append(XSSUtil.encodeForXML(context,strTitle)).append("\" />");
					}
					else if(!isLoginPersonSubscribed && hasOtherSubscribers && bEnablePush)
					{
						...
						sbLink.append("<img src=\"images/iconStatusUserNotSubscribedToEvent.gif\" title=\"").append(XSSUtil.encodeForXML(context,strTitle)).append("\"/>");
					}
					else
					{
