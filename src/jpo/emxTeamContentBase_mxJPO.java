...
public class emxTeamContentBase_mxJPO extends emxDomainObject_mxJPO
{
 ...
 /**
  ...
  public static boolean hasWorkspaceAndFileUpload(Context context, String args[]) throws Exception
  {
    ...
    if(FrameworkUtil.isWsInArchiveState(context, folder)) {
		return boolChecking;
	}
    Access access = folder.getAccessMask(context);
    if(checkActionBarLinkAccess(context,args)) {
        boolChecking = access.hasReadAccess() && access.hasToConnectAccess();
    }
   return boolChecking;
  }

  /**
   * checkCreateCommandUnderFolder - to check whether to display the create document under folder.
   *
   * @param context the eMatrix <code>Context</code> object
   * @param args holds the objectId
   * @return boolean type
   ...
   * @since Team 10-0-1-0
   */
  public static boolean checkCreateCommandUnderFolder(Context context, String args[]) throws Exception
  {
    String loggedInRole = PersonUtil.getDefaultSecurityContext(context);
    if (PersonUtil.isVPLMAdmin(context, loggedInRole)|| PersonUtil.isVPLMReader(context, loggedInRole) || PersonUtil.isVPLMOwner(context, loggedInRole) || PersonUtil.isVPLMContributor(context, loggedInRole))  {
           return false;         
    }
    return hasWorkspaceAndFileUpload(context,args);
  }
  //for IR-506520-3DEXPERIENCER2016x | IR-504117-3DEXPERIENCER2016x | IR-505528-3DEXPERIENCER2016x
  public static boolean isWsNotInArchiveState(Context context, String args[]) throws Exception {
		HashMap programMap = (HashMap) JPO.unpackArgs(args);
		String folderObjectId = (String) programMap.get("objectId");
		DomainObject folder = new DomainObject(folderObjectId);
			    
	    return !FrameworkUtil.isWsInArchiveState(context, folder);
	}
  ...
  public static boolean hasMultipleFiles(Context context, String args[]) throws Exception
  {
	...
	if(FrameworkUtil.isWsInArchiveState(context, BaseObject)) {
		return boolChecking;
	}
  ...
  public static boolean hasRemoveContent(Context context, String args[]) throws Exception
  {
      ...
      if(sOwner.equals(strPerson) || (!routeState.equalsIgnoreCase(stateComplete))) {
    	if (access.hasReadAccess() && access.hasFromDisconnectAccess()){
          boolChecking = true;
        }
      }
     return boolChecking;
...
public static String getWorkspaceId(Context context, String  folderId)
{
      ...
      try{
        ...
        String strProjectType  = DomainConstants.TYPE_PROJECT_MANAGEMENT;
