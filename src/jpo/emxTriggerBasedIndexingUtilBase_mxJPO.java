...
public class emxTriggerBasedIndexingUtilBase_mxJPO
{
    ...
    public static int partialIndex(Context context, String[] args) throws Exception
    {
        try
        {
            ...
            bgp.submitJob(context, jponame, methodName,args, (String)null);
            return 0;
        } catch(Exception ex) {
            ex.printStackTrace();
            throw ex;
    ...
    static DataModelConfig config = null;
    public int relatedIndex(Context context, String[] args) throws Exception
    {
        try
        {
            ...
            if( config == null)
            {
              config = (DataModelConfig) Config.getInstance(context, ConfigType.DataModelConfig);
            }
            ...
            for (int i=0; i<fieldNames.size(); i++)
            {
                ...
                Field field = ((DataModelConfig) config).indexedBOField(fieldName);
                if( field != null )
                {
                	String valueSelectable = field.getSelectable();
                    String newValue = "";
