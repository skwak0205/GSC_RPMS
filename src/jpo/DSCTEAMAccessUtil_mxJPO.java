...
public class  DSCTEAMAccessUtil_mxJPO
{
	...
	{
		if(isCloudEnvironment() || isPODEnvironment())  // *FUN101015:Removal of CSE roles from cloud
			  return true;
	...
	public Boolean checkAccessForVPLMAdmin(Context context, String[] args) throws Exception
	{
		...
		else if(isCloudEnvironment() || isPODEnvironment())   // *FUN101015:Removal of CSE roles from cloud
			  return true;
...
public Boolean checkAccessForVPLMAdminAcrossAllProjects(Context context, String[] args) throws Exception
	...
	{
		//IR-772344 : If user role is not child of 'Integration User',initally is was sending false. So BookmarkWorkspace menu was invisible. 
				//Below changes not required in latest release so commented.
		return true;
		/*if(isCloudEnvironment() || isPODEnvironment())   // *FUN101015:Removal of CSE roles from cloud
			  return true;
	...
	{
		if(isCloudEnvironment() || isPODEnvironment())  // *FUN101015:Removal of CSE roles from cloud // IR-804826-3DEXPERIENCER2021x
			  return true;
