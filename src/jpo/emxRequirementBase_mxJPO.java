...
05/10/2017      HAT1        ZUD     2018x   TSK3278161 - R419: Deprecation of functionalities to clean up.
08/06/2017      KIE1        ZUD     2018x   IR-510602-3DEXPERIENCER2017x: Requirement Specifications - ALL Option does not work and gives the error 
05/10/2017      KIE1        ZUD  	2018x	IR-537609-3DEXPERIENCER2018x: List of requirement specs disappears or gives java error when sorting on column 
05-Jul-21 		VAI1 			  	2021x	IR-859720-Receiving requirement email notifications even when it is turned off.
13-Oct-21 	XEB   KIE1      			  	22x FD01 FUN113274 Mass Detach 
*/
...
// End:R207:RMT Bug 331464
        ...
        throws Exception
    {
        ...
        String strWhereExpression = "type != 'Requirement Proxy'"; 
        ...
        try{
            ...
            selects.addElement(ReqConstants.SELECT_READ_ACCESS);
        ...
        for(int i = list.size() - 1; i >= 0; i--){
            ...
            Map m1 = (Map)Reqs.get(i);
            String access = (String)m1.get(ReqConstants.SELECT_READ_ACCESS);
        ...
        throws Exception
    {
		...
		MapList mapBusIds  = new MapList();
  		...
  		if(strOwnerCondition == null) {
  			mapBusIds = RequirementsCommon.cvFetchData(context, searchLimit, strType);
  		}else{
			mapBusIds = findObjects(context, strType, null,null,strOwnerCondition,null,strWhereCondition,null,true, objectSelects, (short)searchLimit);
		}
        return(mapBusIds);
    }
...
//End IR-187552V6R2014 LX6            
			...
			sbWhereBus.append("policy != 'ProxyItem' and "); //for Releasing structure escaped DOORS Requirement
...
//End IR-187552V6R2014 LX6				
    ...
    public int revisedParentRequirement(Context context, String[] args)throws Exception
    {
        ...
        try
        {
			...
			// Parent object Link Status update IR-700750-3DEXPERIENCER2020x
            if("true".equalsIgnoreCase(EnoviaResourceBundle.getProperty(context, "emxRequirement.updateParent.LinkStatus")))
            {
	            DomainObject oldRevisedObject  = DomainObject.newInstance(context, strObjectId);
	            MapList connReqObjOldList = new MapList();
	            connReqObjOldList = oldRevisedObject.getRelatedObjects(context, relNamesBuffer.toString(), ReqSchemaUtil.getRequirementType(context),
	                    null, selectRelStmts, true, false, (short)1, null, null);
	            
	            connReqObjList.addAll(connReqObjOldList);
            }  
...
/** Utility method for revision triggers
    ...
    public HashMap getRequirementStructureRMBMenu(Context context,String[] args )throws Exception
	{
    	...
    	{
    		menuName = "RMTSpecStructureProxyRequirementRMB";
    	}
    	//-- OSLC RMB --
		
		else if(RequirementsUtil.isRequirement(context, objectId)){
...
//        For this purpose filtering out the unwanted Decision objects.
    ...
    throws Exception
    {
    	 ...
    	 //The findobjects method is invoked to get the list of products
          MapList mapBusIds = findObjects(context, strType, null,null,null,null,null,true, objectSelects);
    ...
    public List getCommittedRequirementProduct(Context context, String[] args)throws Exception
    {
	   ...
	   /**
    *  Method is used to check the selected object is used in multiple structures
    * @param context the eMatrix <code>Context</code> object
    * @param args JPO argument
    * @return integer value 0 for success
    * @throws Exception if operation fails
    */
   public static int checkForMultipleRelations(Context context, String[] args)
      throws Exception
   {
		...
		try
        {
			if (! "".equals(objId))
			{
				...
				String strResult  = MqlUtil.mqlCommand(context, "print bus $1 select $2 dump", objId, rel);
				StringList derList = new StringList();
				if (!"".equals(strResult) && !"null".equals(strResult) && strResult!=null)
				{
					derList =  FrameworkUtil.split(strResult, ",");
				...
				boolean bRestrict =  false;
				for(int i = 0; i < derList.size(); i++){
					 
					 String StrRelName = derList.get(i);
					 
					 //|| StrRelName.equals("Requirement Group")
					 if(StrRelName.equals("Specification Structure") || StrRelName.equals("Sub Requirement") || StrRelName.equals("Requirement Group Content")  || StrRelName.equals("Sub Requirement Group" ))
					 {
						 bRestrict = true;
						 break;
					 }
				...
				if (bRestrict)
				{
					...
					String strContentLabel = EnoviaResourceBundle.getProperty(context, "emxRequirementsStringResource", context.getLocale(), "emxRequirements.Alert.objectUsedInAnotherStruct");

					 System.err.println("Selected Object is used in multiple structures: " + strContentLabel);
					 throw(new FrameworkException(strContentLabel));
				
					//MqlUtil.mqlCommand(context, "error $1",strContentLabel);
					//return 1;
				}
			}
		}
