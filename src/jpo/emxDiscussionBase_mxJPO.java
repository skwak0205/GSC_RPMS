  ...
  public void grantAccess(matrix.db.Context context, String[] args) throws Exception
  {
    ...
    public  Object getDiscussionList(Context context, String[] args) throws Exception
    {
        ...
        if(FrameworkUtil.isObjectId(context,objectId))
        {
            ...
            // added for the Bug 339013 
            if(hasAccessToPrivateDiscussion(context)) {
             objectWhere = "((policy =="+"\""+DomainConstants.POLICY_MESSAGE+"\")||(policy =="+"\""+strPolicy+"\"))";
            }else {
            	objectWhere = "(policy =="+"\""+DomainConstants.POLICY_MESSAGE+"\")";
            }
...
public Object getSubscribedDiscussionList(Context context, String[] args) throws Exception
{
	...
	tempSet.addAll(subscribedMsgList);
	subscribedMsgList.clear();
	subscribedMsgList.addAll(tempSet);
...
{
    return hasAccessToPrivateDiscussion(context);
}
/////////////////// My Discussions /////////////////////////////////////////////////////////////////
/**
...
public boolean isPrivateUser(Context context,String args[]) throws Exception
{
    ...
    String roleReReader =   PropertyUtil.getSchemaProperty(context,"role_3DSRestrictedReader");
    if (loggedInRole.contains(roleOwner) || loggedInRole.contains(roleSecuredOwner) || loggedInRole.contains(roleAdmin) || loggedInRole.contains(roleReader) || loggedInRole.contains(roleReReader) )  {
           return false;         
    }
    return hasAccessToPrivateDiscussion(context);
}
...
private boolean hasAccessToPrivateDiscussion(Context context) throws Exception, FrameworkException {
	...
	person = (Person)DomainObject.newInstance(context, person.getPerson(context).getId(context));
 ...
 */
 ...
 /** 
  * TypeMessageDeleteOverride - gets the list of thread, pubsub, event objects connected to the context Disucssion(Type message). Deletes the thread object if only 1 message object is connected to it. Deletes all the pubsub and event object connected to the message object.
  * @param context the eMatrix <code>Context</code> object
  * @param args holds the following input arguments:
  *        0 - Discussion(Message) Object Id
  * @returns int
  * @throws Exception if the operation fails
  * @since R421.HF2
  * Need to remove this if the delete trigger is not added on Type Message.
  */

	@com.matrixone.apps.framework.ui.ProgramCallable
	public int deleteMessage (Context context, String[] args) throws Exception{
		try
		{
			String objectId = args[0];
			String selectable_thread_object = "to[" + DomainConstants.RELATIONSHIP_MESSAGE + "].from.id";
			String selectable_message_objects = "to[" + DomainConstants.RELATIONSHIP_MESSAGE + "].from.from[" + DomainConstants.RELATIONSHIP_MESSAGE + "].to.id";
			String selectable_event_object = "from[" + DomainConstants.RELATIONSHIP_PUBLISH_SUBSCRIBE + "].to.from[" + DomainConstants.RELATIONSHIP_PUBLISH + "].to.id";
			DomainObject domObject = new DomainObject(objectId);
			StringList strList = new StringList(4);
			StringList strMultiValueList = new StringList(2);
			strList.addElement(SELECT_PUBLISH_SUBSCRIBE_ID);
			strList.addElement(selectable_thread_object);
			strList.addElement(selectable_event_object);
			strList.addElement(selectable_message_objects);
			strMultiValueList.addElement(selectable_event_object);
			strMultiValueList.addElement(selectable_message_objects);
			Map objMap = domObject.getInfo(context, strList, strMultiValueList);
	 
			StringList objectsToDelete = new StringList();
			StringList eventObjectIds = (StringList)objMap.get(selectable_event_object);
			String threadId = (String) objMap.get(selectable_thread_object);
			String PubSubId = (String) objMap.get(SELECT_PUBLISH_SUBSCRIBE_ID);
			if(null != eventObjectIds && eventObjectIds.size() > 0) {
				objectsToDelete.addAll(eventObjectIds);
			}
			if(UIUtil.isNotNullAndNotEmpty(PubSubId)) {
				objectsToDelete.add(PubSubId);
			}
	 
			if(UIUtil.isNotNullAndNotEmpty(threadId)) {
				StringList messageIds = (StringList) objMap.get(selectable_message_objects);
				if(messageIds.size() == 1) {
					objectsToDelete.add(threadId);
				}
			}
			String[] objectsToDeleteArr = new String[objectsToDelete.size()];
			for(int i = 0; i < objectsToDelete.size(); i++) {
				objectsToDeleteArr[i] = objectsToDelete.get(i);
			}
			if(objectsToDeleteArr.length > 0) {
				deleteObjects(context,objectsToDeleteArr);
			}
		}catch (Exception e){
			return 1;
		}
    ...
    public static String getRootMessageId(Context context,String messageObjId) throws Exception
    {
        ...
        if("True".equalsIgnoreCase(isReplyObj))
        {
	 ...
	 public static int sendDiscussionCreatedNotfication(Context context, String[] args) throws Exception {
			if (args == null || args.length < 1) {
				throw (new IllegalArgumentException());
			}
			String mailEnabled = "false";
			StringList subscribersList = SubscriptionUtil.getSubscribersList(context,  args[0], "APPObjectNewDiscussionEvent","object", true);
	    	...
	    	String firstMsgObjId = getRootMessageId(context,args[0]);
	    	StringList sl = new StringList();
	        sl.addElement(getAttributeSelect(ATTRIBUTE_SUBJECT));
	        sl.addElement(DomainConstants.SELECT_DESCRIPTION);
	        sl.addElement(DomainConstants.SELECT_NAME);
	        sl.addElement("to["+RELATIONSHIP_MESSAGE+"].from.to["+RELATIONSHIP_THREAD+"].from.physicalid");
	        ...
	        sl.addElement("to["+RELATIONSHIP_MESSAGE+"].from.to["+RELATIONSHIP_THREAD+"].from.attribute[Title]");
	        ...
	        if(DomainConstants.TYPE_ROUTE.equalsIgnoreCase((String) firstObjMap.get("to["+RELATIONSHIP_MESSAGE+"].from.to["+RELATIONSHIP_THREAD+"].from.type")) && toList.size() > 0) {
		        ...
		        routeInfo.put(RouteTaskNotification.ROUTE_ID, (String) firstObjMap.get("to["+RELATIONSHIP_MESSAGE+"].from.to["+RELATIONSHIP_THREAD+"].from.physicalid"));
		        routeInfo.put(RouteTaskNotification.ROUTE_NAME, (String) firstObjMap.get("to["+RELATIONSHIP_MESSAGE+"].from.to["+RELATIONSHIP_THREAD+"].from.attribute[Title]"));
		        routeInfo.put(RouteTaskNotification.ROUTE_OWNER, (String) firstObjMap.get("to["+RELATIONSHIP_MESSAGE+"].from.to["+RELATIONSHIP_THREAD+"].from.owner"));
	        	...
	        	if(UIUtil.isNullOrEmpty(args[1])){
	        		routeInfo.put(RouteTaskNotification.DISCUSSION_NAME, (String) firstObjMap.get(DomainConstants.SELECT_NAME));
	        		routeInfo.put(RouteTaskNotification.DISCUSSION_SUBJECT, (String) firstObjMap.get(getAttributeSelect(ATTRIBUTE_SUBJECT)));
	        		routeInfo.put(RouteTaskNotification.DISCUSSION_MESSAGE, (String) firstObjMap.get(DomainConstants.SELECT_DESCRIPTION));
	        		notifyObj.sendDiscussionPublishEvent3DNotification(context, routeInfo, "Discussion Created", toList);
	        	}else {
	        		DomainObject replyObject = new DomainObject(args[0]);
	        		sl = new StringList();
	    	        sl.addElement(getAttributeSelect(ATTRIBUTE_SUBJECT));
	    	        sl.addElement(DomainConstants.SELECT_DESCRIPTION);
	    	        Map replyMap = replyObject.getInfo(context,sl);
	    	        routeInfo.put(RouteTaskNotification.DISCUSSION_NAME, (String) firstObjMap.get(getAttributeSelect(ATTRIBUTE_SUBJECT)));
	    	        routeInfo.put(RouteTaskNotification.DISCUSSION_SUBJECT, (String) replyMap.get(getAttributeSelect(ATTRIBUTE_SUBJECT)));
	    	        routeInfo.put(RouteTaskNotification.DISCUSSION_MESSAGE, (String) replyMap.get(DomainConstants.SELECT_DESCRIPTION));
	    	        notifyObj.sendDiscussionPublishEvent3DNotification(context, routeInfo, "Reply Created", toList);
	        	}
	        }
	        String notifyType = "both";
	        if("false".equalsIgnoreCase(mailEnabled) && DomainConstants.TYPE_ROUTE.equalsIgnoreCase((String) firstObjMap.get("to["+RELATIONSHIP_MESSAGE+"].from.to["+RELATIONSHIP_THREAD+"].from.type"))){
				notifyType = "iconmail";
			}
	        if(UIUtil.isNullOrEmpty(args[1])){
	        	Map payload = new HashMap<>();
	 	        payload.put("args_0",  args[0]);
	 	        payload.put("args_1",  "APPObjectNewDiscussionEvent");
	 	        emxNotificationUtil_mxJPO.objectNotification(context, args[0], "APPObjectNewDiscussionEvent", payload,notifyType);
	        }else {
	        	Map payload = new HashMap<>();
	 	        payload.put("args_0",  args[0]);
	 	        payload.put("args_1",  "APPObjectNewReplyEvent");
	 	        emxNotificationUtil_mxJPO.objectNotification(context, args[0], "APPObjectNewReplyEvent", payload,notifyType);
	        }
			return 0;
		}
}
