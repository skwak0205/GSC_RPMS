	...
	*/
	public Vector showRequestedAssignees( Context context, String[] args ) throws Exception
	{
		...
		**/
		...
		mulValSelects.add( "to[" + RELATIONSHIP_REQUESTED_ASSIGNEE + "].from.attribute[" + DomainConstants.ATTRIBUTE_FIRST_NAME + "]" );
		mulValSelects.add( "to[" + RELATIONSHIP_REQUESTED_ASSIGNEE + "].from.attribute[" + DomainConstants.ATTRIBUTE_LAST_NAME + "]" );
		...
		MapList reqMapList = DomainObject.getInfo( context, objIdArray, strList, mulValSelects);
	...
	*/
	public Vector showRequestedDocuments( Context context, String[] args ) throws Exception
	{
		...
		**/
		...
		StringList mulValselects = new StringList();
		mulValselects.add( "from[" + RELATIONSHIP_REQUESTED_DOCUMENT + "].to.id" );
		mulValselects.add( "from[" + RELATIONSHIP_REQUESTED_DOCUMENT + "].to.name" );
		mulValselects.add( "from[" + RELATIONSHIP_REQUESTED_DOCUMENT + "].to.type" );
		...
		try
		{
			reqMapList = DomainObject.getInfo( context, objIdArray, strList, mulValselects);
		}
		finally
		{
	...
	*/
	public Vector showStatusIcon( Context context, String[] args ) throws Exception
	{
		...
		**/
		...
		StringList mulValSelects = new StringList();
		mulValSelects.add( "attribute[" + ATTRIBUTE_GRANT_EXPIRATION_DATE + "]" );
		mulValSelects.add( DomainConstants.SELECT_CURRENT );
		...
		// All Access Request Assignee Information
		MapList reqMapList = DomainObject.getInfo( context, objIdArray, objectSelects, mulValSelects);
...
// IR-020143V6R2011x - End
	...
	*/
	public void autoAccessRequestRevise(Context context, String[] args)  throws Exception
	{
		...
		{
			requestObj.reviseObject( context, true);
		}
	}
		...
		**/
		...
		if ( strNextState.equals( strReview ) )
		{
			...
			try
			{
				...
				mqlCommand.append("modify bus $1 grant $2 access");
				StringList mqlParam = new StringList();
	        	mqlParam.add(objectId);
	        	mqlParam.add(strOriginator);
	        	String[] physicalMasksList = strAccessOriginator.split(",");
	        	int index=0;
	        	for (int i = 0; i < physicalMasksList.length; i++) {
	        		index = i+3;
	        		mqlCommand.append(" $"+index);
	        		if(i!=physicalMasksList.length-1){
	        			mqlCommand.append(",");
	        		}
	        		mqlParam.add(physicalMasksList[i]);
	    		}
	        	MqlUtil.mqlCommand(context, mqlCommand.toString(), mqlParam);
			}
			finally
			{
				ContextUtil.popContext(context);
		...
		if(reqMapList!=null)
		{
				...
				for (int i = 0; i<reqMapListLength; i++)
				{
					...
					if( isprinterFriendly || !hasReadShowAccess(objectAccessMap, objId))
					{
						...
						else if(LAUNCH.equals(column))
						{
							if("HTML".equals(paramList.get("reportFormat")))
							{
								sbColumValues.append("<img border=\"0\" src=\"../common/images/iconActionNewWindow.png\" title=\"Open in new window\">");
							}
						}
					}
					else
			...
			else if(CommonDocument.SELECT_HAS_ROUTE.equals(column))
			{
				for ( int i = 0;i<reqMapListLength ;i++ )
				{
					...
					{
						showObjectRoute = "<img border='0' src='../common/images/iconSmallRoute.png' alt=''>";
					}
					//XSSOK
...
/**
    * This Access program is used to check for Library Central Access Request
    * @param context The ematrix context of the request.
    * @return a boolean
    * @throws Exception
    * @since since Common V6R2009x
    */
     public boolean checkForLibraryCentralAccessRequest(Context context,String[] args)throws Exception
       {
           try{
               String command = "print program eServiceSystemInformation.tcl select property[appVersionLibraryCentral] dump |";
               String result  = MqlUtil.mqlCommand(context,command);
