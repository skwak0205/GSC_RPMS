...
import com.matrixone.MCADIntegration.server.definition.MCADAttribute;
...
public class IEFBuildFolderStructure_mxJPO
{
	...
	{ 
		...
		// IR-653361 - IF datetimepattern present in args ,then read value of datetimepattern, locale, timezoneoffset.
	    try
	    {
	    	int iArgNo = 5;
	        if(args.length > iArgNo)
	    	...
	    	System.out.println("[IEFBuildFolderStructure.fetchContent] EXCEPTION : " + ex.getMessage());
	    	ex.printStackTrace();
	    }
		...
		try 
		{
			...
			{
				if(commandName == null){
					//IR-513586 : If entry not found in typeNavigationMappings of IEFDesktopConfigurations JPO, will throw Exception with below error message
					//String errorMsg ="The command is null for  "+ args[1] +". No entry found in typeNavigationMappings.";
					Hashtable messageTokens = new Hashtable();
					messageTokens.put("TYPE",args[1]);
					MCADServerResourceBundle serverResourceBundle=new MCADServerResourceBundle(args[0]);
					String errMsg = serverResourceBundle.getString("mcadIntegration.Server.Message.EntryMissingIntypeNavigationMappings",messageTokens);
					throw (new FrameworkException(errMsg));
				}
				// IR-653361 - If datetimepattern is not null ,then pass values of datetimepattern, locale, timezoneoffset to evaluateUIComponent as argumnents.
				if(datetimepattern!=null)
				{
					String[] newArgs = {args[0], commandName, busId, sRelationshipName,datetimepattern,locale,timezoneoffset};
	...
	public String getBasicProperties(Context context, String[] args) throws Exception
	{
		...
		//IR-792464-3DEXPERIENCER2021x : check for 'Template to Document' relationship exist on Document
		String relTemplateToDocument		= MCADMxUtil.getActualNameForAEFData(context, "relationship_TemplateToDocument");
		String CDM_SELECT_TemplateToDocument	= "from[" + relTemplateToDocument + "].to.id";
...
MCADServerResourceBundle serverResourceBundle=new MCADServerResourceBundle(lang);
			...
			busSelect.add(CDM_SELECT_TemplateToDocument);
...
//issue related to multiple name display of image file
			...
			propertiesMap.put(DomainConstants.SELECT_FILE_NAME,filenamesList);
						...
						"false".equalsIgnoreCase(strCDMIsVersionObject ))
				{
					...
					//IR-792464-3DEXPERIENCER2021x : check for 'Template to Document' relationship exist on Document
					String templateToDocumentID =null;
					Object vObject1 = propertiesMap.get(CDM_SELECT_TemplateToDocument);
					if(vObject1 != null) 
						templateToDocumentID = (String) vObject1;
					if(templateToDocumentID!= null && !templateToDocumentID.isEmpty())
						propertiesMap.put("istemplatetodocument", "true");
					else
						propertiesMap.put("istemplatetodocument", "false");
			 ...
			 if(vObject != null) policy = (String) vObject;
				...
				propertiesMap.remove(CDM_SELECT_TemplateToDocument);
				...
				//language added for NSL for IR-404274-3DEXPERIENCER2019x : L86 
								...
								while (itr.hasNext())
								{
									...
									{	
										...
										{	//_util.setAttributeValue(context,minorDocument, MxAttribName, MxAttribValue);
											String typeMinorDoc = minorDocument.getTypeName();

											/* IR-767626-3DEXPERIENCER2021x : Doc of type QSD has an integer type attribute Periodic Review Interval, which has a trigger on it which blocks the update on minor attribute.
											So in this case , minor document will not get updated.*/
											try
											{
												minorDocument.setAttributes(context,attList);
											}
											...
											{
												System.out.println("[IEFBuildFolderStructure.updateProperties]...Exception while updating the minor document of type " + typeMinorDoc);
											}
		...
		try
		{
			...
			{               
				...
				if(!notAllowedAttributes.contains(attributeName))
				{
					...
					String sOperator = attrTypeObj.getDataType(context); //IR-548306-3DEXPERIENCER2015x : L86
					...
					if(MCADStringUtils.isNullOrEmpty(sOperator))
					{										
						sOperator = _util.getTypeForAttribute(context, attributeName);
					}
					...
					// IR-601796-3DEXPERIENCER2019x: Added for attributes which are added via Attribute Management

					if(attributeName != null && attributeName.startsWith("XP_") && attributeName.contains(".")) {
						String[] strArray = attributeName.split("\\.");
						argumentsTable1.put("adminName", strArray[strArray.length - 1]);
					} else {
					...
					// IR-687110-3DEXPERIENCER2019x: Added 'mandatory' & 'readonly" properties of attribute to return in getBasicProperties response of MSF
					
					MCADAttribute objMcadAttribute =(MCADAttribute) _util.getAttributeDefinition(context, attributeName);
					if(null != objMcadAttribute) {

						String mandatory = objMcadAttribute.getMandatory();
						if(mandatory != null && mandatory.equalsIgnoreCase("Yes"))
							attribTable.put("mandatory", "true");

						String userAccess = objMcadAttribute.getUserAccess();
						if(userAccess != null && userAccess.equalsIgnoreCase("ReadOnly"))
							attribTable.put("readonly", "true");
					}
				...
				while(nodeEnum.hasMoreElements())
				{
					...
					if(attributeNode != null)
					{
						...
						if("attribute".equals(nodeName))
						{
							...
							if(multiline == null || multiline.equals(""))
								multiline = "false";
		...
		while(keys.hasMoreElements()){
			...
			if(i <= 0) {
				...
				String sMxAttrName = (String)VMxAttrName.elementAt(0);
							...
							if (command.executeCommand(context, "print attribute $1 select $2 $3 $4 $5 $6 dump $7", sMxAttrName, "Valuetype", "multiline", "type", "default", "range" ,"|")){
								...
								if(st.hasMoreTokens())
								{
									...
									if(sDefault != null && sDefault.startsWith("=")) {
										sDefault = sDefault.replace("=", "").trim();
									}
				...
				attribTable.put("defaultvalue", sDefault);
										...
										// IR-669517-3DEXPERIENCER2019x: Added mandatory and readonly properties of attribute

										try {
											MCADMxUtil _util = new MCADMxUtil(context, null,_GlobalCache);
					
											MCADAttribute objMcadAttribute = (MCADAttribute) _util.getAttributeDefinition(context, sMxAttrName);
											if(null != objMcadAttribute) {
											...
											System.out.println("[IEFBuildFolderStructure.getMetaDataInformationForCADToMXAttributes]: Error in getting mandatory and readonly properties of attribute " + sMxAttrName + " :"+ ex.getMessage());
											ex.printStackTrace();
										}
	...
	private boolean doesAttributeExist(Context context, String attributeName) {
		...
		String argsToMQL[] = new String[1];
		
		try {
			
			if(attributeName != null && attributeName.startsWith("XP_") && attributeName.contains(".")) {
				String[] strArray = attributeName.split("\\.");
				argsToMQL[0] = strArray[strArray.length - 1];
			} else {
				argsToMQL[0] = attributeName;
			}
