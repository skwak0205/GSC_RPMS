...
public class emxModelBase_mxJPO extends emxDomainObject_mxJPO {
    ...
    /**
    	...
    	*/
    		...
    		private static final String strDERIVEDPrefix = "to[" + DerivationUtil.RELATIONSHIP_DERIVED_ABSTRACT + "].from.";
    		private static final String strDERIVEDIdSelect = strDERIVEDPrefix + DomainConstants.SELECT_ID;
    		private static final String strDERIVEDNameSelect = strDERIVEDPrefix + "attribute[" + ProductLineConstants.ATTRIBUTE_MARKETING_NAME + "]";
    		private static final String strDERIVEDRevisionSelect = strDERIVEDPrefix + DomainConstants.SELECT_REVISION;
    /**
    ...
    */
    ...
    public static MapList getStructureList(Context context, String[] args)
	    ...
	    if (objectType != null && mxType.isOfParentType(context, objectType, ProductLineConstants.TYPE_MODEL)) {
            ...
            /*modelStructList = (MapList) ${CLASS:emxPLCCommon}.getStructureListForType(context, args);*/
        	modelStructList = (MapList) JPO.invoke(context, "emxPLCCommon", null, "getStructureListForType", args, MapList.class);
        }
       ...
       throws Exception
    {
     ...
     /** This method addede for JPO to JAR conversion returns the Last Unit Number of the Model
      *  @param context the eMatrix <code>Context</code> object
      *  @param args holds the Model Id
      *  @return int value of the updated Last Unit Number
      *  @throws Exception if the operation fails
      */
     public int getLastUnitNumber(Context context, String[] args)
       throws Exception
	{
		Map paramMap = (HashMap) JPO.unpackArgs(args);
		String strModelId = (String) paramMap.get("paramString");
		return getLastUnitNumber(context, strModelId);
	}
...
public StringList getCommittedProductForFeature(Context context,String[]args) throws Exception
{
	...
	for(int i=0;i<slCommitedProduct.size();i++)
	{
		...
		sbProductIconTag.append("<img src=\"../common/images/iconSmallProduct.png\"/><h6>");
...
public Map refreshCandidateFeatureListPageOnApply(Context context, String[] args) throws Exception {
	...
	public void connectToProdutLine(Context context, String[] args) throws Exception {
		...
		String strMode = null;
		String[] strArryMode = (String[]) requestMap.get("mode");
		if(strArryMode != null){
			strMode = strArryMode[0];
		}
		...
		if(strParentID!=null && !strParentID.equals("")){
			...
			if(ProductLineCommon.isNotNull(strMode) && strMode.equalsIgnoreCase("Create")){
 ...
 */
  ...
  */
	...
	public MapList getEffProdDerivationChain(Context context,
		...
		String fromcaller = (String) programMap.get("fromCaller");
		...
		try {
			...
			//if(fromcaller != null && fromcaller.equalsIgnoreCase("applicability"))	{
				for(int i=0;i<mlDerivationStructure.size();i++)
				 	{
					 	Map objMap = (Map)mlDerivationStructure.get(i);
					 	//String strstate = (String)objMap.get(DomainConstants.SELECT_CURRENT);    
					 	String objectId = (String) objMap.get("physicalid");
					 	DomainObject domObj = new DomainObject(objectId);
					 	...
					 	if(Product.isFrozenState(context, objectId)){
	 ...
	 * @param context the eMatrix <code>Context</code> object
	 * @param args
	 * @return MapList - List of all available Models
	 * @throws Exception if the operation fails
	 * @since R420
	 */
	@com.matrixone.apps.framework.ui.ProgramCallable
	public MapList getAllModels(Context context, String[] args)
	{
		MapList listOfModels     = new MapList();
        
        try 
        {
        	StringList objectSelects = new StringList(DomainConstants.SELECT_ID);
            String strType           = ProductLineConstants.TYPE_MODEL;
			listOfModels             = findObjects(context, strType, null,DomainConstants.EMPTY_STRING,null,DomainConstants.QUERY_WILDCARD,null,true, objectSelects);
	...
	/** This is expand program. This is used to get/return expanded structure on Expand All.
	 *  
	 * @param context the eMatrix <code>Context</code> object
	 * @param args - Holds the HashMap containing the following arguments
	 *          programMap - contains ObjectId
	 * @return MapList - Contains List of expanded objects (Product Line, Model, Product)
	 * @throws Exception - if the operation fails
	 * @since R420
	 */
	@com.matrixone.apps.framework.ui.ProgramCallable
	public MapList getMyDeskProductHierarchy(Context context, String[] args) throws Exception
	{
		List<Map<?, ?>> modelsMap = null;
		List<Map<?, ?>> finalMapList = new MapList();
		...
		try 
		{
			...
			String strObjectType      = domainObject.getInfo(context, DomainConstants.SELECT_TYPE);
			
			if(level == 0)
			{
				StringList listSelect = new StringList();
				listSelect.add("from["+ ProductLineConstants.RELATIONSHIP_MAIN_PRODUCT+ "].to.id");
				listSelect.add("from["+ ProductLineConstants.RELATIONSHIP_MAIN_PRODUCT+ "].to.type");
				listSelect.add("from["+ ProductLineConstants.RELATIONSHIP_MAIN_PRODUCT+ "].name");
				Map modelMap = (Map) domainObject.getInfo(context, listSelect);
				
				if(modelMap != null && modelMap.size() > 0 && modelMap.containsKey("from["+ ProductLineConstants.RELATIONSHIP_MAIN_PRODUCT+ "].to.id"))
				{
					int rootProductLevel = 1;
					Map map = new HashMap();
					map.put("type", modelMap.get("from["+ ProductLineConstants.RELATIONSHIP_MAIN_PRODUCT+ "].to.type"));
					map.put("relationship", modelMap.get("from["+ ProductLineConstants.RELATIONSHIP_MAIN_PRODUCT+ "].name"));
					map.put("id", modelMap.get("from["+ ProductLineConstants.RELATIONSHIP_MAIN_PRODUCT+ "].to.id"));
					map.put("level", String.valueOf(rootProductLevel));
					finalMapList.add(map);
					
					List<Map<?,?>> mainDerivedList             = DerivationUtil.getMainDerivations(context, (String) modelMap.get("from["+ ProductLineConstants.RELATIONSHIP_MAIN_PRODUCT+ "].to.id"));
					List<Map<?,?>> productDerivationStructure  = DerivationUtil.getDerivationStructure(context, (String) modelMap.get("from["+ ProductLineConstants.RELATIONSHIP_MAIN_PRODUCT+ "].to.id"), ProductLineConstants.TYPE_PRODUCTS, rootProductLevel, 0, limit);
					
					for(Map derivationMap : productDerivationStructure)
					{
						int intLevel = Integer.parseInt((String)derivationMap.get("level")) + 1;
						
						Map objectMap = new HashMap();
						objectMap.put("type", derivationMap.get("type"));
						objectMap.put("relationship", derivationMap.get("relationship"));
						objectMap.put("id", derivationMap.get("id"));
						objectMap.put("level", String.valueOf(intLevel));
						addToDerivationMap(objectMap, derivationMap);
						finalMapList.add(objectMap);
					}
					
					for(Map mainDerivedMap : mainDerivedList)
					{
						Map objectMap = new HashMap();
						objectMap.put("type", mainDerivedMap.get("type"));
						objectMap.put("relationship", mainDerivedMap.get("relationship"));
						objectMap.put("id", mainDerivedMap.get("id"));
						objectMap.put("level", String.valueOf(rootProductLevel));
						
						finalMapList.add(objectMap);
						
						productDerivationStructure  = DerivationUtil.getDerivationStructure(context, (String) mainDerivedMap.get("id"), ProductLineConstants.TYPE_PRODUCTS, rootProductLevel, 0, limit);
						for(Map derivationMap : productDerivationStructure)
						{
							int intLevel = Integer.parseInt((String)derivationMap.get("level")) + 1;
							
							Map objectDerivationMap = new HashMap();
							objectDerivationMap.put("type", derivationMap.get("type"));
							objectDerivationMap.put("relationship", derivationMap.get("relationship"));
							objectDerivationMap.put("id", derivationMap.get("id"));
							objectDerivationMap.put("level", String.valueOf(intLevel));
							addToDerivationMap(objectDerivationMap, derivationMap);
							finalMapList.add(objectDerivationMap);
						}
					}
			    }
				...
				{
					int rootProductLevel = 1;
					List<Map<?,?>> productDerivationStructure  = DerivationUtil.getDerivationStructure(context, strObjectid, ProductLineConstants.TYPE_PRODUCTS, rootProductLevel, 0, limit);
					for(Map derivationMap : productDerivationStructure)
					{
						int intLevel = Integer.parseInt((String)derivationMap.get("level"));
						Map objectDerivationMap = new HashMap();
						objectDerivationMap.put("type", derivationMap.get("type"));
						objectDerivationMap.put("relationship", derivationMap.get("relationship"));
						objectDerivationMap.put("id", derivationMap.get("id"));
			...
			{
				if(lstModelChildTypes.contains(strObjectType))
				{
					StringList listSelect = new StringList();
					listSelect.add("from["+ ProductLineConstants.RELATIONSHIP_MAIN_PRODUCT+ "].to.id");
					listSelect.add("from["+ ProductLineConstants.RELATIONSHIP_MAIN_PRODUCT+ "].to.type");
					...
					if(modelMap != null && modelMap.size() > 0 && modelMap.containsKey("from["+ ProductLineConstants.RELATIONSHIP_MAIN_PRODUCT+ "].to.id"))
					{
						...
						for(Map mainDerivedMap : mainDerivedList)
						{
							...
							objectMap.put("id", mainDerivedMap.get("id"));
							objectMap.put("level", String.valueOf(rootProductLevel));
							addToDerivationMap(objectMap, mainDerivedMap);
							finalMapList.add(objectMap);
						}
					}
				}
				...
				{
					  int rootProductLevel = 1;
					  List<Map<?,?>> prdStructureList = DerivationUtil.getDerivationStructure(context, strObjectid, ProductLineConstants.TYPE_PRODUCTS, rootProductLevel, 0, limit);
					  for(Map prdStructureMap : prdStructureList)
					  {
					    	if(String.valueOf(level).equals((String)prdStructureMap.get("level")))
					    	{
								Map objectDerivationMap = new HashMap();
								objectDerivationMap.put("type", prdStructureMap.get("type"));
								objectDerivationMap.put("relationship", prdStructureMap.get("relationship"));
								objectDerivationMap.put("id", prdStructureMap.get("id"));
								objectDerivationMap.put("level", prdStructureMap.get("level"));
								addToDerivationMap(objectDerivationMap, prdStructureMap);
		...
		return new MapList(finalMapList);
	}

	private void addToDerivationMap(Map DerivationMap, Map ValueMap){
		String key1 = "to["+ProductLineConstants.RELATIONSHIP_MAIN_DERIVED+"].from.";
		if(ValueMap.containsKey(key1+"id")){
			DerivationMap.put(key1+"id",ValueMap.get(key1+"id"));
			DerivationMap.put(key1+"attribute[Title]",ValueMap.get(key1+"attribute[Title]"));
			DerivationMap.put(key1+"name",ValueMap.get(key1+"name"));
			DerivationMap.put(key1+"attribute[" + ProductLineConstants.ATTRIBUTE_MARKETING_NAME + "]",ValueMap.get(key1+"attribute[" + ProductLineConstants.ATTRIBUTE_MARKETING_NAME + "]"));
			DerivationMap.put(key1+DomainConstants.SELECT_REVISION,ValueMap.get(key1+DomainConstants.SELECT_REVISION));
		}
		key1 = "to["+ProductLineConstants.RELATIONSHIP_DERIVED+"].from.";
		if(ValueMap.containsKey(key1+"id")){
			DerivationMap.put(key1+"id",ValueMap.get(key1+"id"));
			DerivationMap.put(key1+"name",ValueMap.get(key1+"name"));
			DerivationMap.put(key1+"attribute[" + ProductLineConstants.ATTRIBUTE_MARKETING_NAME + "]",ValueMap.get(key1+"attribute[" + ProductLineConstants.ATTRIBUTE_MARKETING_NAME + "]"));
