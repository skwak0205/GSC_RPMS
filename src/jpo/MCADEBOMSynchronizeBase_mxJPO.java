...
//import matrix.db.Visuals;//FUN080585: Removal of deprecated class
...
{	
	protected static final String MATCH_CADMODEL_REV		= "MATCH_CADMODEL_REV";			
	protected static final String LATEST_REV				= "LATEST_REV";
	...
	protected String parentNameAttr			= null;			//IR-801607
	...
	protected boolean isRootDrawing					= false;			//IR-801607
	...
	//IR-804478: balloonXmlWithoutTitle should be set to true only if IEF-ObjectAndFileNameDifferent is true in GCO and title is missing in balloon xml.
	//Also balloon xml should have name same as Object's title
	static final String balloonXmlWithoutTitle 			 = MCADMxUtil.doesballoonXmlWithoutTitle();
	...
	protected void init(Context _context) throws Exception
	{
		...
		parentNameAttr			= MCADMxUtil.getActualNameForAEFData(_context, "attribute_CADObjectName");			//IR-801607
	...
	public void executeCustom(Context _context, Hashtable resultAndStatusTable)  throws MCADException
	{
		try
		{
			// IR-952430: Collaboration triggers were getting called, which was leading to performance issue.
			// Added custom data to stop calling Collaboration as suggested by the respective team.
			_context.setCustomData("IS_IN_SYNC","true");
			// System.out.println("Setting Custom data to IS_IN_SYNC" + _context.getCustomData("IS_IN_SYNC"));
			init(_context);
			...
			{				
				isRootDrawing = true;			//IR-801607 : Specific to case when drawing has model		
		...
		catch(Exception e)
		{
			...
			// IR-952430: Removing custom data which was set at the beginning of EBOM Sync
			try {
			_context.removeFromCustomData("IS_IN_SYNC");
			// System.out.println("Removed Custom data to IS_IN_SYNC");
			}
	...
	protected void validateDrawingForEBOM(Context context, BusinessObject model) throws MatrixException, MCADException, MCADServerException
	{
		...
		for(int j = majorRevisions.size()-1 ; j > -1 ; j--)
		{
			...
			if(partIdOfPreviousRevision != null && !"".equals(partIdOfPreviousRevision))
			{
				...
				//IR-593702: to check current revision in released state or not.
				State stateOfBusObjectRevision 		= _util.getCurrentState(context,_busObject);
				String busObjectcurrentStateName 	= stateOfBusObjectRevision.getName();
				
				boolean isbusObjectReleased = busObjectcurrentStateName.equals(releasedState);
				...
				if(!isbusObjectReleased	&& isModelReleased &&  isPrevisiousRevisionReleased 
	...
	{       				
		String partId = "";
		String partMatchingRule				= ebomConfigObj.getConfigAttributeValue(IEFEBOMConfigObject.ATTR_PART_MATCHING_RULE);	//FUN101346
		try
		{	//[NDM]
			...
			//FUN101346: Changed the code similar to method getRelatedPart in MCADServerGeneralUtil
			String relPartSpecification	= MCADMxUtil.getActualNameForAEFData(_context, "relationship_PartSpecification");
			String retVal = "";
			if (partMatchingRule.equalsIgnoreCase(LATEST_REV)) {
				String Args[] = new String[3];
				Args[0] = busId;
				Args[1] = "to[" + relPartSpecification + "].from.id";	
				Args[2] = "|";

				retVal = _util.executeMQL(_context, "print bus $1 select $2 dump $3", Args); 

				if (retVal.startsWith("true"))
				{
					int lastIndex = retVal.lastIndexOf('|');
					partId = retVal.substring(lastIndex+1);
				}
			}	
			else {
				BusinessObject busObj = new BusinessObject(busId);
				busObj.open(_context);
				String revID = busObj.getRevision();
				busObj.close(_context);

			StringBuffer whereClause = new StringBuffer();
				whereClause.append("((name == const\"").append(relPartSpecification).append("\")").append(" && ")
						.append("(to.id == \"").append(busId).append("\")").append(" && ")
						.append("(from.revision == \"").append(revID).append("\"))");
			...
			Args[4] = "|";
				retVal = _util.executeMQL(_context, "expand bus $1 $2 select rel $3 where $4 dump $5", Args);
			if (retVal.startsWith("true"))
			{
	...
	protected String RecursiveBom(Context _context, String parentPartObjId, Vector cadRelIds, String cadObjId, String cadBusName, String instanceName, 
		...
		try
		{
			...
			{
				// 		[UPR1] IR-628979 : Same and Different Errors are accumulated without exception being thrown
				if(!recursiveEBOMError.contains(e.getMessage()))
			...
			if(partObjId != null && !"".equals(partObjId))
			{
				if(excludeFromBOM == null || (excludeFromBOM != null && "false".equalsIgnoreCase(excludeFromBOM)))
				{
					...
					//IR-383977 : multi-level EBOM SYNC creation from drawings in single transaction does not transfer Find Numbers for all levels 
					if(parentChildFindNumber != null && parentChildFindNumber.length() >0)
						updateFindNumber = "true";
			...
			if(childAndRelIds.size() > 0)
			{
				...
				for (Enumeration childIds = childAndRelIds.keys() ; childIds.hasMoreElements() ;)
				{
					...
					{
						//IR-383977 :multi-level EBOM SYNC creation from drawings in single transaction does not transfer Find Numbers for all levels 
						if(cadNameBalloonNumberMap1.isEmpty())
	...
	protected void deleteUnWantedParts(Context _context, String parentPartObdId, Vector latestChildIdVector,Vector asStoredChildIdVector) throws Exception
	{
		...
		{			
			for(int i = 0 ; i < asStoredChildIdVector.size(); i++)
			{
				...
				if(!childObjid.equals("") && (latestChildIdVector == null || !latestChildIdVector.contains(childObjid)) && !_generalUtil.isPartReleased(_context, parentPartObdId, PART_RELEASE_STATE))
				{
		...
		if(relIdsVector != null && relIdsVector.size() > 0)
		{
			for (int i=0; i < relIdsVector.size(); i++)
			{
				...
				if(relModificaionStatus.equalsIgnoreCase("deleted"))
				{
					...
					//IR-566186 - EBOM sync failed when a part is removed. Because of " it does not show proper error so " is replace by '
	...
	protected String createNewPartSpec(Context _context, String busObjectID, String busName, String instanceName, String busType, String cadType, StringBuffer ebomDoneObjectBuffer) throws Exception
	{
		...
		boolean relationExists = false;		//FUN101346
		boolean drawingPartSpecRelationExists = false;		//IR-801607
		...
		if(!handledPartObjIdsTable.containsKey(busObjectID))
		{
			...
			relationExists = _util.doesRelationExist(_context, busObjectID, matchingPartID, relPartSpecification);		//FUN101346

			if(matchingPartID != null && matchingPartID.length() > 0)
			{	
				boolean matchingPartReleased = _generalUtil.isPartReleased(_context, matchingPartID, PART_RELEASE_STATE);

				//IR-801607 : This case will work only when the Root is drawing and only root drawing will be considered during ebom sync
				// Check for root drawing and whether Part Spec relation exists with Model and not with Drawing,
				// and EC Part is released then connect drawing with EC Part
				if(isRootDrawing) {
					if(!busObjectID.equals(_busObjectID) && relationExists && matchingPartReleased) {
						drawingPartSpecRelationExists = _util.doesRelationExist(_context, _busObjectID, matchingPartID, relPartSpecification);		

						if(!drawingPartSpecRelationExists) {
							String result = "";
							String access = "modify";
							String drawingName	= _busObject.getName();
							Hashtable attrNameValTable = new Hashtable(2);
							attrNameValTable.put(parentNameAttr, drawingName);

							// Connect root drawing with EC Part
							_util.connectBusObjectsWithSameContext(_context, matchingPartID,_busObjectID,relPartSpecification,true,null,null);

							// Get Rel id between Drawing and EC Part
							Vector vRelID = _util.getRelationIdBetweenObjects(_context, matchingPartID, _busObjectID);
							String relID = (String) vRelID.get(0);

							// Grant Access to user for EC Part since EC Part is released
							_generalUtil.grantAccessForUser(_context,matchingPartID, _context.getUser(), access, "modAccess");
							String Args[] = new String[3];
							Args[0] = relID;
							Args[1] = parentNameAttr;
							Args[2] = drawingName;

							// Update attribute on Part Spec Relationship
							result = _util.executeMQL(_context,"mod connection $1 $2 $3", Args);

							// Revoke Access from user for EC Part since EC Part is released
							_generalUtil.revokeAccessForUser(_context,matchingPartID, "modAccess");

						}
					}
					// Set the value to false, since this entire if should be executed only once for Root Drawing 
					isRootDrawing = false;
				}
				...
				//FUN101346 : IF block checks - i. If Part is not released, go ahead and perform ebom
				// ii. If Part is released, connected/not connected and MATCH_CADMODEL_REV, NO_UPDATE
				// iii. If Part is released and not connected and LATEST_REV, go ahead and perform ebom
				//if(!matchingPartReleased || (matchingPartString.equals(LATEST_REV) && matchingPartReleased && !relationExists)) {
				// IR-834603
				if(!matchingPartReleased || (matchingPartReleased && !relationExists)) {
	...
	protected String createNewEBOM(Context _context, String parentPartId, String childPartId, Vector uniqueRelIdsVector,String parentChildFindNumber,boolean isParentFantom,String parentCadObjID,String childObjId) throws Exception
	{
	...
	public void verifyAndDoEBOM(Context _context, BusinessObject busObj, String instanceName, Hashtable resultAndStatusTable) throws Exception
	{
		...
		if(!partsTable.containsKey(busObjectID))
		{
			...
			RecursiveBom(_context, "",null,busObjectID, busName, instanceName, relsAndEnds, busTypeName, ebomDoneObjectBuffer,false,"","", new HashMap());
	...
	protected Vector getFirstLevelChildPartObjIds(Context _context, BusinessObject partObject) throws Exception 
	{
		...
		{
			short level = 1;
			//Visuals vis = null; //_context.getDefaultVisuals();//FUN080585: Removal of deprecated function getDefaultVisuals
			...
			Expansion expansion = _util.expandBusObject(_context, partObject, level,"",relationshipWhereClause.toString(), (short)0, false);//FUN080585: Deprecation of Visuals
	...
	protected boolean getBalloonNumberFromAssociatedDrawing(Context _context, String cadObjId, String parentBusId, HashMap cadNameBalloonNumberMap1) throws Exception
	{
		boolean hasConnectedDrwWithBalloonNumber = false;
				...
				if(null == strBaloonNumberList || strBaloonNumberList.size()==0)
		                ...
		                if(strBaloonNumberList != null)
			        {
				      ...
				      if(!strBaloonValue.equals(""))
				           {
							populateCadNameBalloonNumberMap(strBaloonValue,parentBusId, cadNameBalloonNumberMap1);
							hasConnectedDrwWithBalloonNumber = true;
							break;
				           }
			        }
	...
	protected void populateCadNameBalloonNumberMap(String attBallonNumberVal, String cadObjId, HashMap cadNameBalloonNumberMap1) throws Exception 
	{
		...
		if(attBallonNumberVal != null && attBallonNumberVal.length() >0)
		{
			...
			if(commandPacket != null)
			{
				...
				while(paramChildEnum.hasMoreElements())
				{
					...
					//IR-804478
					if("true".equalsIgnoreCase(balloonXmlWithoutTitle)) {
					...
					//IR-217524-3DEXPERIENCER2018x: check value of all attributes in XML are empty or not
					validateAttributesOfBalloonNumber(name,type,number,title);
				...
				//	cadNameBalloonNumberMap.put(balloonkey,number);
					cadNameBalloonNumberMap1.put(balloonkey,number);
				}
			}
		}
	}
	
	//IR-217524-3DEXPERIENCER2018x: check value of all attributes in XML are empty or not. EBOMSync will fail if any value of attribute is empty or null
	private void validateAttributesOfBalloonNumber(String name, String type, String number, String title) throws Exception
	{
		Boolean isAllAttributePresent = true;
			
		if(_globalConfig.isObjectAndFileNameDifferent())
		{
			if(title == null || title.equals(""))
				isAllAttributePresent = false;
		}
		else if(name == null || name.equals("")) 
			isAllAttributePresent = false;
		
		if(!isAllAttributePresent || type == null || type.equals("") || number == null || number.equals(""))
		{
			//Exception will throw with message "Value of Balloon Number attribute is not formed correctly. Either Name/Title or Type or Number is missing in value-xml."
			MCADServerException.createException(_serverResourceBundle.getString("mcadIntegration.Server.Message.BalloonNoAtttributeMissing"), null);
		}
	}
