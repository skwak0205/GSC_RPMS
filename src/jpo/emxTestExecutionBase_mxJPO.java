 ...
 * @quickreview KIE1 ZUD 17:02:21 : IR-495327-3DEXPERIENCER2018x: R419-STP: On Test Execution, Estimated Start Date & Estimated End Date attributes are not getting saved with modified value.
 * @quickreview KIE1 ZUD 17:09:18 : IR-545202-3DEXPERIENCER2018x: R20-STP: User with Contributor role is Not able to Create Test Execution
 @quickreview KIE1 ZUD 18:05:25 : No relationship between custom Test Case type and Execution Test
 */
...
public class emxTestExecutionBase_mxJPO extends emxDomainObject_mxJPO {
    ...
    protected static final String RELATIONSHIP_SUB_TE = "relationship_SubTestExecution";
    ...
    public MapList getRelatedTestExecutions(Context context, String[] args) throws Exception
    {
        ...
        try{
              ...
              if(domObj.isKindOf(context,DomainConstants.TYPE_ENGINEERING_CHANGE))
              {
               ...
               else if (domObj.isKindOf(context, DomainConstants.TYPE_TEST_CASE)) // added condition for HL Test Execution under Test Case without an EC
              {       
    ...
    public boolean showTestExecutionLinksToAssignedQE (Context context, String[] args) throws Exception {
    	return true; // remove the legacy checks for EC related items
    }
    ...
    public String displayFullCheckBox(Context context, String[] args) throws Exception{
    	return  EnoviaResourceBundle.getProperty(context, "emxProductLineStringResource", context.getLocale(),"emxProduct.Form.Label.CopySubStructure")+ "<input type=\"checkbox\" name=\"CopySubStructure\"/>";
    }
        
    // Get Related Sub Test Executions for a given TE
    @com.matrixone.apps.framework.ui.ProgramCallable
    public static MapList getRelatedTestExe (Context context, String[] args) throws Exception{
    
    	HashMap programMap = (HashMap)JPO.unpackArgs(args);
    	/*System.out.println("POC - "+programMap.values());
      	 System.out.println("POC -- "+programMap.keySet());*/
      	 
    	String objectId    = (String) programMap.get("objectId");
    	
    	String types = ProductLineConstants.TYPE_TEST_EXECUTION;
    	String rels = PropertyUtil.getSchemaProperty(context, RELATIONSHIP_SUB_TE) ;
    	return getRelatedTEWithRel(context, objectId, rels, types);
    }
    
    /***
     * Common Method to get related objects to given TE based on the 
     ...
     * @throws Exception
     */
    public static MapList getRelatedTEWithRel(Context context, String objectId, String rels, String types) throws Exception{
    	
    	MapList testExeStructList = new MapList();
    	DomainObject testExeObj   = DomainObject.newInstance(context, objectId);
    	...
    	testExeStructList = testExeObj.getRelatedObjects(context, rels, types, 
    			objectSelects, relSelects, false, true, (short)1, "", "", 0);	 

    	return testExeStructList;
    	
    }
    
    // Method to get the structure list for TE in UI tree
    public static MapList getStructureList(Context context, String[] args) throws Exception{
    	HashMap programMap = (HashMap) JPO.unpackArgs(args);
    	HashMap<String, String> paramMap   = (HashMap)programMap.get("paramMap");
    	String objectId    = (String)paramMap.get("objectId");

    	String types = ProductLineConstants.TYPE_TEST_EXECUTION + ","+PropertyUtil.getSchemaProperty(context, "type_PlmParameter");
    	String rels = PropertyUtil.getSchemaProperty(context, RELATIONSHIP_SUB_TE) 
    			+ "," +PropertyUtil.getSchemaProperty(context, "relationship_ParameterAggregation");
    	
    	return getRelatedTEWithRel(context, objectId, rels, types);
    }
    
    //Call for the table from the TE Copy Scope field
    @com.matrixone.apps.framework.ui.ProgramCallable
    public static MapList getTestCaseChildren(Context context, String[] args) throws Exception{
    	 HashMap programMap = (HashMap)JPO.unpackArgs(args);
    	 //System.out.println("POC GetTC - "+programMap.values());
    	 //System.out.println("POC GetTC-- "+programMap.keySet());
    	 
    	 String strObjectId = (String) programMap.get("objectId");
         
         // recurse only to 1 level
         short limit = 1;         
         TestExecution te = new TestExecution();
         return te.produceTestCaseStructure(context, strObjectId, limit);
    }
 ...
 // ++ HAT1 ZUD: IR-439318-3DEXPERIENCER2017x fix
    ...
    public void validatePropertiesEstimateDate(Context context, String[] args) throws Exception
    {
    	try{
          ...
          if(!strOldValue.equalsIgnoreCase("") && !"null".equalsIgnoreCase(strOldValue ))
        	  strOldEstimatedDate = com.matrixone.apps.domain.util.eMatrixDateFormat.getJavaDate(strOldValue);
                    
          if(strOldValue.equalsIgnoreCase("") || !strOldEstimatedDate.equals(strEstimatedDate)){
