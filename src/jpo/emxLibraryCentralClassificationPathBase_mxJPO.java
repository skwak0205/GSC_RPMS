...
implements LibraryCentralConstants {
    ...
    public InterfacePath getInterfacePath(Context context, String interfaceName)
        ...
        for (int i = size / 2 - 1; i >= 0; i--) {
            if (lstBooleans.get(i).toString().equalsIgnoreCase("TRUE")) {
                result.add((String)lstParents.get(i));
            }
        }
        ...
        throws FrameworkException
    {
        ...
        while (ifNameIter.hasNext()) {
			...
			try {
				strTitle = new DomainObject(strId).getInfo(context, SELECT_ATTRIBUTE_TITLE);
			} catch (Exception e) {}
            ClassificationInfo cls = new ClassificationInfo(strId, UIUtil.isNullOrEmpty(strTitle) ? strName : strTitle);
            result.add(cls);
        }
      ...
      public Vector getEndItemClassificationPathsFromRelationship(Context context, String endItemId)
         ...
         try {    
             ...
             while(classListItr.hasNext())
             {
                 ...
                 String title = (String)domObj.getInfo(context, DomainConstants.SELECT_ATTRIBUTE_TITLE);
                 if (UIUtil.isNullOrEmpty(title)) {
                	 title           = (String)domObj.getInfo(context, DomainConstants.SELECT_NAME);
                 }
                 ...
                 while (itr.hasNext())
                 {
                     ...
                     ClassificationInfo clsParent = new ClassificationInfo(parentClassId, UIUtil.isNullOrEmpty(parentClassTitle) ? parentclassName : parentClassTitle);
                     classPath.add(clsParent);
                 }
                 
                 ClassificationInfo clsObject = new ClassificationInfo(ObjectId, title);
