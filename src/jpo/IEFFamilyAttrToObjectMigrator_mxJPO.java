...
public class IEFFamilyAttrToObjectMigrator_mxJPO
{
	...
	private StringList multiValue_localcopy = new StringList();
        ...
        throws Exception
    {
		...
		//Changes made for FUN086196 - Discontinued use of DomainConstants.MULTI_VALUE_LIST
		...
		multiValue_localcopy.add(SELECT_MINOR_ID_LIST);
		multiValue_localcopy.add(SELECT_REVISION_LIST);
		multiValue_localcopy.add(SELECT_PART_SPECIFICATION_ID);
		multiValue_localcopy.add(SELECT_PART_SPEC_ATTR_CAD_OBJ_NAME);
    }
	...
	private void migrateAllFamilyObjects(Context context, StringList objectIdList, String integrationName) throws Exception
	{
		...
		MapList mapList = DomainObject.getInfo(context, oidsArray, objectSelectElements,multiValue_localcopy);
	...
	private Hashtable readIDsFile(String fileName) throws Exception
	{
		...
		while((objectIdLine = iefIDsReader.readLine()) != null)
		{
			...
			while(tokenizer.hasMoreElements())
			{
				familyTableIDs.addElement((String)tokenizer.nextElement());
			}
	...
	private void populateRelationDetails(Hashtable typeDetailsMap, StringList objectSelectElements, MCADGlobalConfigObject globalConf)
	{
		...
		while(relsEnum.hasMoreElements())
		{
			...
			multiValue_localcopy.add(relIdParent);
			multiValue_localcopy.add(relParentChildInstance);
			multiValue_localcopy.add(relIdChild);
			multiValue_localcopy.add(relChildParentInstance);
