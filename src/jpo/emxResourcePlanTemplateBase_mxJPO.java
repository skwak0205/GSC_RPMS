...
public class emxResourcePlanTemplateBase_mxJPO extends emxDomainObjectBase_mxJPO
{
	...
	public MapList getTableResourcePlanTemplateData(Context context, String[] args) throws Exception
	{
		try
		{
			...
			//String strPrefferedCurrency = ${CLASS:emxProgramCentralUtilBase}.getUserPreferenceCurrency(context, strErrorMessage);
	...
	public void createResourcePlanFromResourcePlanTemplate (Context context, String[] args) throws MatrixException
	{
		try {
			...
			String sourceTemplateId = (String)paramMap.get("sourceTemplateId");
			String strResourcePlanTemplateId = (String)paramMap.get("ResourceTemplateId");
			String  sourcePALId=ProgramCentralConstants.EMPTY_STRING;
			...
			if(ProgramCentralUtil.isNotNullString(sourceTemplateId))
			{
			 DomainObject sourceObj = DomainObject.newInstance(context,sourceTemplateId);
		     sourcePALId= sourceObj.getInfo(context, "to[" + RELATIONSHIP_PROJECT_ACCESS_LIST + "].from.id");
			}
			else{
			 sourcePALId= projectObj.getInfo(context, "from[" + RELATIONSHIP_INITIATED_TEMPLATE_PROJECT + "].to."+ "to[" + RELATIONSHIP_PROJECT_ACCESS_LIST + "].from.id");
			}
			...
			try {
				...
				ProjectSequence ps = new ProjectSequence(context,sourcePALId);
			...
			busSelect.add(ResourcePlanTemplate.SELECT_RESOURCE_REQUSET_PHASE_PHYSICALID);
			...
			BusinessObjectWithSelectList resourceRequestObjWithSelectList = 
					ProgramCentralUtil.getObjectWithSelectList(context, strRequestIds, busSelect, true);
			...
			for(BusinessObjectWithSelectItr itr= new BusinessObjectWithSelectItr(resourceRequestObjWithSelectList); itr.next();)
			{
				...
				StringList slPhaseIdList = bows.getSelectDataList(ResourcePlanTemplate.SELECT_RESOURCE_REQUSET_PHASE_PHYSICALID);
				
				for(int i=0; i<slPhaseIdList.size(); i++)
				{
					fte = FTE.getInstance(context);
					String strPhaseId = (String)slPhaseIdList.get(i);
					String strTaskWBS = (String) seqData.get(strPhaseId).getDataelements().get(ProgramCentralConstants.KEY_WBS_ID);
