...
public class MCADIntegGetLatestVersionBase_mxJPO 
{
		...
		try
		{
			...
			while (inputids.hasMoreElements())
			{
				...
				if(!MCADStringUtils.isNullOrEmpty(integrationSource))
				{
					...
					// XEW - IR-472394-3DEXPERIENCER2016x +
					...
					else // We are in OCDX environment -IR-395207-3DEXPERIENCER2015x -FLSmidth use case (Rijo Changes)
					{
	...
	// Temporarily revoked the modifications for PCS improvement IR-617578,571243
	...
	//Additional method for PCS Improvement for IR-617578, IR-571243
	private Hashtable getMajorIdAndLatestIDMap(Context context, String[] oids, Hashtable busidBusSelectMap)	throws Exception
	{
		Hashtable majorIdLatestIdMap = new Hashtable();
		Hashtable returnTable = new Hashtable();
		Hashtable latestIDMap = new Hashtable();
		HashSet revisionCreatedManually = new HashSet();
		BusinessObjectWithSelectList buslWithSelectionList = new BusinessObjectWithSelectList();
		boolean isSolutionBasedEnv = false;
		try 
		{
			if (_util.isSolutionBasedEnvironment(context))
			{
				isSolutionBasedEnv = true;
			}
			StringList busSelectionList = new StringList();
			busSelectionList.addElement("id");
			busSelectionList.addElement("type");
			...
			String SELECT_ON_MAJOR_ID = "to[" + REL_LATEST_VERSION + "].from.id";
			String SELECT_ON_MAJOR_WO_ID = "to[" + REL_LATEST_VERSION + "].from.";
			busSelectionList.addElement(SELECT_ON_MAJOR_ID);
			...
			// ---selectables from getLatestForObjectIdsSLWDPM start
			// ----------if outputid == busid------------
			busSelectionList.addElement("last." + SELECT_ON_ACTIVE_MINOR + "id"); // latest minor from major
			busSelectionList.addElement("last.id"); // latest major id from major
			...
			// -----------if outputid== SELECT_ON_MAJOR_ID-----------------------
			busSelectionList.addElement(SELECT_ON_MAJOR_WO_ID + "last." + SELECT_ON_ACTIVE_MINOR + "id"); // latest minor
			// from major
			busSelectionList.addElement(SELECT_ON_MAJOR_WO_ID + "last.id"); // latest major id from major

			if (retrieveHashcode)
			{
				busSelectionList.addElement(
						SELECT_ON_MAJOR_WO_ID + "last." + SELECT_ON_ACTIVE_MINOR + SELECT_ATTR_IEFFILEMESSAGEDIGEST);
				busSelectionList.addElement(SELECT_ON_MAJOR_WO_ID + "last." + SELECT_ATTR_IEFFILEMESSAGEDIGEST);
			}
			// -----------if outputid== SELECT_ON_MAJOR-----------------------
			busSelectionList.addElement(SELECT_ON_MAJOR + "last." + SELECT_ON_ACTIVE_MINOR + "id"); // latest minor from
			// major
			busSelectionList.addElement(SELECT_ON_MAJOR + "last.id"); // latest major id from major

			if (retrieveHashcode)
			{
				busSelectionList
				.addElement(SELECT_ON_MAJOR + "last." + SELECT_ON_ACTIVE_MINOR + SELECT_ATTR_IEFFILEMESSAGEDIGEST);
				busSelectionList.addElement(SELECT_ON_MAJOR + "last." + SELECT_ATTR_IEFFILEMESSAGEDIGEST);
			}
			// --selectables from getLatestForObjectIdsSLWDPM end

			// --selectables for isbulk load check start-----------------------
			String sVersionOfRelName = MCADMxUtil.getActualNameForAEFData(context, "relationship_VersionOf");
			String SELECT_ON_VERSIONOF_REL = "from[" + sVersionOfRelName + "]";
			busSelectionList.add(SELECT_ON_VERSIONOF_REL);
			// --selectables for isbulk load check end-----------------------

			// -------------selectables for DSCUITable method
			// start---------------------------
			String s1 = "last.from[Active Version].to.id";
			String s2 = "from[VersionOf].to.last.from[Active Version].to.id";
			String SELECT_CAD_TYPE_ATT = new StringBuffer("attribute[")
					.append(MCADMxUtil.getActualNameForAEFData(context, "attribute_CADType")).append("]").toString();
			...
			busSelectionList.addElement(SELECT_CAD_TYPE_ATT);
			// busSelectionList.addElement(ATTR_SOURCE);
			// busSelectionList.addElement(DomainConstants.SELECT_ID);
			busSelectionList.addElement(DomainConstants.SELECT_NAME);
			busSelectionList.addElement(DomainConstants.SELECT_DESCRIPTION);
			busSelectionList.addElement(SELECT_UPDATE_STAMP);
			// busSelectionList.addElement(SELECT_ISVERSIONOBJ);
			busSelectionList.addElement(s1);
			busSelectionList.addElement(s2);
			// new selectables added
			busSelectionList.addElement(SELECT_ON_INSTANCE + SELECT_UPDATE_STAMP);
			busSelectionList.addElement(SELECT_ON_INSTANCE + OBJECT_NAME);

			// -------------selectables for DSCUITable method end---------------------------


			buslWithSelectionList = BusinessObject.getSelectBusinessObjectData(context, oids, busSelectionList);
			for (int i = 0; i < buslWithSelectionList.size(); i++)
			{
				BusinessObjectWithSelect busObjectWithSelect = (BusinessObjectWithSelect) buslWithSelectionList
						.elementAt(i);
				String sIsVersion = busObjectWithSelect.getSelectData(SELECT_ISVERSIONOBJ);
				boolean isVersion = Boolean.valueOf(sIsVersion).booleanValue();
				String busID = busObjectWithSelect.getSelectData("id");
				...
				String outputId = busID;
				String outputId_forCallee = busID;
				String outputIdHashcode = "";

				String cadType = (String) busObjectWithSelect.getSelectData(SELECT_CAD_TYPE_ATT);
				Object SELECT_UPDATE_STAMP_ = busObjectWithSelect.getSelectData(SELECT_UPDATE_STAMP);
				Object NAME_ = busObjectWithSelect.getSelectData(DomainConstants.SELECT_NAME);
				Object DES_ = busObjectWithSelect.getSelectData(DomainConstants.SELECT_DESCRIPTION);
				Object instanceNamesObj = (Object) busObjectWithSelect.getSelectDataList(SELECT_ON_INSTANCE + OBJECT_NAME);
				Object instanceUpdatestampObj = (Object) busObjectWithSelect
						.getSelectDataList(SELECT_ON_INSTANCE + SELECT_UPDATE_STAMP);
				String s1_ = busObjectWithSelect.getSelectData(s1);
				String s2_ = busObjectWithSelect.getSelectData(s2);
				if (!MCADStringUtils.isNullOrEmpty(integrationSource))
				{
					if (isVersion)
					{
						String isVersionOfExist = busObjectWithSelect.getSelectData(SELECT_ON_VERSIONOF_REL);
						// if VersionOf relationship is false, it means bulk loading case
						if (!MCADUtil.getBoolean(isVersionOfExist))
						{
							outputId = busObjectWithSelect.getSelectData(SELECT_ON_MAJOR_ID);
							if (isSolutionBasedEnv)
							{

								outputId_forCallee = busObjectWithSelect
										.getSelectData(SELECT_ON_MAJOR_WO_ID + "last." + SELECT_ON_ACTIVE_MINOR + "id"); // minor
								// id
								if (retrieveHashcode)
								{
									outputIdHashcode = busObjectWithSelect.getSelectData(SELECT_ON_MAJOR_WO_ID + "last."
											+ SELECT_ON_ACTIVE_MINOR + SELECT_ATTR_IEFFILEMESSAGEDIGEST);
								}
							} else
							{
								outputId_forCallee = busObjectWithSelect.getSelectData(SELECT_ON_MAJOR_WO_ID + "last.id"); // major
								// id
								if (retrieveHashcode)
								{
									outputIdHashcode = busObjectWithSelect.getSelectData(
											SELECT_ON_MAJOR_WO_ID + "last." + SELECT_ATTR_IEFFILEMESSAGEDIGEST);
								}
							}
						} else
						{
							...
							{
								outputId_forCallee = busObjectWithSelect
										.getSelectData(SELECT_ON_MAJOR + "last." + SELECT_ON_ACTIVE_MINOR + "id"); // minor
								// id
								if (retrieveHashcode)
								{
									outputIdHashcode = busObjectWithSelect.getSelectData(SELECT_ON_MAJOR + "last."
											+ SELECT_ON_ACTIVE_MINOR + SELECT_ATTR_IEFFILEMESSAGEDIGEST);
								}
							} else
							{
								outputId_forCallee = busObjectWithSelect.getSelectData(SELECT_ON_MAJOR + "last.id"); // major
								// id
								if (retrieveHashcode)
								{
									outputIdHashcode = busObjectWithSelect
											.getSelectData(SELECT_ON_MAJOR + "last." + SELECT_ATTR_IEFFILEMESSAGEDIGEST);
								}
							}
						}
					} else
					{
						if (isSolutionBasedEnv)
						{
							outputId_forCallee = busObjectWithSelect.getSelectData("last." + SELECT_ON_ACTIVE_MINOR + "id"); // minor
							// id
							if (retrieveHashcode)
							{
								outputIdHashcode = busObjectWithSelect
										.getSelectData("last." + SELECT_ON_ACTIVE_MINOR + SELECT_ATTR_IEFFILEMESSAGEDIGEST);
							}
						} else
						{
							outputId_forCallee = busObjectWithSelect.getSelectData("last.id"); // major id
							if (retrieveHashcode)
							{
								outputIdHashcode = busObjectWithSelect
										.getSelectData("last." + SELECT_ATTR_IEFFILEMESSAGEDIGEST);
							}
						}
					}
				}
				...
				if (!MCADStringUtils.isNullOrEmpty(integrationSource))
				{
					...
					if (null == outputId_forCallee || true == outputId_forCallee.isEmpty())
					{
						revisionCreatedManually.add(busID);

						outputId_forCallee = busID;

					}

					if (retrieveHashcode)
						outputId_forCallee = outputId_forCallee + "|" + outputIdHashcode;

					returnTable.put(busID, outputId_forCallee);
				} 
				else 
				{
					returnTable.put(busID, busID);
			...
			System.out.println("[MCADIntegGetLatestVersionBase.getLatestForObjectIds]: Exception - " + e.getMessage());
		}

			returnTable.put("buslWithSelectionList", buslWithSelectionList);
			return returnTable;

		}
//Overloaded method for performance improvement-IR-571243/571192
	public Hashtable getLatestForObjectIdsSLWDPM(Context context, String[] oids,
				BusinessObjectWithSelectList buslWithSelectionList) throws Exception {
			Hashtable returnTable = new Hashtable(oids.length);
			HashSet revisionCreatedManually = new HashSet();
		
			try {
				Hashtable busidBusselectMap = new Hashtable();
				returnTable = getMajorIdAndLatestIDMap(context, oids, busidBusselectMap);
			} catch (Exception e) {
				System.out.println("[MCADIntegGetLatestVersionBase.getLatestForObjectIdsSLWDPM]: Error - " + e.getMessage());
			}
