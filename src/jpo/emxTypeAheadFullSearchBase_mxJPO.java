...
import com.matrixone.apps.common.search.TypeAheadNextGen;
...
{ 
	...
	public String getTypeSearchResults(Context context, String[] args) throws Exception
	{
		...
		MapList types = null;
		try{
			types      = UISearchUtil.getTypeAheadSearchTypes(context, charTyped, excludedList, includedList, getLanguage(), deep, bObserveHidden, babstractTypes);
	...
	private void generateXML(Context context, MapList objects, MapList columns)throws Exception
	{
		...
		int noOfObj=Integer.parseInt(savedValuesLimit);
		MapList visibleObjects = new MapList();
		if(objects.size() > noOfObj) {
			searchLimitReached = true;
			for(int i=0;i<noOfObj;i++){
				visibleObjects.add(objects.get(i));
			}
		...
		throws Exception
	{
		...
		//JsonObjectBuilder filtersBuilderClone = FrameworkUtil.getBuilderObjectClone(filters);
        ...
        for(int i = 0; i < typeAheadMappingList.size(); i++)
        {
    	   ...
    	   filters = UISearchUtil.addToFilters(context, filters, typeAheadMappingList.get(i).toString(), charTyped, "Equals");
        }
		...
		String fieldProgramResult 		= getQueryField(context, fieldProgram);	
	    
	    TypeAheadNextGen nextGen=new TypeAheadNextGen(context,rangeUrl,typeAheadMapping,fieldProgramResult,xRequest);
	    results=nextGen.search(context,charTyped);
