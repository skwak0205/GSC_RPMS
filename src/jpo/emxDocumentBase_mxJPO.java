	  ...
	  public String getThumbnailExtention(Context context, String args[]) throws Exception {
		...
		String filename = args[0];
		    if(UIUtil.isNullOrEmpty(filename)) {
		    	...
		    	StringList fileExtList = new StringList();		    	
				...
				StringList fileList = FrameworkUtil.split(filename, SelectConstants.cSelectDelimiter);
				for(String file : fileList) {
					if(UIUtil.isNotNullAndNotEmpty(file)) {
						file = file.substring(file.lastIndexOf(".")+1);
						if(!fileExtList.contains(file)){
							fileExtList.add(file);						
						}
					}
				}
				
				return FrameworkUtil.join(fileExtList, delimeter);
		    }
