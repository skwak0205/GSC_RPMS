...
public class IEFSynchronizeAssignmentsBase_mxJPO
{
    ...
    protected void init (Context context, String languageName, String resetPreferences)throws Exception
    {
		...
		if(!(isCloudEnvironment() || isPODEnvironment()))
		{
    ...
    {
    	System.out.println("Inside  IEFSynchronizeAssignmentsBase.synchAssignmentsForUsers......");
        String sRet = "true";
    ...
    {
    	System.out.println("Inside IEFSynchronizeAssignmentsBase.synchAssignmentsForIndividualUser.....");
        ...
        //if Cloud env or POD env then do not assign harnessUser role. //FUN101015
        if(!(isCloudEnvironment() || isPODEnvironment()))
        {
		 ...
		 *reading the integrations from attribute_IEF-RegistryData instead of generating it from role hierarchy
		 *as there will be no Integration User role on cloud
		*/
		...
		if(isCloudEnvironment() || isPODEnvironment())
		{
			assignedIntegrationsGCONamesMap = (Hashtable)JPO.invoke(context, "IEFGetRegisteredIntegrations", null, "getRegisteredIntegrationsTable", null, Hashtable.class);
		}

		else
			assignedIntegrationsGCONamesMap = util.getAssignedIntegrationsTable(context, personName);
		...
		{
			//IR-865583 : "harnessUser" role has been removed from cloud env. 
			if(!(isCloudEnvironment() || isPODEnvironment()))
	        {
		...
		//IR-865583 : "Designer" role has been removed from cloud env.  
		if(!(isCloudEnvironment() || isPODEnvironment()))
        {
...
/*
		 *FUN101015:Removal of CSE roles from cloud 
		 *
		 *as there will be no "Integration Bulk Loader" role on cloud
		*/
	    boolean isBulkLoaderRoleAssigned = false;
	    if(!(isCloudEnvironment() || isPODEnvironment()))
	    {
	    	 isBulkLoaderRoleAssigned = isRoleAssigned(context, personName, bulkLoaderRoleName);
	    }
        ...
        while(keys.hasMoreElements())
        {
			...
			while (attrItr.next())
			{
				...
				*/
				...
				{
				if((!(isCloudEnvironment() || isPODEnvironment())) && lcoAttrName.equals(bulkLoadingAttrName) && !isBulkLoaderRoleAssigned)
				{
	...
	protected boolean isCloudEnvironment() {
		...
		String cloudENV = System.getenv("DS_IN_CLOUD");
		//System.out.println("--[Inside isCloudEnvironment] -->" + cloudENV); //IR-835161- 	To remove unnecessary SOPs from isCLOUD method.
		if (cloudENV != null && !cloudENV.isEmpty()) {
			isCloud = true;
		}
		return isCloud;
	}

	protected boolean isPODEnvironment() {
		boolean isPod = false;
		String cloudENV = System.getenv("PodDefinitionName");
		//System.out.println("--[Inside isPODEnvironment] -->" + cloudENV); //IR-835161- 	To remove unnecessary SOPs from isCLOUD method.
		if (cloudENV != null && !cloudENV.isEmpty()) {
			isPod = true;
		}
		return isPod;
