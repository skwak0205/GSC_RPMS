...
public class IEFUITableUtility_mxJPO
{
	...
	public MapList renderTable(Context context, String tableName, String language, MapList totalResultList, String hyperlink) throws Exception
	{
		...
		MapList resultMapListLatRev = new MapList(); // L86 for Latest Rev
		...
		else
		{
			...
			else
			{
				...
				StringList sl_lastRev = new StringList();   // L86 for Latest Rev
				...
				//sl_bus.addElement(SELECT_LATEST_REVISION); // L86 
				sl_bus.addElement(ACTIVE_VERSION_ID);
				sl_bus.addElement(IS_DOCUMENTType);
				
				//TSK7928768 return FolderClassification attribute in PLMNavigation response
				String ATTRIBUTE_FOLDER_CLASSIFICATION		  = (String) PropertyUtil.getSchemaProperty(context, "attribute_FolderClassification");
				String SELECT_FOLDER_CLASSIFICATION = "attribute[" + ATTRIBUTE_FOLDER_CLASSIFICATION +"]";
				sl_bus.addElement(SELECT_FOLDER_CLASSIFICATION);		
				//TSK7928768 ----
				...
				BusinessObjectWithSelectList bwslLastRev = null;   // L86 for Latest Rev
				...
				boolean isCDMInstalled = util.isCDMInstalled(context);
				HashMap mapIsVersionable = new HashMap();
				...
				while(resultMapListItr.hasNext())
				{
					...
					else
					{
						...
						if(busId != null && !busId.equals(""))
						{
							...
							/*
							*8-DEC-2020 : IR-815050-3DEXPERIENCER2021x ++++
							*We have improved performance by skipping IEFCDMUtil JPO call.
							*This JPO calls have 4 nested method calls, with many duplicate java object creation.
							*/
							...
							isVersionable = isVersionableObject.booleanValue();*///Commented for IR-815050
							
							
							if (isCDMInstalled) {
								if (!mapIsVersionable.containsKey(busTypeName)) {
									String property = PropertyUtil.getAdminProperty(context, "Type", busTypeName,
											"DISALLOW_VERSIONING");
									if (property != null && "true".equalsIgnoreCase(property)) {
										isVersionable = false;
										mapIsVersionable.put(busTypeName, isVersionable);
									} else {
										isVersionable = true;
										mapIsVersionable.put(busTypeName, isVersionable);
									}
								} else {
									isVersionable = (boolean)mapIsVersionable.get(busTypeName);
								}
							}
							//IR-815050-3DEXPERIENCER2021x ---
						}
						...
						if(isCommonDocumentLike)//[SUPPORT]
						{
							...
							sl_lastRev.addElement(SELECT_LATEST_REVISION);
							
							/*IR-803308-3DEXPERIENCER2021x : below code is already present in loop 
							 * and executing for N times if N no. of Documents are present in folder.
							 * Initially bol_array sent to getSelectBusinessObjectData and bol_array contains busid of N Documents.
							 * Now new array bol_array1(which contains busid of current iterating document) will be sent getSelectBusinessObjectData 
							 */
							String[] bol_array1 =new String[1];
							bol_array1[0] = busId;
							bwslLastRev = BusinessObject.getSelectBusinessObjectData(context, bol_array1, sl_lastRev);
							
							if(bwslLastRev != null)
							{
								resultMapListLatRev = FrameworkUtil.toMapList(bwslLastRev);
							}

							Iterator resultMapListLatRevItr       = resultMapListLatRev.iterator();

							while(resultMapListLatRevItr.hasNext())
							{
								Map sResultMapLatRev     = (Map) resultMapListLatRevItr.next();


							//To show the "file name" column in File Dialog or windows explorer excluding when thable is FileSummary
							...
							{
									String latestRevision = (String) sResultMapLatRev.get(SELECT_LATEST_REVISION);									
								...
								//IR-483991  : multifile icon is shown for a document object if it contains image file (.jpeg, .png)
						...
						else if(objType != null && !objType.equals(""))
						{
							...
							//TSK7928768 return FolderClassification attribute in PLMNavigation response
							finalMap.put("FolderClassification", sResultMap.get(SELECT_FOLDER_CLASSIFICATION));
						}
						else
						{
					...
					if(finalMap != null)
					{
						...
						// Getting language as null in IEFXmlGenerator buildXmlOutput. So to handle null pointer exception language is added in finalMap.
						finalMap.put("language",language);
