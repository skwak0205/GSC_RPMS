	...
	public void executeCustom(Context _context, Hashtable resultAndStatusTable)  throws MCADException
	{
		...
		try
		{
			...
			if(errorMessageTable.errorsOccured()) {
				...
				// IR-736658-3DEXPERIENCER2019x - env variable isCommandDECOrigin is set to false in 
				// ${CLASS:IEFPromoteIfDesignCenterCommand} to remember the origin of command. 
				...
				// we return error in plain text format.
				Args = new String[1];
				Args[0] = "isCommandDECOrigin";
				String isCommandDECOrigin = _util.executeMQL(_context,"get env $1", Args);
				
				if(isCommandDECOrigin.endsWith("false")) 
				{
					MCADServerException.createException(errorMessageTable.getErrorMessageString(), null);
				}
				else 
				{
		...
		catch(Exception exception)
		{
			...
			Args[0] = "isCommandDECOrigin";
			String result2 = _util.executeMQL(_context,"unset env $1", Args);
		}
	}
   ...
   public void getisSelectedIdTable(Hashtable isSelectedIdTable , IEFXmlNode sortedChildNodes)
   {
	...
	public void checkIsParentAheadOfChild(Context _context,Hashtable isSelectedIdTable,IEFXmlNode cadObjectListNode ,DSCServerErrorMessageTable errorMessageTable)
	{
		try
		{
			...
			{	
				...
				String childPolicy = "";
				String childReleasedState = "";
				Hashtable policyStateList = new Hashtable();
				...
				if((MCADAppletServletProtocol.TRUE).equalsIgnoreCase(isSelected) && !alreadyProcessedIds.contains(id))
				{
					...
					StringList childStateList = new StringList();

					//IR-687396
					String policyName		= (String)cadObjectListNode.getAttribute("policyname");
					...
					while(itr.hasNext())
					{
						...
						policyStateList.put(policyName,parentStateList);
					
					//IR-687396
					int relaeseStateIndex = -1;	
					if(null != releasedState)
						relaeseStateIndex = parentStateList.lastIndexOf(releasedState);
					
					if(relaeseStateIndex == -1)
					{
						String errorMessage = _serverResourceBundle.getString("mcadIntegration.Server.Message.IEFReleasedStateAttributeIsNotDefined") ;
						errorMessageTable.addOtherErrorMessage(errorMessage);
					}
					...
					while(allRels.hasMoreElements())
					{
						...
						//IR-738111-3DEXPERIENCER2018x extract child policy and releaseed state
						childPolicy = busToAdd.getPolicy(_context).getName();
						childReleasedState = _globalConfig.getReleasedState(childPolicy);
						
						childStateList = new StringList();
						StateList chStateLst = busToAdd.getStates(_context);
						Iterator itrChild = chStateLst.iterator();
						
						if(!policyStateList.keySet().contains(childPolicy))
						{
							while(itrChild.hasNext())
							{
								State childStateToAdd = (State)itrChild.next();
								childStateList.add(childStateToAdd.getName());
							}
							policyStateList.put(childPolicy,childStateList);
						}
						else
						{
							childStateList = (StringList)policyStateList.get(childPolicy);
						}
						
						int childStateIndex = childStateList.lastIndexOf(childStateName);
						int parentStateIndex = parentStateList.lastIndexOf(parentState.getName());
						busToAdd.close(_context);

						int childReleaseStateIndex = -1;	
						if(null != childReleasedState)
							childReleaseStateIndex = childStateList.lastIndexOf(childReleasedState);
						...
						{
							//IR-687396
							//IR-738111 if the parent policy has more states than child following condition will be executed
							if((isChildSelected.equalsIgnoreCase("false") && (relaeseStateIndex != parentStateIndex && parentStateIndex >= childStateIndex  && childReleaseStateIndex != childStateIndex)) || (isChildSelected.equalsIgnoreCase("true") && (relaeseStateIndex != parentStateIndex && childReleaseStateIndex != childStateIndex && parentStateIndex > childStateIndex)))
							{
								...
								}//IR-738111 if the child policy has more states than parent following condition will be executed
							else if((childStateList.size() > parentStateList.size() &&(relaeseStateIndex-1) == parentStateIndex) &&(childStateIndex<(childReleaseStateIndex-1)))	
							{
									String errorMessage = _serverResourceBundle.getString("mcadIntegration.Server.Message.FirstLevelChildrenReleasedBeforeParent") + ": \"" + busObj.getTypeName() + "\" \"" + busObj.getName()  + "\" \"" + busObj.getRevision() + "\"";
									errorMessageTable.addErrorMessage(_context, busObjIdToAdd,errorMessage );
									errorMessageList.add(errorMessage);
							}
						}
	...
	protected void doPromoteOperations(Context _context, String majorBusObjectId, String objectId, String currentState, String policyName, Hashtable signatureActionTable, Vector handledBusIds, Hashtable resultAndStatusTable, DSCServerErrorMessageTable errorMessageTable) throws Exception
	{
		...
		while (itr.hasMoreElements())
		{
		    ...
		    String result= _util.executeMQL(_context, "get env $1", args1);
			String[] compare = MCADUtil.getTokensSeperatedByFirstDelimiter(result,"|");
				
			if(!majorId.equalsIgnoreCase(compare[1])){
				...
				String validResult		= _util.executeMQL(_context, "get env $1", args1);
				if(!validResult.endsWith("true"))
				{
				String Args[] = new String[1];
				Args[0] = majorId;
...
*/
		    ...
		    String result = _util.executeMQL(_context, "get env $1", args1);
			String[] compare = MCADUtil.getTokensSeperatedByFirstDelimiter(result,"|");
			String busid = majorObject.getObjectId();
			
			args1[0] = "IsDECValidate";
			String validResult		= _util.executeMQL(_context, "get env $1", args1);
			if(!validResult.endsWith("true"))
			{
			if(!busid.equalsIgnoreCase(compare[1])){
		...
		catch(Exception e)
		{
			...
			boolean bObjectAndFileNameDiff = _globalConfig.isObjectAndFileNameDifferent();
			if(bObjectAndFileNameDiff)
				displayName = title;
			
			Hashtable exceptionDetails = new Hashtable();
			exceptionDetails.put("TYPE", type);
			exceptionDetails.put("NAME", displayName);
			exceptionDetails.put("REVISION", revision);
	...
	protected void transferRelationships(Context _context, BusinessObject majorObject, String policyName, DSCServerErrorMessageTable errorMessageTable) throws Exception
	{
		...
		saArgs[0] = majorObject.getObjectId(_context);
		saArgs[1] = "previous";
		String sPrevResult = _util.executeMQL(_context,"print bus $1 select $2 dump", saArgs);
		sPrevResult = sPrevResult.substring(5);
		...
		{          
			...
			{	
				...
				if (command.executeCommand(_context, "print policy $1 select $2 dump", policyName, "state")) {
					String[] strArr = command.getResult().trim().split(",");
					stateList = new StringList(strArr);
				}
				
				State currentState 			= _util.getCurrentState(_context, majorObject);
				String finalizationState 	= _globalConfig.getFinalizationState(policyName);
				String releasedState	 	= _globalConfig.getReleasedState(policyName);
				...
				if(!validResult.endsWith("true"))
				{					
					if(result.equals("")) // If DEC Promote, then Transfer Relationships
					{
						if((stateList.lastIndexOf(currentState.getName()) == stateList.lastIndexOf(releasedState)) && !MCADStringUtils.isNullOrEmpty(sPrevResult))
						{	
					...
					else{ // If NON DEC Promote, then Transfer Relationships
					
						if(((stateList.lastIndexOf(currentState.getName()) == stateList.lastIndexOf(releasedState)) || (stateList.lastIndexOf(currentState.getName()) == stateList.lastIndexOf(finalizationState))) && !MCADStringUtils.isNullOrEmpty(sPrevResult))
						{	
							transferPartSpecRelAndAttributes(_context,majorObject);
							_generalUtil.shuffleAssemblyLikeRelatinships(_context, previousObject, majorObject, false, false, MCADServerSettings.ASSEMBLY_LIKE);
							_generalUtil.shuffleRelatinships(_context, previousObject, majorObject, false, false, MCADServerSettings.EXTERNAL_REFERENCE_LIKE);
