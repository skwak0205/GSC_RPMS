...
public class MCADRenameBase_mxJPO extends IEFCommonUIActions_mxJPO
{
	...
	public void executeCustom(Context _context, Hashtable resultAndStatusTable)  throws MCADException
	{
		...
		try
		{
			...
			String oldBusName = (String)_argumentsTable.get(MCADServerSettings.OLD_NAME);
			String oldBusTitle              = (String)_argumentsTable.get(MCADServerSettings.OLD_TITLE);
			String familyName = _busObject.getName();
			String familyTitle = MCADUtil.getNameWithoutExtension(_busObject.getAttributeValues(_context,attrTitle).getValue());
			if(oldBusTitle == null || oldBusTitle.equals(""))
				oldBusTitle = _busObject.getName();
			...
			resultAndStatusTable.put(MCADServerSettings.OLD_FAMILY_TITLE, familyTitle);//IR-726882-3DEXPERIENCER2019x and IR-726935-3DEXPERIENCER2019x
			StringBuffer renamedObjIds = new StringBuffer();
			// Renaming a SLW or MxProE Object
			if (null!=_integName &&(_integName.equalsIgnoreCase("MxPro") || _integName.equalsIgnoreCase("SolidWorks")))
			{
				if(_globalConfig.isTypeOfClass(cadType, MCADAppletServletProtocol.TYPE_INSTANCE_LIKE))
				{
					String oldProeInstanceName  = "";
					...
					familyTitle = MCADUtil.getNameWithoutExtension(familyObject.getAttributeValues(_context,attrTitle).getValue()); 
					resultAndStatusTable.put(MCADServerSettings.OLD_FAMILY_TITLE, familyTitle); //For instance like objects, title value changes to instance title //IR-726882-3DEXPERIENCER2019x and IR-726935-3DEXPERIENCER2019x
					if(null!=familyTitle && false==familyTitle.isEmpty() && null!=familyName && false==familyName.isEmpty()) {
						oldProeInstanceName  =(String)_generalUtil.getIndivisualInstanceName(oldBusName,oldBusTitle,familyName, familyTitle).get(1);//IR-578862
					}
					if (oldProeInstanceName.equalsIgnoreCase(familyName))
					{
						_busObject = familyObject;
						oldBusTitle	= familyName;
						cadType	= _util.getCADTypeForBO(_context, familyObject);
					}
					//System.out.println("----------------Family name from rename jpo with integ==Solidworks----------------"+familyName+"---familyTitle----"+familyTitle);
					familyObject.close(_context);
				}
			}
			resultAndStatusTable.put(MCADServerSettings.OLD_FAMILY_NAME, familyName);
			...
			String newTitle = "";
			String familyNameFromTitle = null;
			if(instanceRename)
			{
				if(isObjectAndFilenameDifferent)
				{

					familyNameFromTitle = familyTitle;//_generalUtil.getFamilyNameFromObjectName(oldTitle,true); take name from family title if isObjectandFileNamedifferent = true
					
					newTitle = MCADUtil.getDisplayNameForInstance(newName, familyNameFromTitle);
				}
				else
				{
					familyNameFromTitle = familyName;//_generalUtil.getFamilyNameFromObjectName(oldTitle);

					newTitle = MCADUtil.getNameForInstance(_globalConfig, newName,familyNameFromTitle);
				}		
			}		
			...
			while(objItr.next())
			{
				...
				if(instanceRename)
				{
					...
					String actualOldBusName	= (String)_generalUtil.getIndivisualInstanceName(currentBusOldName,"", familyName,familyTitle).get(0);

					String actualNewName	= _generalUtil.getNameForInstance(familyNameFromTitle,newName); //getNewNameForInstanceRename(newName, currentBusOldName);
					...
					if(isObjectAndFilenameDifferent)
					{
						//String familyTitle			 = familyName;//_generalUtil.getFamilyNameFromObjectName(oldTitle,true);
						
						String instanceNameFromTitle = (String)_generalUtil.getIndivisualInstanceName("",oldTitle,familyName,familyTitle).get(1);//Family title is needed here
						
						String indivisualInstanceName = (String)_generalUtil.getIndivisualInstanceName(currentBusOldName,"", familyName, familyTitle).get(0);//Family Name is needed here
						
						actualNewName				 = indivisualInstanceName.replace(instanceNameFromTitle, newName);

						actualNewName				 = _generalUtil.getNameForInstance(familyName, actualNewName);
						//actualNewName				 = _generalUtil.getNameForInstance(familyTitle,newName);

						/*actualNewName				 = indivisualInstanceName.replace(instanceNameFromTitle,oldBusName);

						actualNewName				 = _generalUtil.getNameForInstance(familyTitle,oldBusName);*/

						//getNewNameForInstanceRename(actualNewName, familyTitle);
					...
					if(!newName.equals(actualOldBusName))
					{
						...
						renameDependentDocs(_context, renamedObj, currentBusOldName, actualNewName, oldBusName, newNameForFileOperation, instanceName.trim(), renamedObjIds);
						...
						if(attrList!=null)
						{
							for(int b=0 ; b < attrList.size(); b++)
							{
								...
								if(attrParentInst.equals(actualOldBusName))
								{
									...
									String indivInstName		= (String)_generalUtil.getIndivisualInstanceName(actualNewName,"", familyName, familyTitle).get(0);//IR-578862
				...
				else
				{
					...
					if(isObjectAndFilenameDifferent && oldTitle.contains(".") && newName.contains(".") && oldTitle.substring(0,oldTitle.lastIndexOf(".")).equals(currentBusOldName))
						...
						actualNewName = _util.replace(currentBusOldName, oldBusTitle, newName);
					...
					{
						String oldFamilyNameFromInstanceName = familyName;//_generalUtil.getFamilyNameFromObjectName(currentBusOldName);
						...
						if(isObjectAndFilenameDifferent &&  oldTitle.contains(".") && newName.contains(".") && oldTitle.substring(0,oldTitle.lastIndexOf(".")).equals(oldFamilyNameFromInstanceName))
							...
							newFamilyBusName = _util.replace(oldFamilyNameFromInstanceName, oldBusTitle, newName);
						...
						{
							String proeInstanceName = (String)_generalUtil.getIndivisualInstanceName(currentBusOldName,"", newFamilyBusName, "").get(0);//IR-578862
							if (proeInstanceName.equalsIgnoreCase(oldBusTitle))
							{
						...
						actualNewName = getNewNameForFamilyRename(familyName, currentBusOldName, isProeGenericInstance,newFamilyBusName);
						...
						//rename dependent docs if any
						renameDependentDocs(_context, renamedObj, currentBusOldName, actualNewName, oldBusTitle, newName, instanceName.trim(), renamedObjIds);
					}
					else if(!oldBusTitle.equals(newName))
					{
						isRenamed	= true;
						...
						if(!actualNewName.equals(currentBusOldName))
						{
							...
							if(!alreadyRenamedObjList.contains(busObject.getObjectId()))
						{
							//IR-726882-3DEXPERIENCER2019x and IR-726935-3DEXPERIENCER2019x
							/*if(null!= _globalConfig && null!=_globalConfig.getNamingStrategy() && ""!=_globalConfig.getNamingStrategy() && !_globalConfig.getNamingStrategy().equalsIgnoreCase("AlwaysNumber")) {
							renamedObj = _util.renameObject(_context, busObject, actualNewName);
							}*/
							//IR-756699
								if(null!= _globalConfig){
									if( null==_globalConfig.getNamingStrategy() || "".equalsIgnoreCase(_globalConfig.getNamingStrategy())){
										renamedObj = _util.renameObject(_context, busObject, actualNewName);
									}else if(!_globalConfig.getNamingStrategy().equalsIgnoreCase("AlwaysNumber")) {
										renamedObj = _util.renameObject(_context, busObject, actualNewName);
									}
								}
							alreadyRenamedObjList.add(busObject.getObjectId());
					...
					//get actual server side operations for rename done
					// Set file source attribute for Rename operation
					_generalUtil.doActualRename(_context, renamedObj, actualNewName, actualOldBusName, newName, oldBusTitle, renamedFromRqdOnFamilyLike, true);
				}
				...
				if(isRenamed)
				{
					...
					renamedObjIds.append(oldBusTitle);
	 ...
	 */
	protected String getNewNameForFamilyRename(String newFamilyBusName, String oldInstanceBusName, boolean isProeGenericInstance, String newFamilyNameForRenaming)
	{
		String individualInstName =(String)_generalUtil.getIndivisualInstanceName(oldInstanceBusName,"", newFamilyBusName, "").get(0);
		
		
		String newNameForInstance = _generalUtil.getNameForInstance(newFamilyNameForRenaming, individualInstName);
		

		if (isProeGenericInstance)
		{			
			newNameForInstance = _generalUtil.getNameForInstance(newFamilyNameForRenaming, newFamilyNameForRenaming);
