	...
	private void updateProjectTemplate(Context context, StringList ids) throws FrameworkException{
		...
		try{
			...
			for (Iterator iterator = ids.iterator(); iterator.hasNext();) {
				...
				singleSelects.add(DomainObject.SELECT_REVISION);
				StringList multiSelects = new StringList();

				//Migration for "Project Template" Lifecycle changes.
				DomainObject doObj = DomainObject.newInstance(context, physicalId);
				MapList mlRevs = doObj.getRevisionsInfo(context, singleSelects, multiSelects);

				try {
					if(!mlRevs.isEmpty()){
						int size = mlRevs.size();
						Map templateMap = (Map)mlRevs.get(0);
						//if 1st revision is in 'Inactive' state >>Draft
						if(size==1 && "Obsolete".equalsIgnoreCase((String)templateMap.get(DomainObject.SELECT_CURRENT))){
							String strOID = (String)templateMap.get(DomainObject.SELECT_ID);
							String strCurrent = (String)templateMap.get(DomainObject.SELECT_CURRENT);
							// Demote to Draft
							for(int i=0; i<3; i++) {
								String sCommandStatement = "demote bus $1";
								MqlUtil.mqlCommand(context, sCommandStatement, strOID);
							}
							loadMigratedOids(objectId);
						}else if(size>1){
							//If 2nd last is Active and last rev is 'Inactive', Last rev state will be Draft
							mlRevs.sortStructure(DomainObject.SELECT_REVISION,  "descending","integer");
							Map latestRevMap = (Map)mlRevs.get(0);
							Map secLastRevMap = (Map)mlRevs.get(1);
							if("Obsolete".equalsIgnoreCase((String)latestRevMap.get(DomainObject.SELECT_CURRENT)) 
									&& "Released".equalsIgnoreCase((String)secLastRevMap.get(DomainObject.SELECT_CURRENT)) ) {
								String strOID = (String)latestRevMap.get(DomainObject.SELECT_ID);
								// Demote to Draft
								for(int i=0; i<3; i++) {
									String sCommandStatement = "demote bus $1";
									MqlUtil.mqlCommand(context, sCommandStatement, strOID);
		...
		try{
			...
			while(objectInfoListIterator.hasNext())
			{
				...
				if("None".equals(defaultUserAccess)){
					mqlLogRequiredInformationWriter("Modify value of Attribute Default User Access from None to Read for folder " + objectId +"\n\n");
					defaultUserAccess = "Reader"; // range value  of "Default User Access" attribute is changed from Read to Reader in 21x.HF1 by BPS
