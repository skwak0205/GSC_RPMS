	...
	public Hashtable returnObsoleteInfoForObjectIds(Context context, String[] args) throws Exception
	{
		...
		try
		{
			...
			Set majorIds						 = new HashSet(inputInfoValue.size());
                        Hashtable partialMajorIdInfoTable = new Hashtable();

			while(inputInfoItr.hasNext())
			{
				Vector inputInfo 		= (Vector) inputInfoItr.next();
				String inputMajorBusID	= (String)inputInfo.elementAt(1);
				if(isCheckObsoleteAcrossRevision.equalsIgnoreCase("false") && inputInfo.size() == 12){
					Vector infoVector = new Vector(5);
					infoVector.addElement((String)inputInfo.elementAt(8));
					infoVector.addElement(inputMajorBusID);
					infoVector.addElement((String)inputInfo.elementAt(9));
					infoVector.addElement((String)inputInfo.elementAt(10));
					infoVector.addElement((String)inputInfo.elementAt(11));
					partialMajorIdInfoTable.put(inputMajorBusID, infoVector);
				}else{
			...
			Hashtable majorIdInfoTable = getInfoFromMajorids(context, majorIds, isCheckObsoleteAcrossRevision);
                        majorIdInfoTable.putAll(partialMajorIdInfoTable);
