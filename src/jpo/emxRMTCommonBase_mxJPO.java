...
/*
  ...
  28-OCT-16 KIE1 ZUD  IR-466628-3DEXPERIENCER2018x:V6R2017x-After creating a Customized Requirement Object on web side, the progress circle doesn't stops.
  14-DEC-16 KIE1 ZUD  IR-478678-3DEXPERIENCER2018x: R419-FUN058633-POD: Disabling hidden inherited Requirement types is KO on Cloud.
  01-FEB-17 ZUD       : IR-495987-3DEXPERIENCER2018x Requirements : PLMParameter Value is not correct after changing it from 1.0 to 2.0
  21-FEB-17 KIE1 ZUD  IR-502482-3DEXPERIENCER2018x: Requirement: Parameter - After modifying the parameter value, original value in red color with strike-through is not displayed.
  07-MAR-17 HAT1 ZUD  :IR-506738-3DEXPERIENCER2018x: R419-STP: Test Execution is not counted in Requirement status column.
  09-MAR-17 KIE1 ZUD :IR-478678-3DEXPERIENCER2018x: R419-FUN058633-POD: Disabling hidden inherited Requirement types is KO on Cloud.
  20-MAR-17 KIE1 ZUD  :IR-508770-3DEXPERIENCER2018x: In tree structure, PLM Parameter can't show same value as input value for Boolean Dimension.

  30-MAR-17 HAT1 ZUD  :TSK3433119: ENOVIA GOV TRM Modify Title on just created object in "Req Struct Editor" widget
  19-APR-17 HAT1 ZUD  : TSK3278161 - R419: Deprecation of functionalities to clean up.
  ...
  18-JUL-17 HAT1 ZUD  :IR-536557-3DEXPERIENCER2018x: R420-FUN067990:"Content Report" Command is Not Removed from Contextual menus of Requirement Specification on list View and in Cloning information Tab.
  24-NOV-17 HAT1 ZUD  :IR-555094-3DEXPERIENCER2018x: Create a ReqSpec with a custom policy.
  01-MAR-18 KIE1 ZUD  :IR-581573-3DEXPERIENCER2018x: problem on the validation column display with IE11 not in US language
  01-MAR-18 KIE1 ZUD  :IR-581573-3DEXPERIENCER2018x: problem on the validation column display with IE11 not in US language
  20-MAR-18 KIE1 ZUD  :IR-586733-3DEXPERIENCER2018x:Validation column in RSP structure does not show scheduled Test Execution if estimated start date for Test Execution is set to todays date.
  20-MAR-18 KIE1 ZUD  :IR-588751-3DEXPERIENCER2018x: Validation column for a requirement shows "No test case associated" even though there is a test case
  20-MAR-18 KIE1 ZUD  :IR-580732-3DEXPERIENCER2018x: validation column of the Test Case wrongly at failed
  09-APR-21 GVC ZUD   :IR-809936 : Unexpected error message during Comment creation when ownership inheritance is deactivated.
  05-Jul-21 VAI1 ZUD  :IR-859720-Receiving requirement email notifications even when it is turned off.
  20-APR-22 LVA ZUD   :IR-935003: propagate access/inherit access applied to the object being revised instead of the new revision
*/
...
import com.dassault_systemes.requirements.ReqServices;
...
// End:R207:RMT Bug 357905
  ...
  */
  ...
  /*  public static com.matrixone.jdom.Document getObjectMailXML(Context context, Map info) throws Exception
      ...
      return (${CLASS:emxSubscriptionUtil}.prepareMailXML(context, headerInfo, null, footerInfo));
      }
  */
    ...
    throws Exception
    {
      ...
      try
      {
        ...
        if (lstSpecTypes.contains(thisObjType))
        {
          ...
          //${CLASS:emxNotificationUtilBase}.objectNotification(context, strObjectId, "RMTSpecStructureModifiedEvent", null); // VAI1 : IR-859720
          return;
        }
        ...
        for(int i=0; i<mapParentObjects.size(); i++)
        {
          ...
          if (lstSpecTypes.contains(strObjType))
          {
            ...
            //${CLASS:emxNotificationUtilBase}.objectNotification(context, strObjId, "RMTSpecStructureModifiedEvent", null); // VAI1 : IR-859720
          }
        }
  ...
  public List getHigherRevisionIcon(Context context, String[] args) throws Exception
    {
      ...
      final String SELECT_MAJORID = "majorid";
      final String SELECT_LASTMAJORID = "majorid.lastmajorid";
      ...
      objSelect.add(SELECT_MAJORID);
      objSelect.add(SELECT_LASTMAJORID);
      MapList objList;
      ...
      try
      {
        ...
        objList = DomainObject.getInfo(context, arrObjId, objSelect);
      }
      finally
      {
        if(isContextPushed)
      ...
      strHigherRevisionIconTag = "<img src=\"../common/images/" + strIcon + "\" border=\"0\"  align=\"middle\" "
        + "TITLE=\"" + " " + strTooltipHigherRevExists + "\"" + "/>";
      //Iterating through the list of objects to generate the program HTML output for each object in the table
      for (iCount = 0; iCount < iNumOfObjects; iCount++) {
        Map objMap = (Map)objList.get(iCount);
        if(!objMap.get(SELECT_MAJORID).equals(objMap.get(SELECT_LASTMAJORID))){
          lstHigherRevExists.add(strHigherRevisionIconTag);
        }else{
          lstHigherRevExists.add(" ");
        }
      }
...
//long start = System.currentTimeMillis();
      ...
      try
      {
        ...
        for(int i=0; i<objectList.size();i++)
        {
          ...
          //PLA3: against IR-793514-3DEXPERIENCER2021x
          if(isKindOfRequirement && relType!=null && relType.equals(ReqSchemaUtil.getSubRequirementRelationship(context)))
          {
      ...
      try {
        if (rel != null && rel.length() > 0 )
        {
          ...
          if (relRegistered != null && relRegistered.length() > 0 )
          {
            ...
            try {
              if(busType.equals("Requirement Proxy")) {
                if(relRegistered.equals("relationship_DerivedRequirement")) {
                  icon = "iconReqTypeDerivedRequirementProxy.png";
                  return icon;
                }
                else if(relRegistered.equals("relationship_RequirementBreakdown")) {
                  icon = "iconReqTypeSubRequirementProxy.png";
                  return icon;
                }
              }
...
//START LX6
  ...
  public static String fieldLifecycle(Context context, String[] args) throws Exception {
    ...
    String sResult = "<iframe id='gnvLifecycle' src ='emxLifecycleDialog.jsp?export=false&toolbar=AEFLifecycleMenuToolBar&objectId=" + sOID + "&header=emxFramework.Lifecycle.LifeCyclePageHeading&mode=basic' width='100%' height='100%' frameborder='0' border='0' style='position:static;'></iframe>";
    return sResult;
  ...
  public List getColumnHTML(Context context, String[] args) throws Exception {
    ...
    for (iCount = 0; iCount < iNumOfObjects; iCount++) {
      ...
      String strHTMLTag = "";
      // ++ HAT1 ZUD: OSLC table column blank ++
      if(policy.equalsIgnoreCase("ProxyItem") && (attributeName.equalsIgnoreCase("Difficulty") || attributeName.equalsIgnoreCase("Priority")))
      {
        strHTMLTag = "-";
      }
      // -- HAT1 ZUD: OSLC table column blank  --
  ...
  public int updateProgramAttribute(Context context, String[] args, boolean isBus, boolean isColumn) throws Exception
    {
      ...
      if(isBus){
        ...
        //++ IR-620577-3DEXPERIENCER2018x fix ++
        if(!isBO
           &&"Link Status".equalsIgnoreCase(attributeName)
           && "relationship_DerivedRequirement".equalsIgnoreCase((String)requestMap.get("reportRelationships")))
        {
          String[] args1 = new String[3];
          args1[0] = strRelId;
          args1[1] = attributeName;
          args1[2] = newValue;

          JPO.invoke(context, "emxRMTConnectBase", null, "modifyLinkStatusValue", JPO.packArgs(args1),null);
        }
        //-- IR-620577-3DEXPERIENCER2018x fix --
  ...
  public List getHigherAndActualRevisionIcon(Context context, String[] args) throws Exception
    {
      ...
      for (iCount = 0; iCount < iNumOfObjects; iCount++) {
        ...
        if(!id.equals(m.get("majorid.lastmajorid")) && exportFormat==null){
          lstHigherRevExists.add("<span>"+Revision+"&#160;</span>" + strHigherRevisionIconTag);
        }else{
  ...
  public String getParameterContentValue(Context context, String[] args ) throws Exception
    {
      ...
      {
        if( nullCount > 1) {
          strParameterText = strParameterText.replaceAll("null","").replaceAll("<","").trim();
        } else {
          strParameterText = strParameterText.replaceAll("null",".");
        }
  ...
  /** New getparameterContentValueList
   * work same as getParameterContentValue() method
   * Concatenated parameter object list send
   * @return XML string having decorated HTML for each individual object*/
  public String getParameterContentValueList(Context context, String[] args ) throws Exception {
    String xml = "<rows>";
    String[] objectList = ((String) JPO.unpackArgs(args)).split(",");
    int objectListLength = objectList.length;
    for(int k=0;k < objectListLength;k++) {
      String strObjectIDtoExport = objectList[k];
      String objectID = "";
      String toExport = "";
      if(strObjectIDtoExport.contains("|"))
      {
      ...
      if(toExport.equalsIgnoreCase("true"))
      {
        xml += "<r o=\""+objectID+"\">"+strParameterText+"</r>";
      }
      else {
        String decoratedParam = getParameterDivDecorator(context,objectID,strParameterText, valuationType);
        xml += "<r o=\""+objectID+"\">"+decoratedParam+"</r>";
      }
  ...
  public String setParameterString(Context context, String[] args ) throws Exception
    {
      try
      {
        ...
        if(displayUnit.trim().length() > 0 && !displayUnit.isEmpty())
        {
          ...
          // ++ ZUD IR-495987 New Code for converting to MKS Unit ++
        ...
        if(paraType.equals(ParameterTypes.Boolean)){
          ...
          {
            stringValue +=Param_items[i];
            if(stringValue.length()>0 && i < Param_items.length - 2)
            {
              stringValue +="<";
            }
          }
  ...
  public String getParameterDivDecorator(Context context, String objectID , String Value, PLMParm_ValuationType valuationType) throws Exception
    {
      ...
      String parameterEdit = EnoviaResourceBundle.getProperty(context, "emxRequirementsStringResource", context.getLocale(), "emxRequirements.SBParameter.ParameterEdit");
      ...
      DivDecoratedText += "<div title=' "+parameterEdit+" : " + strParameterTitile +"| PlmParameter' style='display:none;' id= 'ParameterEditor_" + objectID.replace(".", "") + "'> " ;
      DivDecoratedText += "</div>";
      DivDecoratedText +="<div id='Values_"+ objectID.replace(".", "") +"'>"+UIRTEUtil.htmlEncode(context, Value)+"</div>";
      DivDecoratedText +="<div id='Unit_"+ objectID.replace(".", "") +"'>"+Unit+""+displayUnit+"</div>";
      DivDecoratedText +="<div id='paraType_"+ objectID.replace(".", "") +"'>"+paraType+"</div>";
      DivDecoratedText += "</div>";
      ...
      { // always show boolean value in upper case
        ...
        if(paraType.equals(ParameterTypes.String)){
          ...
          for(String str : multiVal){
            // KIE1 added for IR-692394-3DEXPERIENCER2020x
            str = XSSUtil.encodeForHTMLAttribute(context,str);
      ...
      DivDecoratedText +="<span id='previousValue_"+ objectID.replace(".", "")+"' style='color:red;text-decoration:line-through'>";
        ...
        else
        {
          if(isParameter){
            ...
            returnVector.add("<img style='display: block; margin-left: auto; margin-right: auto;' "
                             + "src='images/loading.gif' class='parameterPlaceHolder' data-objectId='" + objectID + "'"
  ...
  public Vector getValidationColumnRTF(Context context, String[] args) throws Exception
    {
      ...
      String TC_NO_TE_VALIDATED    = EnoviaResourceBundle.getProperty(context, "emxRequirementsStringResource", context.getLocale(), "emxRequirements.TestCaseValidationColumn.ToolTip.NoTEValidated");
      ...
      if(doExpand){
        ...
        for (int ii = 0; ii < structure.size(); ii++) {
          ...
          if(ReqSchemaUtil.getTestCaseType(context).equals(objMap.get("kindof"))){
            ...
            strValidationStatus     = (String) JPO.invoke(context,
            ...
            if(strValidationStatus.equalsIgnoreCase("<i>Not Validated</i>"))
            {
              counts[1] += 1;
              names[1].add((String)objMap.get(DomainConstants.SELECT_NAME));
            }
            else if(strValidationStatus.equals("<i>Validation Failed</i>"))
            {
              counts[2] += 1;
              names[2].add((String)objMap.get(DomainConstants.SELECT_NAME));
            }
            else if(strValidationStatus.equals("<i>Validation Passed</i>"))
            {
              counts[3] += 1;
          ...
          if(kind.equals(ReqSchemaUtil.getTestCaseType(context)))
          {
            ...
            if(TestCaseTECountMap.get(objectID) != 0)
            {
              ...
              if(lastTestExecutionObjMap != null) // && isActualEndDateTE == -1
              {
                ...
                String strValidationStatus    = (String) JPO.invoke(context,
                                                                    "emxPLCCommonBase", null, "lastCompletedTestExecutionStatus", JPO.packArgs(mapForArgs1),
                                                                    String.class);

                switch(strValidationStatus)
                {
                case "<i>Not Validated</i>":
                  strTestCaseValidationColumnStatus = EnoviaResourceBundle.getProperty(context, "emxRequirementsStringResource", context.getLocale(), "emxRequirements.TestCaseValidationColumn.TestCaseStatus.NotValidated");
                  TestCaseValidationColumn ="<table><tr>"
                    +"<td width = '45px' style = 'background-color:yellow;' title = \"" + TC_NO_TE_VALIDATED +"\">"
                    + "<a onClick ='testExecutionPopUp(\"" + lastTestExecutionObjID + "\", \"" + lastTestExecutionObjRelID + "\",\"" + objectID + "\")' >" + strTestCaseValidationColumnStatus + "</a>"
                    + "</td>";

                  toExportTestCaseValidationColumn = strTestCaseValidationColumnStatus;
                  break;
                case "<i>Validation Failed</i>":
                  strTestCaseValidationColumnStatus = EnoviaResourceBundle.getProperty(context, "emxRequirementsStringResource", context.getLocale(), "emxRequirements.TestCaseValidationColumn.TestCaseStatus.Failed");
                  TestCaseValidationColumn ="<table><tr>"
                  ...
                  toExportTestCaseValidationColumn = strTestCaseValidationColumnStatus;
                  break;
                case "<i>Validation Passed</i>":
                  strTestCaseValidationColumnStatus = EnoviaResourceBundle.getProperty(context, "emxRequirementsStringResource", context.getLocale(), "emxRequirements.TestCaseValidationColumn.TestCaseStatus.Passed");
                  TestCaseValidationColumn ="<table><tr>"
              ...
              if(nextScheduledTestExecutionObjMap != null)
              {
                ...
                TestCaseValidationColumn += "<td>" + " / "+ "</td>"
                  ...
                  + "<a onClick ='testExecutionPopUp(\"" + nextTestExecutionObjID + "\", \"" + nextTestExecutionObjRelID + "\",\"" + objectID + "\")' >" + strEarliestEstimatedStartDateTE + "</a>"
                  +   "</td>"
                  + "</tr></table>";
                // HAT1 : ZUD IR-242335V6R2015 fix
  ...
  public Map NextTestExecutionScheduled(Context context, MapList TestCaseTestExecutionList)
    {
      ...
      if(TestCaseTestExecutionList.size() != 0)
      {
        ...
        while(testExecutionItr.hasNext())
        {
          ...
          // Firts TE with ESD (time) same as current Date
          if(isAfterCurrentDate == 0 && isAnyNextTEfound == 0 && isBeforeCurrEstimatedStartDate == 0)
          {
            isAnyNextTEfound = 1;
            nextTestExecutionObjMap = currTestExecutionObjMap;
            dateNextEstimatedStartDateTE = currEstimatedStartDateTE;
          }
          // TE with ESD (time) is earlier than last found TE ESD (time) on same date
          if(isAfterCurrentDate == 0 && isAnyNextTEfound == 1 && isBeforeCurrEstimatedStartDate == -1)
          {
            nextTestExecutionObjMap = currTestExecutionObjMap;
            dateNextEstimatedStartDateTE = currEstimatedStartDateTE;
          }
          // Firts TE with ESD (time) with future date
          ...
          // TE with ESD (time) is earlier than last found TE ESD (time)
      ...
      if(notValidatedCount + validationPassedCount + notFulValidatedCount + validationFailedCount != 0)
      {
        if(isExport){
          ...
          strReqSpecTestCasesStatusCount ="<table>"
            ...
            +                   (witLink ? "<a onClick = 'testCaseNotValidatedList(this, \"" + names[0] + "\")'> "  : "<b>")+"<b>" + notValidatedCount +"</b>"
            +                   (witLink ? "</a>" : "</b>") + "</td>"
            + "<td>"+ "  /  " + "</td>"
            +                             "<td  style = 'background-color:yellow;'  title = \"" + notFulValidatedCount + "\u00A0\u00A0" + NLS[1] + "\">"
            +                   (witLink ? "<a onClick ='testCaseNotFulValidatedList(this, \"" + names[1] + "\")' >" : "<b>")+"<b>" + notFulValidatedCount +"</b>"
            +                   (witLink ? "</a>" : "</b>") + "</td>"
            + "<td>"+ "  /  " + "</td>"
            +                             "<td  style = 'background-color:red;' title = \"" + validationFailedCount +  "\u00A0\u00A0" + NLS[2] + "\">"
            +                   (witLink ? "<a onClick ='testCaseValidationFailedList(this, \"" + names[2] + "\")' >" : "<b>")+"<b>" + validationFailedCount +"</b>"
            +                   (witLink ? "</a>" : "</b>") + "</td>"
            + "<td>"+ "  /  " + "</td>"
            +                             "<td  style = 'background-color:#39FF14;'  title = \"" + validationPassedCount + "\u00A0\u00A0" + NLS[3] + "\">"
            +                   (witLink ? "<a onClick ='testCaseValidationPassedList(this, \"" + names[3] + "\")' >" : "<b>")+"<b>" + validationPassedCount +"</b>"
            +                   (witLink ? "</a>" : "</b>") + "</td>"
            +                         "</tr>"
            +                    "</table>";
        }
  ...
  public static String getTreeDisplaySettings(Context context, String[] args)
    {
      ...
      try {
        ...
        // ++ HAT1 ZUD: TSK3433119 ++
        JsonObject jsonObj = JsonUtil.buildJsonObj(treeDisplaySettings);

        String treeDisplayName         = jsonObj.getString("treeDisplayName");
        String treeDisplayTitle        = jsonObj.getString("treeDisplayTitle");
        String treeDisplayRevision     = jsonObj.getString("treeDisplayRevision");
        String treeDisplaySeperation   = jsonObj.getString("treeDisplaySeperation");
        String treeDisplayTitleMaxSize = jsonObj.getString("treeDisplayTitleMaxSize");
        // -- HAT1 ZUD: TSK3433119 --
  ...
  public StringList getExistingDependTargetReqSpecs(Context context, String[] args)
    {
      ...
      try {
        ...
        for(int i=0;i<relatedReqSpecMapList.size();i++)
        {
          ...
          lDependentReqSpec.add( (String)Object.get("id"));
        }
...
// -- HAT1 ZUD: IR-363246-3DEXPERIENCER2016x fix
  ...
  public String updateRichTextContent(Context context, String[] args) throws Exception {
    ...
    {
      newValue = new String(Base64.getMimeDecoder().decode(newValue), StandardCharsets.UTF_8); //IR-556378-3DEXPERIENCER2018x
      ...
      if (!dmoObject.getAttributeValue(context, "Content Type").equals(formatToSave) && ReqSchemaUtil.isRequirement(context, strObjectId)) {
        ...
        if (formatToSave.equals("html") && paramMap.get("isNewObject") == null) {
          ...
          if (reqContentData.length() != 0) {
            ...
            // Check-in
            try(ByteArrayInputStream bInputStream = new ByteArrayInputStream(Base64.getMimeDecoder().decode(dmoObject.getAttributeValue(context, "Content Data")));
      ...
      String contentText = new String(Base64.getMimeDecoder().decode(newValue.substring(limitDataRichText + 1)), StandardCharsets.UTF_8);
...
//                "$('.field').dblclick(function() { " +
  ...
  /*
    ...
    ByteArrayInputStream bInputStream = new ByteArrayInputStream(Base64.getMimeDecoder().decode(richTextContent));
    GZIPInputStream gInputStream = new GZIPInputStream(bInputStream);
  ...
  */
  ...
  StringList getColumnAttributeTypes(Context context, MapList objectList, Map columnMap) throws Exception {
    ...
    if (attrind >= 0) {
      ...
      for (int i = 0; i < iNumOfObjects; i++) {
        attrTypeList.add((String)((Map) attrTypes.get(i)).get(select));
      }
...
// ++ KIE1 ZUD LA functionality for TRM Adoption Of ECM
  ...
  public Object getFirstRevision(Context context, String[] args) throws Exception {
    ...
    if(strSymbolicPolicy == null) {
      strSymbolicPolicy = (String)requestMap.get("Policy");
    }
    strPolicy=strSymbolicPolicy;

    if(strSymbolicPolicy != null && strSymbolicPolicy.startsWith("policy_"))
      strPolicy = PropertyUtil.getSchemaProperty(context, strSymbolicPolicy);

    if(strPolicy == null || strPolicy.isEmpty()) //Policy not found in requestMap.
    {
      //Finding type
      ...
      String strType = "";
      strType = strSymbolicType;
      if(strSymbolicType.contains("type_"))
        strType = PropertyUtil.getSchemaProperty(context, strSymbolicType);

      strType = strType.replace(" ", "");

      // retrieve the Custom default policy as defined in emxRequirements.properties file
      String symbolic_policy = ReqServices.getCustomDefaultPolicy(context, strType);
      if(null != symbolic_policy && !symbolic_policy.isEmpty())
      {
        strPolicy = ReqSchemaUtil.getSchemaProperty(context, symbolic_policy);
      }
      else //Default policy.
      {
        DomainObject obj = new DomainObject();
        // if we weren't able to retrieve the policy from properties
        // use the default policy provided by domain object (first in the sequence)
        strPolicy = obj.getDefaultPolicy(context, strType);
      }
      //No Policy in found in requestMap, CustomDefaultPolicy & DefaultPolicy.
...
// Added function for showing Active Change Order/Action On structure browser of Requirement Specification & Requirement
  ...
  public boolean hideColumnOfStructureBrowser(Context context, String[] args)throws Exception
    {
      ...
      //-------IR-820676-3DEXPERIENCER2020x-------------------------------------------
      String customFreezePane = "";
      String customFreezePaneList[];
      //default freezePane = Name,Title,DUP_Title,Revision
      if(tableName.contains("~"))
      {String strPrintMQL  = "print  table $1 select $2";
        customFreezePane = MqlUtil.mqlCommand(context, strPrintMQL, tableName, "property[customFreezePane].value");
        String customFreezePaneValue[] = customFreezePane.split(" = ");
        if(customFreezePaneValue.length>1)
          customFreezePane = customFreezePaneValue[1];
      }
      //--------------------------------------------------------------------------------
      ...
      //----------------- IR-839015-3DEXPERIENCER2020x-----------------------------------
      if(!customFreezePane.equalsIgnoreCase(""))
      {return true;
      }
  ...
  // ++ HAT1 ZUD: TSK3278161 ++
  /**
   * Returns a boolean value if Derived Requirement Commands are not be available for Requirement Specification action menu.
   * @param context - the eMatrix <code>Context</code> object
   * @param args - None
   * @return the link
   * @throws Exception
   */
  public static boolean showInsertNewOrExistingDerivedReqCmd(Context context, String[] args)
    {
      boolean showCmd = false;

      //By-default RMTStructureBrowserAddExistingDerivedRequirementPreCheck Commands will not be available.
      return showCmd;
    }

  /**
   * Returns a boolean value if Version category command(Under Change Control) for Requirement is to show.
   * @param context - the eMatrix <code>Context</code> object
   * @param args - None
   * @return the link
   * @throws Exception
   */
  public static boolean showReqVersionCategoryCmd(Context context, String[] args)
    {
      boolean showCmd = true;
      //By-default Version category command(Under Change Control) for Requirement is hidden.
      String showReqVersionCategory = "";
      try {
        showReqVersionCategory = EnoviaResourceBundle.getProperty(context, "emxRequirements.RequirementVersion.disable");
      } catch (FrameworkException e) {
        // TODO Auto-generated catch block
        e.printStackTrace();
        return showCmd;
      ...
      if(showReqVersionCategory.equalsIgnoreCase("true"))
        showCmd = false;
  ...
  /**
   * Returns a boolean value if Req-spec Action --> Menu Content Report is to show.
   * @param context - the eMatrix <code>Context</code> object
   * @param args - None
   * @return the link
   * @throws Exception
   */
  public static boolean showReqSpecContentReportMenu(Context context, String[] args)
    {
      //By-default Menu Content Report will not be available for Requirement Specification action menu.
      boolean showCmd = true;
      try
      {
        Map programMap = (HashMap) JPO.unpackArgs(args);
        String table = "";
        table = (String) programMap.get("table");

        String showReqSpecContentReport = "";
        showReqSpecContentReport = EnoviaResourceBundle.getProperty(context, "emxRequirements.ContentReport.disable");
        if(table.equalsIgnoreCase("RMTSpecStatusTable") && showReqSpecContentReport.equals("true"))
          showCmd = false;
  ...
  /**
   * Returns a boolean value if RaiseECActionLink command is to show.
   * @param context - the eMatrix <code>Context</code> object
   * @param args - None
   * @return the link
   * @throws Exception
   */
  public static boolean showRaiseECActionLinkCmd(Context context, String[] args)
    {
      //By-default RaiseECActionLink Commands will not be available.
      return false;
    }
  /**
   * Returns a boolean value if AttachECCategory command is to show.
   * @param context - the eMatrix <code>Context</code> object
   * @param args - None
   * @return the link
   * @throws Exception
   */
  public static boolean showAttachECCategoryCmd(Context context, String[] args)
    {
      //By-default AttachECCategory Commands will not be available.
      return false;
    }
  // -- HAT1 ZUD: TSK3278161 --
  ...
  // ++ HAT1 ZUD: IR-536557-3DEXPERIENCER2018x fix ++
  /**
   * Returns a boolean value true if Menu 'Content Report' is to show in
   * Menu type_SoftwareRequirementSpecificationRMB & RMTSpecificationsListBottomActionBar.
   * @param context - the eMatrix <code>Context</code> object
   * @param args - None
   * @return boolean true - to show Menu, false - to hide Menu
   * @throws Exception
   */
  public static boolean showRMTRequirementSpecExportContentMenu(Context context, String[] args)
    {
      //By-default 'RMTRequirementSpecExportContent' will not be available.
      return false;
    }
  // -- HAT1 ZUD: IR-536557-3DEXPERIENCER2018x fix --
  ...
  public String getAccessTypeValue(Context context, String[] args) throws Exception
    {
      ...
      String rootType = RequirementsUtil.getRootType(context, type);
      if(rootType.isEmpty()) {
        throw new Exception("Invalid Object Type.");
      }
      if("DOCUMENTS".equals(rootType)) {
        rootType = ReqSchemaUtil.getRequirementSpecificationType(context);
      }

      String strAccess = null;
      //get default inherit access value according to user preference
      boolean isInheritedByDefault = RequirementsUtil.getInheritAccessPreference(context, rootType); 
      ...
      if (mode == null || mode.equalsIgnoreCase("create"))
      {
        ...
        strAccess += "<tr><td><input type=\"checkbox\" name=\"AccessType\" id=\"AccessType\" value=\"Inherited\" " + (isInheritedByDefault ? "checked=\"checked\"" : "") + "/>";
        strAccess += "</td></tr></table>";
      }
      return strAccess;
    }

  public void updateAccessTypeValue(Context context, String[] args)
    throws Exception
    {
      HashMap programMap = (HashMap) JPO.unpackArgs(args);
      HashMap paramMap = (HashMap)programMap.get("paramMap");
      ...
      String newValue = (String)paramMap.get("New Value");
      if(!"Inherited".equalsIgnoreCase(newValue)){
        newValue = "Specific";
      }

      DomainObject dom = DomainObject.newInstance(context, objectId);
      dom.open(context);
      dom.setAttributeValue(context, PropertyUtil.getSchemaProperty(context,"attribute_AccessType"), newValue);
      dom.close(context);
    }

  public String getPropagateAccessValue(Context context, String[] args) throws Exception
    {
      HashMap programMap = (HashMap) JPO.unpackArgs(args);
      ...
      String strAccess = null;
      //get propagate access default value according to user preference
      boolean  isPropagateByDefault = RequirementsUtil.getPropagateAccessPreference(context, rootType);
      ...
      if (mode == null || mode.equalsIgnoreCase("create"))
      {
        ...
        strAccess += "<tr><td><input type=\"checkbox\" name=\"PropagateAccess\" id=\"PropagateAccess\" value=\"TRUE\" " + (isPropagateByDefault ? "checked=\"checked\"" : "") + "/>";
        strAccess += "</td></tr></table>";
      }
      return strAccess;
    }
   ...
   * @since R2018x
   */
  public void updatePropagateAccessValue(Context context, String[] args)
    throws Exception
    {
      HashMap programMap = (HashMap) JPO.unpackArgs(args);
      HashMap paramMap = (HashMap)programMap.get("paramMap");
      ...
      DomainObject dom = DomainObject.newInstance(context, objectId);
      dom.open(context);
      dom.setAttributeValue(context, PropertyUtil.getSchemaProperty(context,"attribute_PropagateAccess"), newValue);
      dom.close(context);
    }

  /**
   * Initialize the "propagate access" and "inherit access" properties
   * on new requirements entities created through a "create"
   * transaction
   *<p>
   * "propagate access" is initialized according to the user settings for the
   * object type, provided the admin settings allow for it. Otherwise it is
   * reset to "no propagation"
   *<p>
   * Likewise "intherit access" is initialized according to to the user
   * settings for the object type, provided the admin settings allow for
   * it. Otherwise, it is reset to "no inheritance"
   *<p>
   * Triggers: (see RMTAppBusObjR420.tcl):
   *  - TypeRequirementCreateAction
   *  - TypeUserRequirementCreateAction
   *  - TypeChapterCreateAction
   *  - TypeCommentCreateAction
   *  - TypeRequirementGroupCreateAction
   *  - TypeSoftwareRequirementSpecificationCreateAction
   *<p>
   * Note: at the time this method is called, this JPO is
   * constructed with the pid of the new object
   *
   * @param context The MatrixOne context
   * @param args    The data provided through the trigger
   *                  - args[0] OBJECTTYPE (broken, see :WARNING:)
   */
  public void ownershipInheritanceDefault(Context context, String args[]) throws Exception {
    String objectId = getObjectId(context);
    String type = args[0];

    String kind = this.getInfo(context, "type.kindof");
    if("DOCUMENTS".equals(kind)) {
      kind = ReqSchemaUtil.getRequirementSpecificationType(context);
    }

    // GVC:210409:IR-809936 : Error message during Comment creation when Ownership Inheritance is desactivated
    // because "Propagate Access" attribute does not exist for this type --> Adding condition to not go through this code in case of Comment
    boolean isKindOfComment = kind.equals(ReqSchemaUtil.getCommentPolicy(context));
    if(!RequirementsUtil.isPropagateAccessAllowed(context) && !isKindOfComment) {
      this.setAttributeValue(context, "Propagate Access", "FALSE");
    }
    else if(!isKindOfComment){
      //Set propagate access value for the requirement entity creation form
      this.setAttributeValue(context, "Propagate Access", (RequirementsUtil.getPropagateAccessPreference(context, kind) + "").toUpperCase());
    }

    if(!RequirementsUtil.isInheritAccessAllowed(context)){
      this.setAttributeValue(context, "Access Type", "Specific");
    }
    else{ 
      //Set inherit access value for the requirement entity creation form
      this.setAttributeValue(context, "Access Type", RequirementsUtil.getInheritAccessPreference(context, kind) ? "Inherited" : "Specific");
    }
  }

  /**
   * Initialize the "propagate access" and "inherit access" properties
   * on new requirements entities created through a "clone" transaction
   *<p>
   * "propagate access" is initialized according to the user settings for the
   * object type, provided the admin settings allow for it. Otherwise it is
   * reset to "no propagation"
   *<p>
   * Likewise "inherit access" is initialized according to to the user
   * settings for the object type, provided the admin settings allow for
   * it. Otherwise, it is reset to "no inheritance"
   *<p>
   * Triggers: (see RMTAppBusObjR420.tcl):
   ...
   *<p>
   * Note: at the time this method is called, this JPO is
   * constructed with the pid of the clone object
   *
   * @param context The MatrixOne context
   * @param args    The data provided through the trigger
   *                  - args[0] OBJECTTYPE (broken, see :WARNING:)
   */  
  public void ownershipInheritanceDefaultOnClone(Context context, String args[]) throws Exception {
    String objectId = getObjectId(context);
    String type = args[0];
    String kind = this.getInfo(context, "type.kindof");
    if("DOCUMENTS".equals(kind)) {
    ...
    if(kind.equals(ReqSchemaUtil.getUseCaseType(context))){ //no ownership inheritance for use case
      return;
    }
    if(!kind.equals(ReqSchemaUtil.getCommentPolicy(context)) ) {
      if(!RequirementsUtil.isPropagateAccessAllowed(context)) {
        this.setAttributeValue(context, "Propagate Access", "FALSE");
      }
      else if(RequirementsUtil.getResetPropagateAccessOnClonePreference(context)){
        //Set propagate access value for the requirement entity created through a "clone" transaction
        this.setAttributeValue(context, "Propagate Access", (RequirementsUtil.getPropagateAccessPreference(context, kind) + "").toUpperCase());
      }
    }
    if(!RequirementsUtil.isInheritAccessAllowed(context)){
      ...
      if(RequirementsUtil.getResetInheritAccessOnClonePreference(context)){
        //Set inherit access value for the requirement entity created through a "clone" transaction
        this.setAttributeValue(context, "Access Type", RequirementsUtil.getInheritAccessPreference(context, kind) ? "Inherited" : "Specific");
      }
    }
  }
  ...
  /**
   * Initialize the "propagate access" and "inherit access" properties
   * on new requirements entities created through a "major revision"
   * transaction
   *<p>
   * "propagate access" is initialized according to the user settings for the
   * object type, provided the admin settings allow for it. Otherwise it is
   ...
   *<p>
   * Triggers: (see RMTAppBusObjR420.tcl):
   *  - TypeRequirementMajorReviseAction
   *  - TypeCommentMajorReviseAction
   *  - TypeChapterMajorReviseAction
   *  - TypeRequirementGroupMajorReviseAction
   *  - TypeSoftwareRequirementSpecificationMajorReviseAction
   *<p>
   * Note: at the time this method is called, this JPO is
   * constructed with the pid of the object to revise
   *
   * @param context The MatrixOne context
   * @param args    The data provided through the trigger
   *                  - args[0] OBJECTTYPE (broken, see :WARNING:)
   *                  - args[1] NEWOBJECTID
   */
  public void ownershipInheritanceDefaultOnMajorRevise(Context context, String args[]) throws Exception {
    requireNonNull(context);
    requireNonNull(args);
    if (args.length<2) throw new IllegalArgumentException();
    requireNonNullAndNonEmpty(args[1]); // new revision pid

    //:WARNING: see RMTAppBusObjR420.tcl : args[0] is supposed to
    // be OBJECTTYPE but is always ""
    // Fortunately, the object type can be retrieved through 
    // pid of the revision anyway

    DomainObject newObject = DomainObject.newInstance(context, args[1]);

    String kind = newObject.getInfo(context, "type.kindof");
    {
      if ("DOCUMENTS".equals(kind))
        kind = ReqSchemaUtil.getRequirementSpecificationType(context);
    }

    // ownership inheritance doesnt apply to Use Cases
    if (kind.equals(ReqSchemaUtil.getUseCaseType(context))) {
      return;
    }

    // (1) "Propagate Access" property
    //--------------------------------
    boolean supportsPropagate = false;
    {
      if (!kind.equals(ReqSchemaUtil.getCommentType(context)))
        supportsPropagate = true;
    }

    if (supportsPropagate) {
      // (1) Admin settings: propagate access allowed = NO
      // Ignore the user settings, set "Propagate Access" to false in any
      // case
      if (!RequirementsUtil.isPropagateAccessAllowed(context)) {
        newObject.setAttributeValue(context, "Propagate Access", "FALSE");
      }
      // (2) Admin settings: propagate access allowed = YES
      else {
        // User settings: "PropagateAccess"/"Reset to default value" for the
        // "Major Revise" transactions = YES
        // Set "Propagate Access" according to the user settings for the
        // object kind
        if (RequirementsUtil.getResetPropagateAccessOnMajorRevisePreference(context)) {
          String propagateValue = RequirementsUtil.getPropagateAccessPreference(context, kind) ? "TRUE" : "FALSE";
          newObject.setAttributeValue(context, "Propagate Access", propagateValue);
        }
        // User settings: "PropagateAccess"/"Reset to default value" for the
        // "Major Revise" transactions = NO
        // Leave the "Propagate Access" attribute as-is.
        // :NOTE: the attribute value is copied from the previous revision
        else {
          // NOTHING
        }
      }
    ...
    // (2) "Inherit Access" property
    //------------------------------
    {
      // (1) Admin settings: inherit access allowed = NO
      // Ignore the user settings, set "Access Type" to "Specific" in any
      // case
      if (!RequirementsUtil.isInheritAccessAllowed(context)) {
        newObject.setAttributeValue(context, "Access Type", "Specific");
      }
      // (2) Admin settings: inherit access allowed = YES
      else {
        // User settings: "Inherit Access"/"Reset to default value" for the
        // "Major Revise" transactions = YES
        // Set "Inherit Access" according to the user settings for the object
        // kind
        if (RequirementsUtil.getResetInheritAccessOnMajorRevisePreference(context)) {
          String inheritValue = RequirementsUtil.getInheritAccessPreference(context, kind) ? "Inherited" : "Specific";
          newObject.setAttributeValue(context, "Access Type", inheritValue);
        }
        // User settings: "Inherit Access"/"Reset to default value" for the
        // "Major Revise" transactions = NO
        // Leave the "Inherit Access" attribute as-is.
        // :NOTE: the attribute value is copied from the previous revision
        else {
          // NOTHING
        }
  ...
  public boolean displayPropagateAccess(Context context, String args[]) throws Exception {
    return RequirementsUtil.isPropagateAccessAllowed(context);
  }
  public boolean displayInheritAccess(Context context, String args[]) throws Exception {
    return RequirementsUtil.isInheritAccessAllowed(context);
  }
  public boolean displayOwnershipInheritancePreference(Context context, String args[]) throws Exception {
    return RequirementsUtil.isPropagateAccessAllowed(context) || RequirementsUtil.isInheritAccessAllowed(context) || RequirementsUtil.isAdmin(context);
  }
  // ++ HAT1 ZUD: OSLC Requirement Proxy Category hide components ++
  /**
   * Returns a boolean value true if Menu 'Content Report' is to show in
   * Commands 1. CloneInfo
   *      2. RMTRequirementSpecifications
   * Category 1. RMTRequirementUseCaseTreeCategory
   *      2. RMTRequirementTestCaseTreeCategory
   * Portal   1. RMTTraceabilityParameterReport
   *      2. RMTTraceabilityTestCaseReport
   * @param context - the eMatrix <code>Context</code> object
   * @param args - None
   * @return boolean true - to show Menu, false - to hide Menu
   * @throws Exception
   */
  //Related Objects
  public static boolean showRMTRequirementSpecifications(Context context, String[] args) throws Exception
    {
      return isRequirementProxy(context, args);
    }
  public static boolean showCloneInfo(Context context, String[] args) throws Exception
    {
      return isRequirementProxy(context, args);
    }
  public static boolean showRMTRequirementUseCaseTreeCategory(Context context, String[] args) throws Exception
    {
      return isRequirementProxy(context, args);
    }

  public static boolean showRMTRequirementTestCaseTreeCategory(Context context, String[] args) throws Exception
    {
      return isRequirementProxy(context, args);
    }

  //Traceability Protal.
  public static boolean showRMTTraceabilityParameterReport(Context context, String[] args) throws Exception
    {
      return isRequirementProxy(context, args);
    }
  public static boolean showRMTTraceabilityTestCaseReport(Context context, String[] args) throws Exception
    {
      return isRequirementProxy(context, args);
    }


  public static boolean isRequirementProxy(Context context, String[] args) throws Exception
    {
      //By-default 'RMTRequirementSpecExportContent' will not be available.
      Map programMap = (HashMap) JPO.unpackArgs(args);
      String objectId = (String)programMap.get("objectId");
      ...
      String isInterfaceProxyItem = domObj.getInfo(context, "interface[ProxyItem]");
      if("TRUE".equalsIgnoreCase(isInterfaceProxyItem))
      {
        return false;
      }
  ...
  // -- HAT1 ZUD: OSLC Requirement Category hide components --
  /**
   * prevent unsupported connection to Requirement Proxy object
   *
   *
   * @param context The Matrix Context object
   * @param args The trigger arguments. Following should be the contents of this array
   ...
   * @throws MatrixException if operation fails.
   */
  public int triggerCheckRequirementProxyConnection(Context context, String[] args) throws MatrixException {
    final int ALLOW_MODIFICATION = 0;
    final int DENY_MODIFICATION = 1;
    try {
      // Argument check
      ...
      String strRelType= args[3];

      if ("Specification Structure".equals(strRelType)) {
        //Get error message
        String strLanguage = context.getSession().getLanguage();
        String strErrorMessage = EnoviaResourceBundle.getProperty(context, "emxRequirementsStringResource", context.getLocale(), "emxRequirements.Error.UnsupportedClient");
  ...
  // ++ KIE1 IR-648749-3DEXPERIENCER2019x
  public static boolean showLegacyEffectivityColumn(Context context, String[] args)throws Exception
    {
      boolean showEffectivityColumn             = true;
      Map programMap                    = (HashMap) JPO.unpackArgs(args);
      Map fieldMap                      = (HashMap) programMap.get("SETTINGS");
      String columnSetting                = (String)fieldMap.get("isCFF");

      String strExprValue                 = "";

      //retrieving the EffectivityDecoupling Value
      String strListMQL                   = "list expression $1";
      String strExpr                      = MqlUtil.mqlCommand(context, strListMQL, "DecouplingActivation");
      ...
      if(strExpr != null && ! "null".equalsIgnoreCase(strExpr) && ! "".equalsIgnoreCase(strExpr)) {
        ...
        strExprValue = MqlUtil.mqlCommand(context, strPrintMQL, "DecouplingActivation");
      }

      switch(strExprValue)
      {
      case "EnableDecoupling":
        if("true".equalsIgnoreCase(columnSetting))
        {
          showEffectivityColumn = false;
        }
        break;
      case "DisableDecoupling":
        if(!"true".equalsIgnoreCase(columnSetting))
        {
          showEffectivityColumn = false;
        }
   ...
   */
  private static void requireNonNullAndNonEmpty(String str) {
