...
public class emxPersonBase_mxJPO extends emxDomainObject_mxJPO
{
        ...
        throws Exception
    {
        if (!context.isConnected())
            throw new Exception(EnoviaResourceBundle.getProperty(context, "emxPersonOrgModelStringResource", context.getLocale(), "emxComponents.Generic.NotSupportedOnDesktopClient"));
        return 0;
    }
        ...
        try
        {
			...
			else if(sEvent.equals("Promote"))
			{
				...
				if(isOnPremise) {
					...
					MqlUtil.mqlCommand(context, "modify person \""+"$1"+"\" type active \""+"$2"+"\" email", true, personName, sEmailFlag);
				}	
			}
			else if(sEvent.equals("Demote"))
			{
				...
				if(isOnPremise) {
					...
					MqlUtil.mqlCommand(context, "modify person \""+"$1"+"\" type inactive disable email", true, personName);
				}
			}
        }
        catch (Exception ex)
...
//added on 12 march - end
    ...
    if(mapList != null)
    {
        ...
        {
            String strMessage = EnoviaResourceBundle.getProperty(context,"emxPersonOrgModelStringResource",new Locale(languageStr),"emxComponents.Warning.ObjectFindLimit") + queryLimit + EnoviaResourceBundle.getProperty(context,"emxComponentsStringResource",new Locale(languageStr),"emxComponents.Warning.Reached");
               ...
               throws Exception
    {
        ...
        else if(personObject.ATTRIBUTE_FIRST_NAME.equals(strAttributeName))
        {
            ...
            if (strFullNameFormat != null)
            {
				...
				// Update the Cached value with new fullname
				PersonUtil.setPersonProperty(context, strPersonName, PersonUtil.PROPERTY_FULL_NAME, strNewFullName);
            }
        }
    ...
    */
     ...
     */
                ...
                throws Exception
     {
            ...
            if(personObject.ATTRIBUTE_EMAIL_ADDRESS.equals(strAttributeName))
            {
    			...
    			// Update the Cached value with new emailId
    			PersonUtil.setPersonProperty(context, strPersonName, PersonUtil.PROPERTY_EMAIL, strNewAttributeValue);
            }
     }
    ...
    public MapList getAssignedRoleList(Context context, String[] args) throws FrameworkException {
        try {
            ...
            return new emxRoleUtil_mxJPO(context, args).getRoleListForSummaryTable(getExtractedRolesFromRoleList(assignments), getExtractedRolesFromRoleList(hidden));
        } catch (Exception e) {
    ...
    private MapList getExtractedRolesFromRoleList(List roleList){
    	HashSet<String> rolesList = new HashSet<String>();
    	Iterator it = roleList.listIterator();
        while(it.hasNext()){
        	...
        	} else if(roleName.startsWith("Grant.")) {
        		continue;
        	}
	...
	/**
	 * Create Personal Workspace Object
	 * 
	 * @param context the ENOVIA <code>Context</code> object
	 * @param args request arguments
	...
	/**
	 * Delete Personal Workspace Object
	 * 
	 * @param context the ENOVIA <code>Context</code> object
	 * @param args request arguments
	 * @throws Exception if operation fails
	...
	/**
	 * Gets Personal Workspace Id for a given Person
	 * 
	 * @param context the ENOVIA <code>Context</code> object
	 * @param personId person id for which Personal Workspace is retrieved
	 * @return Personal Workspace Id
	 * @throws Exception if operation fails
	 * @deprecated since R2022x GA, Personal Workspace are deprecated
	 */
	...
	/**
	 * API to construct the person image from 3dswym in Cloud env
	 * @param context - The matrix context
	 * @param args - request arguments
	 * @return - column values
	 * @throws Exception
	 */
	public Vector getPersonData(Context context, String[] args) throws Exception {
		HashMap programMap = (HashMap)JPO.unpackArgs(args);
		MapList objList = (MapList)programMap.get("objectList");
		int objListSize = objList.size();
		HashMap paramList = (HashMap) programMap.get("paramList");
		boolean isprinterFriendly = paramList.get("reportFormat") != null;
        String swymURL = FrameworkUtil.getSwymURLForPersonImage(context); 
		...
		for (int i=0; i < objListSize; i++) {
			...
			String name = (String)map.get("name");
		 	if(!isprinterFriendly) {
				strBuff.append("<img style='width:25px; height:25px; border-radius:100px; border:solid 1px #d1d4d4' ");
				if(UIUtil.isNotNullAndNotEmpty(swymURL)){
					String finalURL = swymURL + name;
					strBuff.append("src='"+finalURL+"' />");
				}else{
					strBuff.append("src='images/iconPerson123.png' />");
				}
		 		strBuff.append("<span style='padding-left:10px' >"+ XSSUtil.encodeForXML(context, PersonUtil.getFullName(context, name))+"</span>");
		 	}else {
		 		strBuff.append(XSSUtil.encodeForXML(context, PersonUtil.getFullName(context, name)));
		 	}
	...
	public int checkChangeOwnerForUserGroup(Context context, String[] args)
			throws Exception
			{
					...
					String roleLeader =   PropertyUtil.getSchemaProperty(context,"role_VPLMProjectLeader");
					...
					if("owner".equals(KindOfOwner) && !Owner.equals(NewOwner) && DomainConstants.POLICY_GROUP_PROXY.equals(Policy)){
						Iterator it = PersonUtil.getAssignments(context,NewOwner).iterator();
		                while(it.hasNext()){
		                	String roleName = it.next().toString() ; 
		                	...
		                	if(roleName.startsWith(roleLeader+".") || roleName.startsWith(roleAuthor+".") || roleName.startsWith(roleAdmin+".")){
		                		roleexisted = true;
		                		break;
		            		}
		                }
						if(!roleexisted) {
							String strMessage = EnoviaResourceBundle.getProperty(context, "emxComponentsStringResource",context.getLocale(), "emxComponents.UserGroup.ChangeOwnerCheck");
							//${CLASS:emxContextUtil}.mqlNotice(context,strMessage);
							throw new MatrixException(strMessage);
						 }
					}
					return 0;	
			}
