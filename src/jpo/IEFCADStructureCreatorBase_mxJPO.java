...
public class IEFCADStructureCreatorBase_mxJPO
{
	...
	protected String PART_RELEASE_PHASE                 = "";
	
	//IR-635284 : Start
	protected String gcoName							= "";			
	protected BusinessObject gcoObject					= null;			
	protected String busTypeMapping						= "";	
	protected String gcoType							= "";			
	//IR-635284 : End
    ...
    public void initialize(Context context,MCADGlobalConfigObject inputGCO, MCADLocalConfigObject inputLCO, String languageName) throws MCADException
    {
        try
        {
			...
			this.PART_RELEASE_PHASE = ebomConfObject.getConfigAttributeValue(IEFEBOMConfigObject.ATTR_NEW_PART_RELEASE_PHASE);
	...
	public Hashtable createStructure(Context context, String[] args) throws Exception
	{
		...
		try
		{
			...
			//IR-635284 : Start
			gcoType 		  			=  MCADMxUtil.getActualNameForAEFData(context, "type_MCADInteg-GlobalConfig");
			if(null == gcoName || "".equals(gcoName))
				gcoName 	  				= (String)inputLCO.getIntegrationNameGCONameMapping().get(integrationName);
			if(null == gcoObject || "".equals(gcoObject))
				gcoObject 					= util.getGCOObjectFromName(context, gcoName, gcoType);
			if(null== busTypeMapping || "".equals(busTypeMapping))
				busTypeMapping				= gcoObject.getAttributeValues(context, MCADMxUtil.getActualNameForAEFData(context, "attribute_MCADInteg-BusTypeMapping")).getValue();
			//IR-635284 : End
    ...
    protected boolean traversePartStructure(Context context, String partID, String parentDesignObjID, String relId, String[] arguments, String folderId) throws MCADException
    {
        ...
        {	
			...
			while(strTok.hasMoreTokens())
			{
				...
				if(traversePartStructure(context, childObjectId, associatedDesignObjID, relationshipId, arguments,folderId))
					isStructureCreated = true;
			}
        }
        catch(Exception e)
    ...
    {
		HashSet mappedTypes = util.getValueFromMapping(busTypeMapping, cadType);		//IR-635284 : Get Values of Bus Type Mapping from GCO for specific CAD type
    	String specId = "";
		...
		if(partSpecIdList != null && !partSpecIdList.isEmpty())
		{
			...
			for(int i = 0 ; i < partSpecIdList.size() ; i++)
			{
				...
				if (null == integObjectId || integObjectId.isEmpty()) continue;
				
				//IR-553343: If integObjectId is type of Family or Component then return integObjectId otherwise return ""
				...
				String integObjectCadType = util.getCADTypeForBO(context, busObject);
     			if(globalConfigObject.isTypeOfClass(integObjectCadType, MCADAppletServletProtocol.TYPE_FAMILY_LIKE))
				{
						Hashtable exceptionDetails = new Hashtable(3);
						exceptionDetails.put("TYPE", busObject.getTypeName());
						exceptionDetails.put("NAME", busObject.getName());
						exceptionDetails.put("REVISION", busObject.getRevision());
						busObject.close(context);
						MCADServerException.createException(resourceBundle.getString("mcadIntegration.Server.Message.FamilyTypeInPartSpecification",exceptionDetails), null);		
				}
				if(globalConfigObject.isTypeOfClass(integObjectCadType, MCADAppletServletProtocol.TYPE_ASSEMBLY_LIKE) ||
  				   globalConfigObject.isTypeOfClass(integObjectCadType, MCADAppletServletProtocol.TYPE_COMPONENT_LIKE))
				{
						//IR-635284 :Start
						String specifId = generalUtil.getValidObjctId(context, integObjectId);
						BusinessObject b1 = new BusinessObject(specifId);
						b1.open(context);
						String objtype = b1.getTypeName();
						b1.close(context);
						//Compare mapped types with the Object Type
						if(mappedTypes.contains(objtype)) {
							specId = specifId;
							busObject.close(context);
							break;
						}
	...
	{
		//IR-461325 : to get Revision value
		...
		else if("$$Revision$$".equals(attrName))
		{
			partObj.open(context);
			String revision = partObj.getRevision();
	...
	protected void matchRevisionSequence(Context context, String designObjPolicy, String partObjId) throws Exception
	{
		...
		{				
			...
			if(PART_RELEASE_PHASE.equalsIgnoreCase("Development")){
				partObjSeq = MCADMxUtil.getRevisionSequenceForDevelopmentPhase(context);
			}
