...
public class emxProgramBusinessGoalBase_mxJPO extends emxDomainObject_mxJPO
{
	...
	throws Exception
	{
		try
		{
			...
			String sloggedInUser = context.getUser().toString();
			...
			Iterator itrBusinessGoal = businessGoalList.iterator();
			while(itrBusinessGoal.hasNext()){				
				Map mapBusinessGoalInfo = (Map) itrBusinessGoal.next();
				String sOwner = (String) mapBusinessGoalInfo.get(BusinessGoal.SELECT_OWNER);
				if(!sloggedInUser.equals(sOwner)){
					mapBusinessGoalInfo.put("disableSelection", "true");
				}
	...
	throws Exception
	{
		...
		try
		{
			...
			for (int i = 0; i < actionListSize; i++)
			{
				...
				String comments = XSSUtil.encodeForXML(context,(String) objectMap.get(BusinessGoal.SELECT_DESCRIPTION));
	...
	public String getBusinessUnit(Context context, String args[]) throws Exception
	{
		...
		Map organizationInfoMap = new HashMap();
		if(null != organizationId && !"".equals(organizationId)){
			
			StringList orgSelects = new StringList(2);
			orgSelects.add(DomainObject.SELECT_TYPE);
			...
			organizationInfoMap = domainObject.getInfo(context,orgSelects);
		}
		
		String organizationType = (String)organizationInfoMap.get(DomainObject.SELECT_TYPE);
		String organizationTitle = (String)organizationInfoMap.get(DomainConstants.SELECT_ATTRIBUTE_TITLE);
	...
	public String getBusinessUnitModeEdit(Context context, String args[]) throws Exception
	{
		...
		busSelects.add(businessGoal.SELECT_ORGANIZATION_NAME);
		busSelects.add(businessGoal.SELECT_ORGANIZATION_ID);
		busSelects.add("to[" + RELATIONSHIP_ORGANIZATION_GOAL + "].from."+DomainConstants.SELECT_ATTRIBUTE_TITLE);
		
		Map businessGoalMap = businessGoal.getInfo(context, busSelects);
		String organizationName = (String)businessGoalMap.get(businessGoal.SELECT_ORGANIZATION_NAME);
		String organizationTitle = (String)businessGoalMap.get("to[" + RELATIONSHIP_ORGANIZATION_GOAL + "].from."+DomainConstants.SELECT_ATTRIBUTE_TITLE);
	...
	public void createBusinessGoal(Context context, String args[]) throws Exception
	{
		...
		businessGoal.startTransaction(context, true);
		businessGoal.create(context, null, BusinessGoalName, selectedPolicy, context.getVault().getName(), attributeMap, BusinessUnitId, parentId);
		String objectType = "";
	...
	public MapList getBusinessUnitSpecifictoCompany(Context context, String[] args) throws MatrixException 
	{
		...
		try {
			...
			busSelects.add(businessUnit.SELECT_ATTRIBUTE_TITLE);
			...
			for (Iterator iterator = mpProjectList.iterator(); iterator.hasNext();) {
				...
				StringList slMemberIds = FrameworkUtil.split(memberIds, matrix.db.SelectConstants.cSelectDelimiter);
				 ...
				 if((slprojectList.contains(strPtojectId)|| strProjectState.equalsIgnoreCase(DomainObject.STATE_PROJECT_SPACE_ARCHIVE)|| 
						 ...
						 ||strProjectType.equalsIgnoreCase(ProgramCentralConstants.TYPE_EXPERIMENT))&& (!strProjectVisibility.equals("Company") || slprojectList.contains(strPtojectId))){
