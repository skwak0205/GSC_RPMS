 		...
 		try {
 	 		...
 	 		for(BusinessObjectWithSelectItr itr1= new BusinessObjectWithSelectItr(folderContentWithSelectList); itr1.next();)
 	        {
				...
				StringBuffer files = new StringBuffer();				
 	 			...
 	 			if(null != slFolderContentsIds && null != slFolderFiles) {
 	 				int slFolderContentsIdsSize = slFolderContentsIds.size();					
					for(int i=0; i < slFolderContentsIdsSize; i++)
					{
 	 					...
 	 					for(int j=0; j < noOfFiles; j++) {
 	 						...
 	 						if(strPrinterFriendly.equalsIgnoreCase("false")) {
		 	 					if(strReportFormat.equals("CSV") || strReportFormat.equals("Text")){
		 	 						...
		 	 						if(j < (noOfFiles - 1)) { //to append space between file names except after last file.
		 	 							sbfFolderFileURL.append(" ");
		 	 						}
		 	 					}else {
		 	 						 sbfFolderFileURL.append("<a href=javascript:emxTableColumnLinkClick(" +
			 	                              "'../components/emxCommonDocumentPreCheckout.jsp?action=download&appProcessPage=&emxSuiteDirectory=programcentral&relId=null");
			 						  sbfFolderFileURL.append("&jsTreeID=null&suiteKey=ProgramCentral&objectId=");
			 						  sbfFolderFileURL.append(slFileIds.get(j));
			 	                      sbfFolderFileURL.append("',700,600);>");
			 						  sbfFolderFileURL.append(XSSUtil.encodeForHTML(context,slFolderFiles.get(j)));
			 	                      sbfFolderFileURL.append("</a><br>");
		 	 					}
	 	 					}else{
	 	 						sbfFolderFileURL.append(XSSUtil.encodeForHTML(context,slFolderFiles.get(j)));
	 	 					}
					...
					sbfFolderFileURL.append(DomainConstants.EMPTY_STRING);
				}		
				 documentFileList.add(sbfFolderFileURL.toString());
				 sbfFolderFileURL.delete(0, sbfFolderFileURL.length());
			}
 		}
 		catch (Exception e) {
			e.printStackTrace();
