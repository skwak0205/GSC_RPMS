...
import com.dassault_systemes.vplm.ImplLinkManagementItf.interfaces.ICommonImplementLinkNavigationItf;
import com.dassault_systemes.vplm.ImplLinkManagementItf.services.CommonImplementLinkProvider;


import com.dassault_systemes.platform.model.Oxid;
import com.dassault_systemes.platform.model.itf.IOxidService;
import com.dassault_systemes.platform.model.itf.nav.IOxIdentifiable;
...
import com.dassault_systemes.platform.model.itf.IAttributesAccess;
import com.dassault_systemes.platform.model.services.BrowseServicesProvider;
...
public class emxVPLMTraceabilityReportBase_mxJPO extends emxDomainObject_mxJPO
{
   ...
   static public final String ATTRV_V_NAME = "PLMEntity.V_Name";
   //START : LX6 IR-347523-3DEXPERIENCER2016x Issue in traceability report
   // Schema Constants:
   ...
   private ICommonImplementLinkNavigationItf navItf = null;
   private IPLMxCoreAccess 	     coreModeler;
   private MapList               reqObjects = new MapList();
   private boolean               isUnicornEnabled = false;
   
   private INavBusProvider navBusProvider = null;
   private IOxidService idService = null;
   ...
   public emxVPLMTraceabilityReportBase_mxJPO(Context context, String[] args) throws Exception
   {
	  ...
	  navBusProvider = NavigationServicesProvider.getNavBusProvider();
	  idService = IdentificationServicesProvider.getOxidService();
   }
   ...
   public boolean initVPLMContext(String objectId)
   {
      try
      {
         ...
         plmSession = PLMCoreModelerSession.getPLMCoreModelerSessionFromContext(emxContext);
         if (plmSession != null)
         {
			...
			navItf = CommonImplementLinkProvider.getCommonImplementLinkNavigationItf();
      ...
      throws ClassNotFoundException, Exception
   {
      ...
      if (coreModeler == null || navItf == null)
...
//END : LX6 IR-361052 - Requirement-Physical Traceability not avaible for Product sub type
	...
	// ++ ZUD Added new function for migration ++ //
   private INavBusObject getNavBusFromPid(Context context, String ObjlID) throws Exception
	{
		System.out.println("#############OBJID######################"+ObjlID);
		DomainObject obj = DomainObject.newInstance(context,ObjlID);
		Oxid oxid = idService.getOxidFromPid(context, obj.getInfo(context, "physicalid"));
		return navBusProvider.createNavBusObject(context, oxid);
	}

	public List<INavBusConnection> getNavBusConnectionFromRelId(Context context, String[] relIDs) throws Exception
	{
		List<INavBusConnection> listOfNavBusConn = new ArrayList<INavBusConnection>();
		for(int i=0;i<relIDs.length;i++)
		{
			DomainRelationship rel = new DomainRelationship(relIDs[i]);
			listOfNavBusConn.add(navBusProvider.createNavBusConnection(context, rel));
		}
		 return listOfNavBusConn;
	}
	private List<INavBus> getPointedInstance(Context context, String[] relIDs) throws Exception
	{
		 // ++ ZUD test for Migration ++
		 IPLMDictionaryPublicItf dico = new IPLMDictionaryPublicFactory().getDictionary();
		 IPLMDictionaryPublicClassItf iType = dico.getClass(context, "RFLPLMImplementConnection");
		 
		List<INavBusConnection> listOfNavBusConn = getNavBusConnectionFromRelId(context,relIDs);
		List<INavBusObject> listOfImplementngRoots = new ArrayList<INavBusObject>();
		
		listOfImplementngRoots = navItf.getImplLinksFromPathOfInstancesImplemented(context, iType, listOfNavBusConn);
		boolean[] iComputeStatus = {false, false, false};
		List<LinkRelationInformation> listImplementingILs = navItf.getImplementingFromListOfLinks(context, listOfImplementngRoots, iComputeStatus);
		List<List<INavBus>> implementing = null;
		List<INavBus> listImplementingILRef = new ArrayList<INavBus>();
		for(LinkRelationInformation ImplementedILS : listImplementingILs)
			{
				implementing = ImplementedILS.getPointedObjects();
				if(!implementing.isEmpty()) {
					List<INavBus> lImplemented = implementing.get(0);
					listImplementingILRef.addAll(lImplemented);
				}
			}
		return listImplementingILRef;
		// -- ZUD test for Migration --
	}
	private List<INavBus> getPointedReference(Context context, String physicalID) throws Exception
	{
		    // ++ ZUD test for Migration ++
		 IPLMDictionaryPublicItf dico = new IPLMDictionaryPublicFactory().getDictionary();
		 IPLMDictionaryPublicClassItf iType = dico.getClass(context, "RFLPLMImplementConnection");
		 INavBusObject NavbusObj =  this.getNavBusFromPid(context, physicalID);
		 
		//List<INavBusObject> listOfNavBus = new ArrayList<INavBusObject>();
		List<INavBusObject> listOfImplementngRoots = new ArrayList<INavBusObject>();
		//listOfNavBus.add(NavbusObj);
		listOfImplementngRoots = navItf.getImplLinksFromImplementedReference(context, iType, NavbusObj);
		if(listOfImplementngRoots.isEmpty())
			return null;
   ...
   //END : LX6 IR-361052 - Requirement-Physical Traceability not avaible for Product sub type
      ...
      checkForNull("navItf", navItf, debugTrace);
      ...
      try
      {
		...
		List<INavBus> listImplementingILRef = getPointedReference(context , emxId);
		if(listImplementingILRef == null) return null;
		 printDebugTrace("Number of VPLM Roots Found", listImplementingILRef == null ? "0" : Integer.toString(listImplementingILRef.size()), debugTrace);
		for (INavBus curRoot : listImplementingILRef) {
            checkForNull("curRoot", curRoot, debugTrace);
            if(curRoot == null) continue;
			Oxid curRootOxid = null;
			curRootOxid = ((IOxIdentifiable) curRoot).getOxid();
			
			String curRtPlmId = curRootOxid.getPhysicalid();
            printDebugTrace("getPLMIdentifier for this rootType returns :", curRtPlmId, debugTrace);
            //START : LX6 IR-361052 - Requirement-Physical Traceability not avaible for Product sub type            
            IPLMDictionaryPublicClassItf iFunctionalType = dico.getClass(context, curRootOxid.getType().substring(curRootOxid.getType().indexOf("/")+1));
            // check the entity type
            if(iFunctionalType.isKindOf(context, rootType)){
//END : LX6 IR-361052 - Requirement-Physical Traceability not avaible for Product sub type            	 
               Map plmObject = new HashMap();
               plmObject.put(VPLM_ID, curRtPlmId);

               // Add all the plm object attributes to the hashmap...
			   IAttributesAccess attrAccess = BrowseServicesProvider.getAttributesAccessServices();
			   Map<String, Object> attributes = null;
			   attributes = attrAccess.getAttributes(context, curRootOxid, null);
               printDebugTrace("BEGIN Dumping the attributes of VPLM root object==============", "", debugTrace);
               for (String key : attributes.keySet())
               {
                  //String key = (String) keys.nextElement();
                  String val = attributes.get(key) + ""; //IR A0669007: sometimes this returns com.dassault_systemes.vplm.data.PLMQLNULL
      ...
      try
      {
         ...
         reqObjects = emxObject.getRelatedObjects(context, strRelType, strObjTypes, objSelects, relSelects,
           ...
           for (int i = 0; i < reqObjects.size(); i++)
           {
              ...
              String reqPLMRelID = reqRelId;
			  //reqPLMRelIDs == null || reqPLMRelIDs.length == 0 ? null : reqPLMRelIDs[0];
   ...
   private MapList getPLMReferences(Context context,String rootId, String reportType,String implementingRootType)throws Exception{
	   ...
	   for (int i = 0; i < reqObjects.size(); i++)
	      {
			  ...
			  IPLMDictionaryPublicItf dico = new IPLMDictionaryPublicFactory().getDictionary();
			  //printDebugTrace("rootType", rootType, debugTrace);
			  List<INavBus> listImplementingILRef = null;
			  printDebugTrace("Number of VPLM Roots Found", listImplementingILRef == null ? "0" : Integer.toString(listImplementingILRef.size()), debugTrace);
	          try{      	  	
					listImplementingILRef = getPointedReference(context , objId);
					if(listImplementingILRef == null) continue;
				}
	         ...
	         for (INavBus curRoot : listImplementingILRef)
	          {
               ...
               // Add all the plm object attributes to the hashmap...
				...
				attributes = attrAccess.getAttributes(context, curRootOxid, null);
				 ...
				 String pairId = curRtPlmId + "|" + objId;
   ...
   private MapList getPLMInstances(Context context, String rootId, String reportType,String implementingRootType)throws Exception{
	   ...
	   for (int i = 0; i < reqObjects.size(); i++)
	   {
		   ...
		   List<INavBus> listImplementingILRef = null;
		   try{
			   listImplementingILRef = getPointedInstance(context , path);
		   }
		   catch(Exception plmException)
		   {
			   plmException.printStackTrace();
		   ...
		   for (INavBus curRoot : listImplementingILRef)
		   {
			   // get Reference from Instance
			   INavBusConnection currInst = (INavBusConnection) curRoot;
			   if(curRoot == null) continue;
				Oxid curInstOxid = null;
				curInstOxid = ((IOxIdentifiable) curRoot).getOxid();
			   IPLMDictionaryPublicItf dico = new IPLMDictionaryPublicFactory().getDictionary();
			   IPLMDictionaryPublicClassItf iFunctionalType = dico.getClass(context, curInstOxid.getType().substring(curInstOxid.getType().indexOf("/")+1));
			   
			   INavBusObject currInstFrom = currInst.getFrom();
			   INavBusObject currInstTo = currInst.getTo();
			   checkForNull("curRoot", curRoot, debugTrace);
				
				if(currInstTo == null) continue;
				Oxid curRootOxid = null;
				curRootOxid = ((IOxIdentifiable) currInstTo).getOxid();
				
				if(currInstFrom == null) continue;
				Oxid currInstFromOxid = null;
				currInstFromOxid = ((IOxIdentifiable) currInstFrom).getOxid();
				
				 String curRtPlmId = curRootOxid.getPhysicalid();
				 String OwnerPlmId = currInstFromOxid.getPhysicalid();
   ...
   private MapList getVPLMTraceabilityFLData(Context context, String rootId, String reportType) throws Exception
   {
      ...
      if (navItf == null || coreModeler == null)
         return(dataList);
      ...
      dataList.addAll(getPLMReferences(context,rootId, reportType,implementingRootTypeReference));
      return dataList;
   }
