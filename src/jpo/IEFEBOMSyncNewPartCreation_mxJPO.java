...
public class IEFEBOMSyncNewPartCreation_mxJPO
{
  ...
  MCADServerResourceBundle	serverResourceBundle	= null;
	...
	String famID		= args[8];
		...
		//Added for Go To Production--Start
		String PART_RELEASE_PHASE = ebomConfObj.getConfigAttributeValue(IEFEBOMConfigObject.ATTR_NEW_PART_RELEASE_PHASE);
		INITIAL_PART_REVISION	= getRevisionSequenceForReleasePhase(context,PART_RELEASE_PHASE);
		String PART_POLICY_NAME = DomainConstants.POLICY_EC_PART;
		//Added for Go To Production--End
		...
		{			
			...
			String sPartReleasePhase = MCADMxUtil.getActualNameForAEFData(context, "attribute_ReleasePhase");
			String sChangeControlled = MCADMxUtil.getActualNameForAEFData(context, "attribute_ChangeControlled");
			AttributeList attrLst = new AttributeList(3);
			if(partAttributeList != null &&  partAttributeList.contains(sPartReleasePhase) && partAttributeList.contains(sChangeControlled)){
				String changeControlled = "True";
				
				if(DEVELOPMENT.equalsIgnoreCase(PART_RELEASE_PHASE)){
					changeControlled = "False";
				}

				Attribute attr_release_phase = new Attribute(new AttributeType(sPartReleasePhase), PART_RELEASE_PHASE);
				Attribute attr_change_cont = new Attribute(new AttributeType(sChangeControlled), changeControlled);

				attrLst.add(attr_release_phase);
				attrLst.add(attr_change_cont);
			}
			//Added for BGTP-End
				...
				else
				{
					...
					if(checkIfAttributeExistsOnPart)
					{
						...
						if(!attributeList.contains(partAttrName))
						{
							...
							String tempPartAttrName = StringUtil.split(partAttrName, ".").size() > 1 ? (String) StringUtil.split(partAttrName, ".").get(1) : partAttrName;
							if(!StringUtil.split(strInterfaceAttr, "|").contains(tempPartAttrName)){
	...
	private String getSystemAttributeValues(Context context, BusinessObject cadObj, String attrName) throws Exception
	{
		...
		{
			cadObj.open(context);
			String revision = cadObj.getRevision();
			cadObj.close(context);
			return revision;
		}
		else
			return "";
	...
	public static String getRevisionSequenceForReleasePhase(Context context, String sReleasePhase) throws Exception {
    	String sPartRevSequence = null;
    	if(sReleasePhase.equalsIgnoreCase(DEVELOPMENT)){
    		sPartRevSequence = MqlUtil.mqlCommand(context,"print policy $1 select $2 dump",DomainConstants.POLICY_DEVELOPMENT_PART,"minorsequence");
    	}else{
    		sPartRevSequence = MqlUtil.mqlCommand(context,"print policy $1 select $2 dump",DomainConstants.POLICY_EC_PART,"minorsequence");
    	}
    	StringList slRevSequence = FrameworkUtil.split(sPartRevSequence, ",");
    	return (String)slRevSequence.get(0);
    }
