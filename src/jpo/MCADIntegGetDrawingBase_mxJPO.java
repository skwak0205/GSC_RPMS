...
// [NDM] QWJ
		...
		while(inputIds.hasMoreElements())
		{
			...
			if(null!=relIdParentDetails && !relIdParentDetails.isEmpty())
			{
...
// [NDM] QWJ
	private Hashtable getPreviousRevisionVersionIds(Context context, ArrayList objectIds) throws Exception 
	{
		...
		String SELECT_EXPRESSION_MAJOR_ID 				= "from[" + REL_VERSION_OF + "].to.id";
		String SELECT_EXPRESSION_MINOR_IDS				= "to[" + REL_VERSION_OF + "].from.id";
		String SELECT_EXPRESSION_MINOR_IDS_FROM_MINOR	= "from[" + REL_VERSION_OF + "].to.to[" + REL_VERSION_OF + "].from.id";
		...
		busSelectionList.add(SELECT_EXPRESSION_MAJOR_ID);
		busSelectionList.add(SELECT_EXPRESSION_MINOR_IDS);
		busSelectionList.add(SELECT_EXPRESSION_MINOR_IDS_FROM_MINOR);
		...
		for(int i=0; i<busWithSelectionList.size(); i++)
		{
			...
			String majorId = "";
			StringList minorIdList = new StringList();
			ArrayList allIdList = new ArrayList();
			
			if(isVersion)
			{
				majorId = busObjectWithSelect.getSelectData(SELECT_EXPRESSION_MAJOR_ID);
				minorIdList = busObjectWithSelect.getSelectDataList(SELECT_EXPRESSION_MINOR_IDS_FROM_MINOR);
			}
			else
			{
				majorId = busObjectWithSelect.getSelectData(DomainObject.SELECT_ID);
				minorIdList = busObjectWithSelect.getSelectDataList(SELECT_EXPRESSION_MINOR_IDS);
			}
			
			allIdList.add(majorId);
			allIdList.addAll(minorIdList);
...
//[IR-346257]:XLV:START
//Only consider latest revision, thus it float drawing will be checked across versions only and not across revisions.	
				int j = (majorRevisions.size() - 1 );
				//for(int j = (majorRevisions.size() - 1 ); j > -1 ; j--)
//[IR-346257]:XLV:END	
				{
					String majorId 			= busObjectWithSelect.getSelectData("revisions[" + revision + "].id");
					String sRevIndex 		= busObjectWithSelect.getSelectData("revisions[" + revision + "].revindex");
					...
					if(revIndex<=currentRevIndex)
					{
						...
						StringList minorVersions = busObjectWithSelect.getSelectDataList("revisions[" + revision + "]." + SELECT_ON_ACTIVE_MINOR + REVISIONS);
						for(int k = (minorVersions.size() - 1 ); k > -1 ; k--)
						{
							String minorId	= busObjectWithSelect.getSelectData(
									"revisions[" + revision + "]." + SELECT_ON_ACTIVE_MINOR + 
...
//[IR-346257]:XLV:END	
				{
					String majorId 			= busObjectWithSelect.getSelectData(SELECT_ON_MAJOR + "revisions[" + revision + "].id");
					String sRevIndex 		= busObjectWithSelect.getSelectData(SELECT_ON_MAJOR + "revisions[" + revision + "].revindex");
					...
					if(revIndex<=currentRevIndex)
					{
						...
						StringList minorVersions = busObjectWithSelect.getSelectDataList(SELECT_ON_MAJOR + "revisions[" + revision + "]." + SELECT_ON_ACTIVE_MINOR + REVISIONS);
