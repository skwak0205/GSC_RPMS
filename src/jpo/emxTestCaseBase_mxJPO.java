 ...
 * @quickreview KIE1 ZUD 17:12:12 : IR-562019-3DEXPERIENCER2018x : R418 -FUN074416: Unable to enter Estimated completion date on property page of TC object
 */
...
public class emxTestCaseBase_mxJPO extends emxDomainObject_mxJPO {
    ...
    public static MapList getStructureList(Context context, String[] args) throws Exception{
      ...
      //Added mxType for IR-092856V6R2012 
        if(objectType != null && mxType.isOfParentType(context, objectType,ProductLineConstants.TYPE_TEST_CASE)){
            ...
            /*testCaseStructList = (MapList) ${CLASS:emxPLCCommon}.getStructureListForType(context, args);*/
        	testCaseStructList = (MapList) JPO.invoke(context, "emxPLCCommon", null, "getStructureListForType", args, MapList.class);
        }
    ...
    // TODO: 
    // Need to update the TE UI form to point to this method
    /**
     * Method is used to validate the Estimated Completion Date field
     * 
     * @param context the eMatrix <code>Context</code> object 
      ...
      //Iterating through the list of objects to generate the program HTML output for each object in the table
        for (iCount = 0; iCount < iNumOfObjects; iCount++) 
        {
            ...
            {            
				...
				try(InputStream rts = RichEditFactory.getRichEditUtil().getContentStream(context, arrObjId[iCount], Format.HTML_PREVIEW)){
					contentData = ConversionUtil.streamToString(rts);
				}catch(Exception ex){
					ex.printStackTrace();
				}
				contentData = org.owasp.esapi.ESAPI.encoder().decodeForHTML(contentData);
...
//		      System.out.println("Inside ReserveTree: programMap = \n  " + programMap);
    ...
    // Need to enable the stand alone TC command on global Menu
    // 16x HF 13 porting
    public Boolean isStandAloneTCEnabled (Context context, String[] args)
    {
    	final String STR_BUNDLE = "emxProductLineStringResource";
    	try{
    		String enabled = EnoviaResourceBundle.getProperty(context, STR_BUNDLE, context.getLocale(), "eServiceSuiteProductLine.StandAloneTestCase");
    ...
    @com.matrixone.apps.framework.ui.ProgramCallable
    public MapList getTestCases (Context context, String[] args) throws Exception {
    	
    	MapList testCaseObjs = new MapList();
    	String strType = ProductLineConstants.TYPE_TEST_CASE;
    	StringList objectSelects = new StringList(SELECT_ID);
    	
    	testCaseObjs = findObjects(context, strType, DomainConstants.QUERY_WILDCARD, null, objectSelects); 
    			
    	return testCaseObjs;
    }
    
    @com.matrixone.apps.framework.ui.ProgramCallable
    public MapList getOwnedTestCases (Context context, String[] args) throws Exception {
    	
    	MapList testCaseObjs = new MapList();
    	String strType = ProductLineConstants.TYPE_TEST_CASE;
    	StringList objectSelects = new StringList(SELECT_ID);
    	testCaseObjs = findObjects(context, strType, DomainConstants.QUERY_WILDCARD, DomainConstants.QUERY_WILDCARD, 
    			context.getUser(), DomainConstants.QUERY_WILDCARD, null, false, objectSelects);
