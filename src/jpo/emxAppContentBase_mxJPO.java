...
// End of Pre Migration Code - SC
  ...
  public String getRouteContentScope(Context context , String[] args)
	...
	String sType= args[1];
    String RootType= args[2];
	
	String sDefaultType = EnoviaResourceBundle.getProperty(context,"emxComponents.Routes.DefaultContentType");
	StringList sDefaultSymbolicTypeList =  FrameworkUtil.split(sDefaultType, ",");
	boolean isValidType = false;

	for(int i =0; i< sDefaultSymbolicTypeList.size() && UIUtil.isNotNullAndNotEmpty(sType) ;i++ ){
	String symbolicType = PropertyUtil.getSchemaProperty(context,(String)sDefaultSymbolicTypeList.get(i));
