...
public class IEFFamilyTableDataMigrator_mxJPO
{
	...
	private boolean processResults(Context context, BusinessObjectWithSelect busWithSelect) throws Exception
	{
		...
		String familyName = busWithSelect.getSelectData(MCADServerSettings.FAMILY_NAME_FROM_INSTANCE); //fetching family name from the output of select clause
		log("DEC-[IEFFamilyTableDataMigrator_mxJPO.processResults] familyName:"+familyName);
		name        = (String)_serverGeneralUtil.getIndivisualInstanceName(name,"",familyName,"").get(0);//IR-578862
    ...
    private StringList getBusSelectList()
    {
        ...
        selectList.add(ID);
        selectList.add(NAME);
        selectList.add(MCADServerSettings.FAMILY_NAME_FROM_INSTANCE); //Adding select clause to fetch family name
        Enumeration relList       = familyLikeRelsAndEnds.keys();
