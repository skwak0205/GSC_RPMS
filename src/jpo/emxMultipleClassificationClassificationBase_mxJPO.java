...
*/
    ...
    */
    ...
    /***
     * Export the Library for widget
     * @param context the ENOVIA <code>Context</code> object
     * @param args String array containing the Library Object Id
     * @return Map contains the file name and Exported Library information
     * @throws FrameworkException
     */
	public HashMap exportLibraryForWidget(Context context, String[] args) throws Exception {
		HashMap programMap=(HashMap)JPO.unpackArgs(args);
		String objectId=(String)programMap.get("LibraryObjectId");
		Element libXML=null;
		DomainObject domObj = DomainObject.newInstance(context,objectId);
		String objectType=domObj.getInfo(context, SELECT_TYPE);
		boolean validLibraryType = false;
		com.matrixone.apps.classification.Classification classification=new com.matrixone.apps.classification.Classification();
		if(objectType.equals(TYPE_GENERAL_LIBRARY)){
			libXML=classification.createTaxonomyXML(context, objectId, GENERAL_LIBRARY_ELEMENT_NAME, true, false);
			validLibraryType = true;
		}
		else if(domObj.isKindOf(context, TYPE_GENERAL_LIBRARY)){
			String aliasName=FrameworkUtil.getAliasForAdmin(context, "type", objectType, true);
			if(aliasName == null || "".equals(aliasName))
				aliasName=FrameworkUtil.getAliasForAdmin(context, "type", objectType, false);
			if(aliasName == null || "".equals(aliasName)){
				...
				String message = MessageUtil.getMessage(context,
						null,
						"emxLibraryCentral.LoadAllowedTypes.SymbolicNameNotFound",
						new String[]{EnoviaResourceBundle.getAdminI18NString(context, "type", objectType, locale.getLanguage())},
						null,
						locale,
						"emxLibraryCentralStringResource");
				throw new Exception(message);
			...
			aliasName=aliasName.substring(aliasName.indexOf("_")+1, aliasName.length());
			libXML=classification.createTaxonomyXML(context, objectId, aliasName, true, false);
			validLibraryType = true;
		}
		HashMap returnMap=new HashMap();
		if(validLibraryType){
			MapList subClassesList = (MapList)domObj.getRelatedObjects(context,
					RELATIONSHIP_SUBCLASS,
					TYPE_GENERAL_CLASS,
					new StringList(DomainObject.SELECT_ID),null,false,true,(short)1,null,null, 0);
			
			Iterator itr=subClassesList.iterator();
			while(itr.hasNext()){
				Map subClassMap=(Map)itr.next();
				String generalClassId=(String)subClassMap.get(DomainObject.SELECT_ID);
				Element generalclassXML = getAttributeGroupandSubClassesforGL(context, generalClassId, true, false);
				libXML.addContent(generalclassXML);
			}
			XMLOutputter compactOutputter=MxXMLUtils.getOutputter(true);
			String genLibXMLStr = compactOutputter.outputString(libXML);
	
			returnMap.put("fileName", getLibraryExportFileName(context, objectId));
			returnMap.put("fileContent", genLibXMLStr);
		}
		return returnMap;
	}
