 ...
 **  @quickREview 14:12:26 NS7 IR-346980-3DEXPERIENCER2016x MQL Injection
 **  @quickREview 15:05:28 AMA3 Method to update Sharepoint site details
 **  @quickReview 15:08:17 AMA3 SharePoint Data fetch from LCO changes - modified function registerAppsite
 **  @quickREview 15:08:24 ODW IR-388121-3DEXPERIENCER2016x: Incorrect error message is shown while revising file with older revision and incorrect file is getting revised if revise with files operation is performed 
 **  @quickReview 15:11:26 RZW Active Version access passed in file list
 **  @quickReview 17:02:28 RZW Created MSFSearchTable
 **  @quickReview 17:04:20 ODW IR-510615-3DEXPERIENCER2016x: Not able to launch MS project from with in Compass
 **  @quickReview 17:09:15 ACE2 TSK3577429: ENOVIA_BAA_MSF_3DEXPERIENCE_2017x_FD05_Microsoft Project Rest Service Adoption and Improvements
 **   + RESTFull Validate resoures
 **  @quickreview 17:12:08 ODW IR-567521-3DEXPERIENCER2017x: Existing local copy of the Project is not opened when open operation is performed from Search or Compass
 *
 */
...
import ds.enovia.apps.msf.sharepointRegister.SPAppRegistrationEnum;
import ds.enovia.apps.msf.sharepointRegister.SharePointRegister;
...
public class msoiMSFCommon_mxJPO extends emxDomainObject_mxJPO
{
	...
	//MQL Injection
	public String executeMQL(Context context, String[] args) throws Exception
	{
		String MQLOutput = "";
		...
		try
		{
			...
			for(int paramCount = 2; paramCount <= args.length; paramCount++)
			{
				if(args[paramCount].toString().equals("ENDOFMQLPARAMETERS"))
					break;
				else
					commandArguments.add(args[paramCount]);
			}

			MQLOutput = MqlUtil.mqlCommand(context, strCommand, commandArguments);
		}
		catch (Exception ex)
		{
			ex.printStackTrace();
	...
	public String getSPRegisteredAppsSitesWithTokens(Context context, String[] args) throws Exception
	{
		String retVal = "<SPSites></SPSites>";
		try {
			return SharePointRegister.GetSPRegisteredAppSitesWithTokens(context);
		}
		catch (Exception ex)
		{
			ex.printStackTrace();
	...
	public String updateSPRegisteredAppSite(Context context, String[] args) throws Exception
	{
		try {
			SPAppRegistrationEnum spRegEnum  = SharePointRegister.RegisterSharePointAppTokens(context,args[1],"","",args[2]);

			if(!SPAppRegistrationEnum.Registered.equals(spRegEnum))
			{
	...
	public String deleteSPRegisteredAppSite(Context context, String[] args) throws Exception
	{
		String retVal = "Delete operation successful";
		try {
			SharePointRegister.DeleteSPRegisteredAppSite(context, args);
		}
		catch (Exception ex)
		{
			ex.printStackTrace();
	...
	public String registerSPAppSite(Context context, String[] args)
			throws Exception {
		SPAppRegistrationEnum spRegEnum = null;
		try {
			if (args[6].equals("Tokens"))
				spRegEnum = SharePointRegister.RegisterSharePointApp(context,
						args[1], args[2], args[3], Boolean.valueOf(args[4]),
						args[5]);
			else
				spRegEnum = SharePointRegister
						.RegisterSharePointWithCredentials(context, args[1],
								args[2], args[3], Boolean.valueOf(args[4]),
								args[5]);

			return spRegEnum.toString();
		} catch (Exception ex) {
			ex.printStackTrace();
			throw ex;
		}
	...
	//RZW+ IR-375844-3DEXPERIENCER2016x Moved from IEFPMCUtilBase
	...
	public Vector GetType(Context context, String[] args) throws Exception
	{
		...
		try
		{
			...
			for (Iterator itrSelectables = mapList.iterator(); itrSelectables.hasNext();)
			{
				Map objectMap = (Map) itrSelectables.next();
				String objType = (String)objectMap.get(DomainConstants.SELECT_TYPE);
				vecType.add(EnoviaResourceBundle.getTypeI18NString(context, objType, currentLanguage));
			}
		}
		catch (Exception ex)
		{
	...
	**/
	...
	public String GetDetailsForLaunchApplication(Context context, String[] args) throws Exception
	{
		...
		try
		{
			StringList selects = new StringList(9);
			...
			addElement(responseElement, "BusinessID", (String)outputMap.get(SELECT_ID));
	...
	private Element addElement(Element parentElement, String... nameData) 
	{
		Element element = null;
		for (int i = 0; i < nameData.length; i += 2)
		{
			element = new Element(nameData[i]);
			if (i + 1 < nameData.length) { element.addContent(nameData[i + 1]); }
