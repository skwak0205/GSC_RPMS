	...
	throws MatrixException
	{
		try {
			...
			for(int i=0; i< size; i++)
			{
				...
				if(isProject)
					strList.add("false");
				else if("true".equalsIgnoreCase(modifyAccess) || "Create".equalsIgnoreCase(current))
					strList.add("true");
				else
					strList.add("false");
			}
	...
	MapList objectInfoList = new MapList();
    	...
    	for(int i=0;i<objectListSize;i++){
    		...
    		if(ProgramCentralUtil.isNotNullString(isVersionAbleObject)){
    			...
    			boolean showDropIcon = false;
    			if("TRUE".equalsIgnoreCase(toconnect) && "TRUE".equalsIgnoreCase(todisconnect) && "TRUE".equalsIgnoreCase(fromDisconnect)){
        			showDropIcon = true;
        		}
