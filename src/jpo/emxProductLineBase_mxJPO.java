...
/** Alias for dot. */
 protected static final String DOT = ".";
   ...
   * @param context the eMatrix <code>Context</code> object
     ...
     * 
     * @param context
     * @param strOwnerCondition - The owner
     * @return MapList - MapList containing the id of Product Line objects based on whereCondition .
     * @throws Exception
     */
    protected MapList getDesktopProductLines (Context context, String strOwnerCondition) throws Exception {
    	//String list initialized to retrieve data for the Product Lines
        StringList objectSelects = new StringList(DomainConstants.SELECT_ID);
		objectSelects.add("from["+ProductLineConstants.RELATIONSHIP_PRODUCT_LINE_MODELS+"]");
        objectSelects.add("from["+ProductLineConstants.RELATIONSHIP_SUB_PRODUCT_LINES+"]");
        String strType = ProductLineConstants.TYPE_PRODUCT_LINE;
        StringList strCompany = ProductLineUtil.getUserCompanyIdName(context);
        String strCompanyId = (String)strCompany.get(0);
        // forming the where clause
        StringBuffer strWhereExpression = new StringBuffer(85);
        if(strOwnerCondition!=null && !strOwnerCondition.equals("")) {
            strWhereExpression.append("to["+ProductLineConstants.RELATIONSHIP_SUB_PRODUCT_LINES+"]==False");
            strWhereExpression.append(" && ");
            strWhereExpression.append("owner==" + "\"" + strOwnerCondition + "\"");
        }else {
        	strWhereExpression.append("to["+ProductLineConstants.RELATIONSHIP_SUB_PRODUCT_LINES+"]==False");
        }
        
        DomainObject companyObject = new DomainObject(strCompanyId);
        MapList mlObjList = companyObject.getRelatedObjects(context, new String(), strType, objectSelects, new StringList(),
				false, true, (short) 0, strWhereExpression.toString(), null, (short) 0, false, true, (short) 1, null, null, null, DomainConstants.EMPTY_STRING, null);
		for(int i=0; i<mlObjList.size(); i++) {
          	 
			Map mlObj = (Map) mlObjList.get(i);
          	if(((String)mlObj.get("from["+ProductLineConstants.RELATIONSHIP_PRODUCT_LINE_MODELS+"]")).equalsIgnoreCase("False") && ((String)mlObj.get("from["+ProductLineConstants.RELATIONSHIP_SUB_PRODUCT_LINES+"]")).equalsIgnoreCase("False")){
				mlObj.put("hasChildren", "false");
			}	 		 
         }
        return  mlObjList;
   ...
   /** This method gets the object Structure List for the context Product Line object.This method gets invoked
    ...
    public static MapList getStructureList(Context context, String[] args)
	    ...
	    if (objectType != null && strRelPattern.equals(ProductLineConstants.RELATIONSHIP_PRODUCTS)) {
            ...
            /*productlineStructList = (MapList) ${CLASS:emxPLCCommon}.getStructureListForType(context, args);*/
        	productlineStructList = (MapList) JPO.invoke(context, "emxPLCCommon", null, "getStructureListForType", args, MapList.class);
        }
...
/*        //Instantiating a StringList for fetching value of Company Id
    	...
    	//call overloaded method without where condition
        MapList mapBusIds = getDesktopProductLines(context, null);
        return  mapBusIds;
    }
    /**
...
/*        //Instantiating a StringList for fetching value of Company Id
    	...
    	//call overloaded method without where condition
        MapList mapBusIds = getDesktopProductLines(context, context.getUser());
        return  mapBusIds;
    }
    /**
    ...
    public static StringList getCompany(Context context, String[] args)
	    ...
	    String strCompanyTitle = (String)strCompany.get(2);
		vaultList.add(strCompanyTitle);		
		return vaultList;
	}
		...
		try {
			...
			if (productLineId!=null && !productLineId.isEmpty()) {
				...
				if (hasModelAlreadyConnected) {
					returnInt = 1;
					/*${CLASS:emxContextUtilBase}.mqlNotice(context,warningMessage.toString());*/
					MqlUtil.mqlCommand(context, "notice $1", warningMessage.toString());
				} else {
		...
		try 
		{
			...
			if(level != 1)
			{
				...
				if(lstProductChildTypes.contains(strObjectType)){
					...
					List<Map<?,?>> productDerivationStructure = DerivationUtil.getDerivationStructure(context, strObjectid, ProductLineConstants.TYPE_PRODUCTS, 1, 0, limit);
					for(Map derivationMap : productDerivationStructure)
					{
						int intLevel = Integer.parseInt((String)derivationMap.get("level"));
						if(level == 0 || intLevel <= level){
				...
				{
				plProductMap = domainObject.getRelatedObjects(context, relPattern.toString(), typePattern.toString(), objectSelects, new StringList(),
						false, true, (short) 0, null, null, (short) 0, false, true, (short) 1, null, null, null, DomainConstants.EMPTY_STRING, null);
				
				for(Map map : plProductMap)
				{
					int expandLevel = Integer.parseInt((String)map.get("level"));
					if(lstPLChildTypes.contains(map.get("type")) && (level == 0 || expandLevel <= level))
					{
						finalMapList.add(map);
					}
					else if(lstModelChildTypes.contains(map.get("type")) && (level == 0 || expandLevel <= level))
					{
						finalMapList.add(map);
					}
					else if(lstProductChildTypes.contains(map.get("type")) && (level == 0 || expandLevel <= level))
					{
						finalMapList.add(map);
						List<Map<?,?>> mainDerivedList             = DerivationUtil.getMainDerivations(context, (String) map.get("id"));
						List<Map<?,?>> productDerivationStructure  = DerivationUtil.getDerivationStructure(context, (String) map.get("id"), ProductLineConstants.TYPE_PRODUCTS, Integer.parseInt((String) map.get("level")), 0, limit);
						int rootProductLevel = Integer.parseInt((String)map.get("level"));
			...
			else
		    {
			  if(lstPLChildTypes.contains(strObjectType))
			  {
				  ...
				  StringList objectSelects = new StringList(DomainConstants.SELECT_ID);
				  finalMapList = domainObject.getRelatedObjects(context, relPattern.toString(), typePattern.toString(), objectSelects, new StringList(),
							false, true, (short) 1, null, null, (short) 0, false, true, (short) 1, null, null, null, DomainConstants.EMPTY_STRING, null);
			   }
			   else if(lstModelChildTypes.contains(strObjectType))
			   {
				  StringList objectSelects = new StringList(DomainConstants.SELECT_ID);
				  String strRelPattern     = ProductLineConstants.RELATIONSHIP_MAIN_PRODUCT +","+ ProductLineConstants.RELATIONSHIP_MAIN_DERIVED;
				  plProductMap = domainObject.getRelatedObjects(context, strRelPattern, ProductLineConstants.TYPE_PRODUCTS, objectSelects, new StringList(),
							false, true, (short) 0, null, null, (short) 0, false, true, (short) 1, null, null, null, DomainConstants.EMPTY_STRING, null, (short) 1);
					
				  String rootProductLevel = "";
				  for(Map mainDerivationMap : plProductMap)
				  {
					 if(ProductLineConstants.RELATIONSHIP_MAIN_PRODUCT.equals((String)mainDerivationMap.get("relationship")))
					 {
						 rootProductLevel = (String)mainDerivationMap.get("level");
						 finalMapList.add(mainDerivationMap);
					 }
					 else
					 {
						 mainDerivationMap.put("level", rootProductLevel);
						 finalMapList.add(mainDerivationMap);
					 }
				  }
			   }
			   ...
			   {
				  List<Map<?,?>> prdStructureList = DerivationUtil.getDerivationStructure(context, strObjectid, ProductLineConstants.TYPE_PRODUCTS, level, 0, limit);
				  for(Map prdStructureMap : prdStructureList)
				  {
				    	if(String.valueOf(level).equals((String)prdStructureMap.get("level")))
				    	{
							...
							//int intLevel = Integer.parseInt((String)prdStructureMap.get("level")) + 1;
