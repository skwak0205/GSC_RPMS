...
public class DECCaptureModifiedAttributes_mxJPO 
{
	...
	public DECCaptureModifiedAttributes_mxJPO (Context context, String args[] ) throws Exception
	{
		...
		this.resourceBundle = new MCADServerResourceBundle(language);
		this.cache = new IEFGlobalCache();
		this.serverGeneralUtil = new MCADServerGeneralUtil(context, null, resourceBundle, cache);
	}
	...
	public void skipCaptureModifiedAttributes(Context context, String[] args) throws Exception
	{
		String skipTrigger	= args[0].trim();
		String sRPEName = "SkipCaptureModifiedAttributes";
		if("true".equalsIgnoreCase(skipTrigger)){
			MQLCommand command = new MQLCommand();
			command.executeCommand(context, "set env global $1 $2", sRPEName, skipTrigger);
			System.out.println("---RPE set to skip captureModifiedAttributes---");
		}else if("false".equalsIgnoreCase(skipTrigger)){
			MQLCommand command = new MQLCommand();
			command.executeCommand(context, "unset env global $1", sRPEName);
			System.out.println("---RPE unset to skip captureModifiedAttributes---");
		}else{
			throw new Exception("DECCaptureModifiedAttributes_mxJPO.skipCaptureModifiedAttributes: only true or false are valid arguments");
		}
	...
	{		
		...
		String mqlArgs[] = new String[2];
		mqlArgs[0] = "global";
		mqlArgs[1] = "SkipCaptureModifiedAttributes";
		String skipTrigger = mxUtil.executeMQL(context, "get env $1 $2",mqlArgs);
		
		if(!skipTrigger.endsWith("true")){
			...
			if (getRPEforOperation(context, mxUtil, "Finalize").equals("") && getRPEforOperation(context, mxUtil, "UndoFinalize").equals(""))
			{
				...
				if (!isVersionObject)
				{
					...
					if(majorSync.equals(""))
					{
						...
						isMinorSync	 = true;
						//IR-515338: If the coming bo is minor, getActiveMinor() returns null. Hence provided a check.
						minorBus	 = mxUtil.getActiveMinor(context, bo);
                        if(minorBus==null)
								minorBus=bo;
						// IR-569530-3DEXPERIENCER2017x [ RGH1]
						// to check for modifed data added as Custom attribute via MQL or 3DSpace Control Centre App
						if((attrNm.startsWith("$$") && attrNm.endsWith("$$")) || serverGeneralUtil.doesAttributeExistsOnType(context, bo.getTypeName(), attrNm) || serverGeneralUtil.doesAttrExistsOnRACEInterface(context, bo.getTypeName(), attrNm, cache))
						{
				...
				{
					//IR-515338: If the coming bo is major, getMajorObject() returns null. Hence provided a check.
					...
					BusinessObject activeMinorBus	= new BusinessObject();
					if(majorBus==null)
							activeMinorBus=bo;
						else
							activeMinorBus = mxUtil.getActiveMinor(context, majorBus);
						
					if(minorSync.equals("") && bo.equals(activeMinorBus) && majorBus!=null)
					{
			...
			{				
				...
				if(mappingToken.countTokens() >= 2)
				{
					...
					if(mxSideAtrToken.countTokens() >= 2)
					{
						...
						if(mxAttr.equals(attrNm) && objType.equals(mxType) && !mxAttrSet.contains(mxAttr))
						{
