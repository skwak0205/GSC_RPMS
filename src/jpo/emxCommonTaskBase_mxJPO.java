...
*/
        ...
        throws Exception
    {
        ...
        mulValSelects.add(SELECT_PREDECESSOR_IDS);
        mulValSelects.add(SELECT_PREDECESSOR_TYPES);
        busSelects.add(SELECT_PREDECESSOR_IDS);
        busSelects.add(SELECT_PREDECESSOR_TYPES);
        Map objMap = getInfo(context, busSelects, mulValSelects);
        ...
        throws Exception
    {
     ...
     public HashMap getTaskFilterRangeValues(Context context, String[] args) throws Exception {
    	 try{ 
	     		...
	     		approvalStatus.add("No Action");
	     		approvalStatus.add("Approve");
	     		approvalStatus.add("Reject");
	     		approvalStatus.add("Abstain");
	     		approvalStatus.add("Accept");
	     		approvalStatus.add("Complete");
	     		approvalStatus.add("Ignore");
	     		
	     		String statusKey = "emxFramework.LifecycleTasks.";
	     		Locale sLocale = context.getSession().getLocale();
	     		String sApprovalObjName = "";
	     		String sApprovalObjTranslatedName = "";
	     		LinkedHashMap hmAttrRangeValues = new LinkedHashMap();
	     		
	     		for(int i = 0; i<approvalStatus.size(); i++){
	     			sApprovalObjName = approvalStatus.get(i);
	     			if(sApprovalObjName.indexOf(" ") != -1){
	     				sApprovalObjName = FrameworkUtil.findAndReplace(sApprovalObjName," ", ""); 
	     			}
	     			sApprovalObjTranslatedName = EnoviaResourceBundle.getProperty(context, "", sLocale, statusKey+sApprovalObjName );
	     			hmAttrRangeValues.put(approvalStatus.get(i), sApprovalObjTranslatedName);
