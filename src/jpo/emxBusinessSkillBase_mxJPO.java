...
public class emxBusinessSkillBase_mxJPO extends emxDomainObject_mxJPO
{
        ...
        throws Exception
    {
        ...
        String roleAdministrator = PropertyUtil.getSchemaProperty(context, "role_ServiceAdministrator");
        String roleOrganizationManager = PropertyUtil.getSchemaProperty(context, "role_OrganizationManager");
        ...
        Vector loggedInPersonRoles = PersonUtil.getAssignments(context);
        boolean hasEditAccess = false;
        if(loggedInPersonRoles.contains(roleResourceManager) || loggedInPersonRoles.contains(roleAdministrator) || loggedInPersonRoles.contains(roleOrganizationManager)){
        	hasEditAccess = true;
        }else if(loggedInRole.contains(roleVPLMAdmin)){
        	hasEditAccess = true;
        }
    ...
    public static Object getParentSkill(Context context, String[] args) throws Exception
    {
        ...
        String parentSkill = EnoviaResourceBundle.getProperty(context, "emxPersonOrgModelStringResource", context.getLocale(), "emxComponents.BusinessSkill.ParentSkillLevel");
  ...
  */
			...
			String strExpandFilter) throws FrameworkException 
	{
        ...
        if(dmoObject.isKindOf(context,DomainConstants.TYPE_ORGANIZATION))
        {
            strRelationshipPattern =  RELATIONSHIP_ORGANIZATION_SKILL + "," + RELATIONSHIP_SUBSKILL;
        }
        else if(dmoObject.isKindOf(context,TYPE_BUSINESS_SKILL))
        {
	...
	public Map getRangeValuesBusinessSkillTypeForm(Context context, String[] args)throws Exception {
	 	   Map returnMap = new HashMap();
	        StringList rangeSkillTypeDisplay = new StringList();
	        StringList rangeSkillTypeActual = new StringList();
	        String strAttributeName = ATTRIBUTE_COMPETENCY;
	        ...
	        // actual range values
	         ...
	         HashMap<Integer,String> seqNameValue = new HashMap();
	         HashMap<Integer,String> seqNameValueDispaly = new HashMap();
	         for(int j=0;j<attributeRange.size();j++) {
	        	 switch (attributeRange.get(j).toString()) {
	        	 	case "Excellent":
	        	 		seqNameValue.put(4, attributeRange.get(j).toString());
	        	 		seqNameValueDispaly.put(4, attributeDisplayRange.get(j).toString());
	        	 		break;
	        	 	case "Good":
	        	 		seqNameValue.put(2, attributeRange.get(j).toString());
	        	 		seqNameValueDispaly.put(2, attributeDisplayRange.get(j).toString());
	        	 		break;
	        	 	case "Poor":
	        	 		seqNameValue.put(1, attributeRange.get(j).toString());
	        	 		seqNameValueDispaly.put(1, attributeDisplayRange.get(j).toString());
	        	 		break;
	        	 	case "Very Good":
	        	 		seqNameValue.put(3, attributeRange.get(j).toString());
	        	 		seqNameValueDispaly.put(3, attributeDisplayRange.get(j).toString());
	        	 		break;
	        	 }
	         }
  
	         for (int i = 1; i <= attributeRange.size(); i++) {             
	              	   rangeSkillTypeDisplay.add(seqNameValueDispaly.get(i));
	              	   rangeSkillTypeActual.add(seqNameValue.get(i));      
	             }     
	         returnMap.put("field_choices", rangeSkillTypeActual);
	         returnMap.put("field_display_choices", rangeSkillTypeDisplay);
	         return returnMap;

	    }
// End:R208:PRG Advanced Resource Planning
	@com.matrixone.apps.framework.ui.ProgramCallable
	public Vector getOrgSkillsName(Context context, String[] args) throws Exception {
		HashMap programMap = (HashMap) JPO.unpackArgs(args);
        HashMap paramMap = (HashMap)programMap.get("paramList");

        MapList mlObject = (MapList)programMap.get("objectList");

        Vector vOrgSkill = new Vector();
        ...
        if ( mlObject != null)
        {
            ...
            for (int i = 0; i < iObject; i++)
            {
                ...
                Map valueMap = (Map) mlObject.get(i);
				String strRootNode = (String) valueMap.get("Root Node");
                boolean isRootNode = (! UIUtil.isNullOrEmpty(strRootNode)) && "True".equalsIgnoreCase(strRootNode)?true:false;
				
                DomainObject obj = new DomainObject((String)valueMap.get(DomainConstants.SELECT_ID));
				if(isRootNode)
                {
                	orgSkill = obj.getInfo(context, DomainConstants.SELECT_ATTRIBUTE_TITLE);
                } else {
                	orgSkill = (String) valueMap.get(DomainConstants.SELECT_NAME);
                }
                
                vOrgSkill.add(orgSkill);
