...
public class emxSecurityMigrationMigratePersonObjectsBase_mxJPO extends
  ...
  public emxSecurityMigrationMigratePersonObjectsBase_mxJPO(Context context,
    ...
    public void migrateObjects(Context context, StringList objectList) throws Exception
    {
      ...
      mulValSelects.add(MemberName);
      mulValSelects.add(MemberId);
      mulValSelects.add(MemberType);
      mulValSelects.add(MemberRevision);
      mulValSelects.add(MemberProjectRole);
      mulValSelects.add(SecurityContextName);
      mulValSelects.add(memberRelId);     // added for PRG
      ...
      String[] oidsArray = new String[objectList.size()];
        ...
        try
        {
          ...
          {
			String comName = PropertyUtil.getSchemaProperty(context, "role_CompanyName");  
            context.resetRole("ctx::VPLMAdmin."+comName+".Default");
          ...
          if( securityContextNames == null)
          {
            ...
            if ("TRUE".equalsIgnoreCase(sResult))
            {
                ...
                while(addItr.hasNext()) {
                    ...
                    // create a security context with 'Basic User' role
					mqlLogRequiredInformationWriter("Checking the role "+roleName+" is child of Exchange User. ");
                    String symbolicName = PropertyUtil.getSchemaProperty(context, roleName);
                    if(UIUtil.isNotNullAndNotEmpty(symbolicName)) {
                    	if(roleName.equals("role_ExchangeUser") || ComponentsUtil.isChildRole(context, "Exchange User", symbolicName)) {
    						addBasicUserRole = true;    						
                         }
                    } else {                    	
						mqlLogRequiredInformationWriter("Warning : Missing Symbolic Name or Role corresponding to symbolic name : " + roleName);
                    }				
    ...
    private DomainObject createAndAssignSecurityContext(Context context, String personId, String personName, String roleName, String companyType, String companyName, String companyRev) throws Exception{
    	...
    	Map securityContextNamesMap = personObj.getInfo(context, new StringList(SecurityContextNameSelect), new StringList(SecurityContextNameSelect));
    	StringList securityContextNames = (StringList)securityContextNamesMap.get(SecurityContextNameSelect);
    	String defaultProjectName = DomainAccess.getDefaultProject(context);
		if(UIUtil.isNullOrEmpty(defaultProjectName)){
			defaultProjectName = "GLOBAL";
		}
    	String scId = DomainAccess.createSecurityContext(context,defaultProjectName, roleName, companyName, companyType, companyRev);
