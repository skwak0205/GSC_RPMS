 ...
 **
 * @quickreview ZUD      17:11:16   :IR-478873-3DEXPERIENCER2018x : Table borders are not visible in Requirement Properties page/view.
*/
...
public class emxRichEditorBase_mxJPO extends emxDomainObject_mxJPO
{
    ...
    public String getRichTextControl(Context context, String[] args) throws Exception {
		...
		if ((PFmode != null && PFmode.equals("true")) || (reportFormat != null && "CSV".equals(reportFormat))){
        	if("CSV".equals(reportFormat)) 
			{
				contentText = dmoObject.getAttributeValue(context, "Content Text");
        	}else {
        		contentText = dmoObject.getAttributeValue(context, "Content Data");
        	}
			return contentText;
		}
    	...
    	sb.append("<div id=\"NewRichTextEditor\" class=\"richTextContainer\" style=\"min-height:20px; overflow-y: auto; overflow-x: auto;\">");
    ...
    public void tidyRCOFiles(Context context, String args[]) throws Exception {
		...
		String content = obj.getAttributeValue(context, "Content Data");	
    	//String content = args[0];
		int start = 0;
		Vector<String> RCOs = new Vector<>();
		do{
			int fileNameStart = content.indexOf("|rcoFileName|:|", start),
					fileNameEnd = content.indexOf("|", fileNameStart   + "|rcoFileName|:|".length());
			if(fileNameStart == -1) {
				break;
			}
			String fileName = content.substring(fileNameStart + "|rcoFileName|:|".length(), fileNameEnd);
			RCOs.add(fileName);
			start = fileNameEnd;
		}while(start != -1);    	
