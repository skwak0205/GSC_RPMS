...
//IR-606320-Unable to demote X-CAD Design data on V6R2013x-R2018x upgraded environment 
	...
	public void executeCustom(Context _context, Hashtable resultAndStatusTable)  throws MCADException
	{
		...
		try
		{
			// IR-465581-3DEXPERIENCER2017x - No check needed for maturity change
			/* _busObject.open(_context);
	...
	*/
	...
	protected boolean disconnectRelationships(Context _context, BusinessObject fromBus, String relName,
		...
		try
		{
			...
			Expansion expansion = _util.expandBusObject(_context, fromBus, level,"",relationshipWhereClause.toString(), (short)0, false);//FUN080585: deprecation of Visuals
	...
	private void lockCheck(Context context, String objectId) throws Exception
	{
		...
		{	
			...
			//Lock not to be checked for Admin
				...
				{
                                        busObj.open(context);
					if (!_generalUtil.checkLockStatus(context, busObj))
					{
						Hashtable exceptionDetails = new Hashtable(4);
						exceptionDetails.put("TYPE",busObj.getTypeName());
						exceptionDetails.put("NAME",busObj.getName());
						exceptionDetails.put("REVISION",busObj.getRevision());
						exceptionDetails.put("LOCKER",busObj.getLocker(context).getName());
