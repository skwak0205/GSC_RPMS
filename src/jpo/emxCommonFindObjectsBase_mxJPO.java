...
public class emxCommonFindObjectsBase_mxJPO
{
    ...
    try
    {
        ...
        {
            throw new IllegalArgumentException(EnoviaResourceBundle.getProperty(context, "emxPersonOrgModelStringResource", context.getLocale(), "emxComponents.CommonFindObject.TypeCannotBe"));
        }else{
        	 isType = true;
        }
    ...
    try
    {
        ...
        if(isRel==null){
        	String cmd = "list type $1";
            try
            {
                String result  = MqlUtil.mqlCommand(context, cmd, type);
