	 ...
	 try{
		 ...
		 while(objectListItr.hasNext()){
			 ...
			 if(object!=null && object.size()>0){
				 ...
				 lstProjectSpaceChildTypes.add(ProductLineConstants.TYPE_PRODUCTS);
				 ...
				 for(int i=0;i<mlPRojMile.size();i++){
					 ...
					 if(domPord.isKindOf(context,ProductLineConstants.TYPE_PRODUCTS)){
...
/**
 ...
 public boolean isMilestoneDispalyAllowed(Context context,String args[])
 {
	 ...
	 try {
		 ...
		 if(bProgram && bCFF && "Enabled".equalsIgnoreCase(strExprValue)){
			...
			// }else{
				 ...
				 if(dmoObject.isKindOf(context, ProductLineConstants.TYPE_PRODUCTS)){
