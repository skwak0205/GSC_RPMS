...
//			}  		
	...
	public void updateFTEColumnDataByTimeLine(Context context,String[]args) throws Exception {
		try 
		{
			...
			if(ProgramCentralUtil.isNotNullString(strRequestID)){
				if(!updatedRequestList.contains(strRequestID))
					updatedRequestList.add(strRequestID);
				projectSpaceResourceRequestInfo.remove(strRequestID+nYear+nTimeLine);
				projectSpaceResourceRequestInfo.remove(strRequestID+"0"+"0");  // added for update issue of Total column when FTE column is updated in Timeline View  of Resource Plan Table 
