...
public class emxAEFCollectionBase_mxJPO
{
        ...
        throws Exception
    {
        ...
        String fromFlag = (String) paramMap.get("fromFlag");
        
        if(UIUtil.isNotNullAndNotEmpty(fromFlag) && "UIShortcutUtil".equalsIgnoreCase(fromFlag))
        {
        	String strSystemGeneratedCollectionLabel = EnoviaResourceBundle.getProperty(context, "emxFrameworkStringResource", new Locale(strLang), "emxFramework.ClipBoardCollection.NameLabel");
    		if(strSetname.equalsIgnoreCase(strSystemGeneratedCollectionLabel)){
    			strSetname = EnoviaResourceBundle.getProperty(context, "emxFramework.ClipBoardCollection.Name");
    		}
        }
		else {
        ...
        */
        ...
        resultSelects.add(DomainConstants.SELECT_ATTRIBUTE_TITLE);
        resultSelects.add("type.kindof["+ DomainConstants.TYPE_ORGANIZATION +"]");
  ...
  public Vector getDescription(Context context,String[] args)
      ...
      public static MapList getIcon(Context context, String[] args) throws Exception {
    	  ...
    	  if(objectList != null){
    		  for (int i=0; i < objectList.size(); i++){
    			  ...
    			  iconHm.put(objectInfo, "iconSmallCollections.png");
    			  iconMap.add(iconHm);
    		  }
    	  }
    	  return iconMap;
	...
	*/
	...
	throws Exception{
		...
		try {
			...
			for (int i=0; i < collMapList.size(); i++) {
				...
				hm.put("modified", collMap.get(DomainConstants.SELECT_MODIFIED));
    ...
    @com.matrixone.apps.framework.ui.ProgramCallable
    public MapList getCollectionsWithCquery(Context context,String[] args) throws Exception {
    	MapList returnMapList = new MapList();
        SelectList resultSelects = new SelectList(4);
        resultSelects.add("physicalid");
        resultSelects.add("current.access[read]");
		resultSelects.add("current.access[show]");
				
		MapList collectionsMapList = SetUtil.getCollections(context);
		
        for(int i=0; i < collectionsMapList.size(); i++) {
        	HashMap eachCollection = new HashMap();
        	
        	Map<String, String> collectionsMap = (Map<String, String>) collectionsMapList.get(i);
        	String sCollectionName  = (String)collectionsMap.get(DomainConstants.SELECT_NAME);
			String sCollectionCount = (String)collectionsMap.get("count");
			
            eachCollection.put("name",sCollectionName);
            eachCollection.put("id[connection]", (String)collectionsMap.get(DomainConstants.SELECT_ID));
            //eachCollection.put("description", (String)collectionsMap.get(DomainConstants.SELECT_DESCRIPTION));  for now sending cquery as description
            eachCollection.put("count",sCollectionCount);
            StringBuilder collectionItemsQueryStrBuilder = new StringBuilder();
            MapList collectionItemMapList = SetUtil.getMembers(context, sCollectionName, resultSelects);
			for(int j = 0; j < collectionItemMapList.size(); j++) {
				HashMap<String, String> collectionItemMap = (HashMap<String, String>) collectionItemMapList.get(j);
				
				if (collectionItemMap != null) {
					String strPhysicalId = collectionItemMap.get("physicalid");
					boolean isShowAccess = "true".equalsIgnoreCase(collectionItemMap.get("current.access[show]"));
					boolean isReadAccess = "true".equalsIgnoreCase(collectionItemMap.get("current.access[read]"));

					if (UIUtil.isNotNullAndNotEmpty(strPhysicalId) && (isReadAccess && isShowAccess)) {
						if(j > 0) {
							collectionItemsQueryStrBuilder.append(",");
						}
						
						collectionItemsQueryStrBuilder.append(strPhysicalId);
					}
				}
			}
			if(collectionItemsQueryStrBuilder.length() == 13) {
				collectionItemsQueryStrBuilder.append(sCollectionName);
			}
			eachCollection.put("description", collectionItemsQueryStrBuilder.toString());   //this has to be cquery in future
			String strSystemGenCollection = EnoviaResourceBundle.getProperty(context, "emxFramework.ClipBoardCollection.Name");
            if(sCollectionName.equalsIgnoreCase(strSystemGenCollection))
            {
            	sCollectionName = EnoviaResourceBundle.getProperty(context, "emxFrameworkStringResource", context.getLocale(), "emxFramework.ClipBoardCollection.NameLabel");
            }
			eachCollection.put("id", sCollectionName+":"+collectionItemsQueryStrBuilder.toString());
			returnMapList.add(eachCollection);
        }
