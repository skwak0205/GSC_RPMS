...
public class emxTriggerValidationResultsBase_mxJPO
{
    ...
    throws Exception
  {
        ...
        for(int i=0;i<objSize;i++)
        {
            ...
            if (strExpFormat != null && ("text".equalsIgnoreCase(strExpFormat) || "csv".equalsIgnoreCase(strExpFormat))){
              v.add(XSSUtil.encodeForHTML(context, strResult));
                ...
                if(STR_PASS.equalsIgnoreCase(strResult) && strResult!=null)
                {                	 
					resStr.append(EnoviaResourceBundle.getFrameworkStringResourceProperty(context, "emxFramework.TriggerValidationReport.Result.Pass", new Locale(languageStr)));
                }
                if(STR_FAIL.equalsIgnoreCase(strResult) && strResult!=null)
                {
                	resStr.append(EnoviaResourceBundle.getFrameworkStringResourceProperty(context, "emxFramework.TriggerValidationReport.Result.Fail", new Locale(languageStr)));                	
                }
                if(STR_WARNING.equalsIgnoreCase(strResult) && strResult!=null)
                {
                	resStr.append(EnoviaResourceBundle.getFrameworkStringResourceProperty(context, "emxFramework.TriggerValidationReport.Result.Warning", new Locale(languageStr)));                	
                }
              //XSSOK
     ...
     * @throws Exception
     */
	 public StringList changeColor(Context context, String[] args) throws Exception {		
		...
		while(itr.hasNext()) {
			...
			if(map.containsKey("result")) {
				String triggerResult= (String) map.get("result");
				if(triggerResult.equalsIgnoreCase(STR_PASS)){
					slResults.addElement("trigger-result-pass");
				}
				if(triggerResult.equalsIgnoreCase(STR_FAIL)){
					slResults.addElement("trigger-result-fail");
				}
				if(triggerResult.equalsIgnoreCase(STR_WARNING)){
					slResults.addElement("trigger-result-warning");
				}	
			}	
		}		
		return slResults;
  ...
  public String executeTriggers(Context context, String[] args) throws Exception
  {
    ...
    //loop to execute the list of triggers that user have selected
        ...
        String methodArgs[] = new String[16];
        for(int k=0;k<15;k++)
        {
            methodArgs[k] = (String) attMap.get(sl.get(k));
        }
        methodArgs[15] = "validate";
