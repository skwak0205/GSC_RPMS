...
import com.matrixone.apps.common.DecisionNotification;
...
//Addition End:30-Apr-2010:di1:R210:PRG:Meeting Usability
    ...
    */
    ...
    public boolean canDeleteDecision(Context context, String[] args)
    	    throws Exception
    {
    	HashMap programMap = (HashMap) JPO.unpackArgs(args);
    	...
    	if(UIUtil.isNullOrEmpty(objectId))
    	{
    		...
    		Access contextAccess = dObject.getAccessMask(context);
    		showCommand = contextAccess.hasFromDisconnectAccess();
    	}
    	return showCommand;
    }
    
    @com.matrixone.apps.framework.ui.PostProcessCallable
	public void send3DNotificationOnCreate(Context context, String[] args) throws Exception {

		HashMap programMap = (HashMap) JPO.unpackArgs(args);
		HashMap paramMap = (HashMap) programMap.get("paramMap");
		String decisionId = (String) paramMap.get("newObjectId");
		DomainObject busUnitBO = DomainObject.newInstance(context, decisionId);
		String title = (String) busUnitBO.getInfo(context, DomainConstants.SELECT_ATTRIBUTE_TITLE);
		Map<String, String> decisionInfo = new HashMap<>();
		decisionInfo.put(DecisionNotification.DECISION_TITLE, title);
		decisionInfo.put(DecisionNotification.DECISION_ID, decisionId);

		List<String> toList = new ArrayList<>();
		toList.add(context.getUser());
		DecisionNotification notifyObj = new DecisionNotification();
		notifyObj.sendDecisionCreated3DNotification(context, decisionInfo, toList);

	}

	public void send3DNotificationOnChangeOwner(Context context, String[] args) throws Exception {
		DomainObject busUnitBO = DomainObject.newInstance(context, args[1]);
		String title = (String) busUnitBO.getInfo(context, DomainConstants.SELECT_ATTRIBUTE_TITLE);

		Map<String, String> decisionInfo = new HashMap<>();
		decisionInfo.put(DecisionNotification.DECISION_NAME, args[0]);
		decisionInfo.put(DecisionNotification.DECISION_ID, args[1]);
		decisionInfo.put(DecisionNotification.OWNER_NAME, args[2]);// need to display in mail
		decisionInfo.put(DecisionNotification.OLD_OWNER_NAME, args[3]);
		decisionInfo.put(DecisionNotification.DESCRIPTION, args[4]);// need to display in mail
		decisionInfo.put(DecisionNotification.DECISION_TITLE, title);
		DecisionNotification notifyObj = new DecisionNotification();
		notifyObj.send3DNotificationOnChangeOwner(context, decisionInfo);
		
	}

	public void send3DNotificationOnStateChange(Context context, String[] args) throws Exception {

		DomainObject domainObject = new DomainObject(args[1]);
		...
		String policy = DomainConstants.POLICY_DECISION;
		String lang = context.getSession().getLanguage();
		String currentStateNLS= EnoviaResourceBundle.getStateI18NString(context, policy, args[2], lang);
		
		Map<String, String> decisionInfo = new HashMap<>();
		decisionInfo.put(DecisionNotification.DECISION_ID, args[1]);
		//decisionInfo.put(DecisionNotification.CURRENT_STATE, args[2]);
		decisionInfo.put(DecisionNotification.CURRENT_STATENLS, currentStateNLS);
		decisionInfo.put(DecisionNotification.ACTION, args[3]);
		//decisionInfo.put(DecisionNotification.OWNER_NAME, args[4]);
		decisionInfo.put(DecisionNotification.DECISION_TITLE, title);

		List<String> toList = new ArrayList<>();
		toList.add(originator);
		DecisionNotification notifyObj = new DecisionNotification();
		notifyObj.sendDecision3DNotificationOnStateChange(context, decisionInfo, toList);
	}
