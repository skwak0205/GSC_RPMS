...
public class emxRouteBase_mxJPO extends emxDomainObject_mxJPO
{
    ...
    public static final String  sRelRouteNode                =  PropertyUtil.getSchemaProperty("relationship_RouteNode");
...
//added for 308049
  ...
  */
    ...
    throws Exception
  {
     ...
     if (((sOwner.equals(context.getUser()) && (isRouteEditable)) || (contextAccess.hasToConnectAccess() && contextAccess.hasFromConnectAccess())) && !loggedInRole.contains(roleOwner) && !loggedInRole.contains(roleSecuredOwner) && !loggedInRole.contains(roleAdmin) )
        showLink = true;
...
//added for the bug 341480 End
   ...
   public MapList getRouteAccessSummaryMemberList(Context context,String[] args) throws Exception {
       ...
       selectables.addElement("to[" + RELATIONSHIP_EMPLOYEE + "].from.attribute[Title]");
       selectables.addElement(com.matrixone.apps.common.Person.SELECT_COMPANY_ID);
		selectables.addElement(com.matrixone.apps.common.Person.SELECT_COMPANY_ID);
		selectables.addElement(DomainConstants.SELECT_ATTRIBUTE_TITLE);
		selectables.addElement(strKindOfProxyGroup);
       ...
       //initate route - end
       selectPersonRelStmts.addElement("attribute[Route Owner Task]");
               ...
               if(UIUtil.isNotNullAndNotEmpty(templateId)&& ("Maintain Exact Task List".equals(taskSetting)||"Extend Task List".equals(taskSetting) || "Modify Task List".equals(taskSetting))){
				   ...
				   while(iterator.hasNext()) {
                      ...
                      if(personName.equals(memberName)){
       	   		   ...
       	   		   isAccessColumnEditable = isAccessColumnEditable == "false" ? "false" : (hasPerson ? "false" : "true");
               }               
...
/**
		...
		public int checkAssigneeForTask(Context context,String[] args)throws Exception
		{
			if(args!=null && args.length>1){
			  ...
			  relStmts.addElement(chooseUserGroup);
    ...
    public int routeChangeOwnerProcess(Context context, String[] args) throws Exception {	
		...
		if("owner".equals(kindOfOwner) && (!newOwner.equals(oldOwner))){
			...
			try {
                ...
                taskObjectSelects.addElement("from[" + DomainConstants.RELATIONSHIP_PROJECT_TASK + "].id");
                taskObjectSelects.addElement("from[" + DomainConstants.RELATIONSHIP_PROJECT_TASK + "].from.attribute["+sAttrRouteOwnerTask+"]");
                taskObjectSelects.addElement("from[" + DomainConstants.RELATIONSHIP_PROJECT_TASK + "].from.id");
    ...
    private void notifyOwner(Context context,String[] routeInfo) throws Exception {
		...
		String routePhysicalId = routeInfo[8];
		...
		routeInfoNotif.put(RouteTaskNotification.ROUTE_ID, strObjectId);
		...
		routeInfoNotif.put(RouteTaskNotification.SENDER_USER, routeInfo[9]);
	...
	public static int sendRouteStarted3DNotification(Context context, String[] args) throws Exception {
		...
		if(toList.size() > 0 ) {
			...
			if(!routeContentList.isEmpty()) {
				...
				routeTitle = (String) routeContent.get("from[Object Route].to.attribute[Title]");
			}else {
			...
			RouteTaskNotification notifObj = RouteTaskNotification.getInstance(context, sRouteContent3DTypes.toString());
	...
	public StringList getRouteListInCurrentState(Context context, String[] args) throws FrameworkException {
	 ...
	 public static int setDataForRouteDeletionNotification(Context context, String[] args) throws Exception {
			...
			Route routeObj = new Route();
			routeObj.setId(sRouteId);
			String routeTitle = (String) routeObj.getInfo(context, "attribute["+DomainConstants.SELECT_ATTRIBUTE_TITLE+"]");
	...
	public static int checkParentTaskAndRoute(Context context, String[] args) throws Exception {
			...
			if("Started".equals(newValue) && "Route".equalsIgnoreCase(type)) {
				...
				selects.add("to[Task Sub Route].from.from[Route Task].to.name");
				...
				if(UIUtil.isNotNullAndNotEmpty(parentTaskStatus) && "Complete".equalsIgnoreCase(parentTaskStatus)) {
					...
					String parentRouteName = (String) details.get("to[Task Sub Route].from.from[Route Task].to.name");
