...
* @quickreview KIE1 ZUD 09:12:2019 : IR-686430 MKCA Error promotion
* @quickreview      ZUD 11:03:2021 : IR-747011-3DEXPERIENCER2020x  IR-749653-3DEXPERIENCER2020x
* @quickreview SSE17 ZUD 29:07:2021 : IR-874027 Unable to demote Chapter from Release to Frozen state an error message displayed
* @quickReview KIE1 GVC 4/02/2022: FUN119640: Manage configuration context new column in Dashboard manage impact of configuration model in BPS
*/
...
import com.dassault_systemes.requirements.ReqConfigurationService;
...
//END : LX6 IR-251026V6R2014x NHIV6R-037278: Duplication of Tests Case and Use case is KO when None option is selected for anyone of them		   
      ...
      throws Exception
   {
	  ...
	  String strExpandLevel = (String)programMap.get("strExpandLevel"); // ++ AGM1 ZUD IR-661358-3DEXPERIENCER2021x --
	  ...
	  // ++ AGM1 ZUD IR-661358-3DEXPERIENCER2021x 
      // ExpandLevel is blank in case of 'ALL'
      if(null == expLevel || 0 == expLevel.length()){
		  expLevel = strExpandLevel;
	  }
      // -- AGM1 ZUD IR-661358-3DEXPERIENCER2021x 
      ...
      throws Exception
   {
    ...
    private static MapList expandTreeSortAndFilter(Context context, String objectId, String expandRels,
        ...
        try {
            ...
            objSelect.addElement(SELECT_KINDOF);  // VAI1 : IR-754086
       ...
       throws Exception
   {
        ...
        throws Exception
    {
         ...
         try
        {
            ...
            String strReleaseChapter = "";
            String strReleaseComment = "";
            String strReviewChapter = "";
            String strReviewComment = "";
            ...
            if(domReq.isKindOf(context,(RequirementsUtil.getChapterType(context))))
            {
               strChapterPolicy = (String)objectPolicy.get("policy"); // getting registered object policy
         	   strReleaseChapter = FrameworkUtil.lookupStateName(context, strChapterPolicy, strSymbReleaseState);
         	   strReviewChapter = FrameworkUtil.lookupStateName(context, strChapterPolicy, strSymbReviewState);
         	   strReviewComment = FrameworkUtil.lookupStateName(context, strCommentPolicy, strSymbReviewState);
         	   strReleaseComment = FrameworkUtil.lookupStateName(context, strCommentPolicy, strSymbReleaseState);
            ...
            for(int i=0; i<mapSubObjects.size(); i++)
            {
                ...
                if(!doReleasedParentsExist(context,strSubObjId)){ //IR-036353V6R2011
					...
					if(domSubObj.isKindOf(context,(RequirementsUtil.getChapterType(context))))
	                    domSubObj.setState(context, strReviewChapter);
	                else
	                    domSubObj.setState(context, strReviewComment);
                }
            }
            ...
            if(domReq.isKindOf(context,(RequirementsUtil.getChapterType(context))))
                domReq.setState(context, strReviewChapter);
            else
   ...
   private List<String> getAttribute(List<String[]> strFullPath, String Attribute)
   {
       ...
       throws Exception
    {
       try
       {
          ...
          {
            // KIE1 Added for changesov issue
           boolean isContextuser = false;
           try {
                 String isAccessGranted = EnoviaResourceBundle.getProperty(context, "emxRequirement.multipleOwnership.access");
                 if("true".equalsIgnoreCase(isAccessGranted))
                 {
                     String contextUserRole = context.getRole();
                     String[] userRole = contextUserRole.split("::");
                     
                     boolean isAuthorizeduser = false;
                     String secContext= "", userName ="", access= "";
                     MapList results = DomainAccess.getAccessSummaryList(context, RootId);
                     for(int i =1; i <= results.size() -1 ; i++)
                     {
                         Map user = (Map)results.get(i);
                         userName = (String)user.get("username");
                         access = (String)user.get("access");
                         if(null != userName && userName.equalsIgnoreCase("null") && userName.equalsIgnoreCase(context.getUser()) && "Leader".equalsIgnoreCase(access))
                         {
                           isAuthorizeduser = true;
                         }else {
                           secContext = (String)user.get("name"); 
                           if(userRole[1].contains(secContext) && "Leader".equalsIgnoreCase(access))
                           {
                             isAuthorizeduser = true;
                           }
                         }
                     ...
                     DomainObject parentObj = DomainObject.newInstance(context, parObjId);
                     ...
                     DomainObject childObj = DomainObject.newInstance(context, objectIdArray[counter]);
                     String attrAccessType = childObj.getInfo(context, attrAccessTypeSelect);
        
                     if("TRUE".equals(attrPropogateAccess) && "Inherited".equals(attrAccessType) && isAuthorizeduser){
				...
				if(newParent.isKindOf(context, ReqSchemaUtil.getRequirementType(context)))
				{
					...
					if(refrid != null && !"".equals(refrid)){
						...
						if(!refoid.equals(newpid) &&  !relType.equals(refRelType))
						{
							...
							else if (relType.equals(ReqSchemaUtil.getDerivedRequirementRelationship(context)))
							{
	   ...
	   public Vector getColumnConfigurationData(Context context, String[] args)throws Exception
	    { 
	        try
	        {
	        	...
	        	MapList modleList = ReqConfigurationService.getAttachedModel(context, idsList);
	            ...
	            for (int iCount = 0; iCount < objectList.size(); iCount++) 
	            {
                    ...
                    String models = (String)((Map)modleList.get(iCount)).get(strObjectId);
...
// -- HAT1 ZUD: ReqSpec dependency - enhancements.
  ...
  //JX5
        			...
        			if((objectType.equalsIgnoreCase("Requirement")||objectKindOf.equalsIgnoreCase("Requirement")) && singleNodeExpandFlag){
	        			...
	        			for(int j = 0; j < coveredList.size();j++ )
	        			{
	            			...
	            			childobjectLevel	= Integer.toString(tempLevel);
								...
								// ++AGM1 ZUD IR-723328-3DEXPERIENCER2021x
	...
	protected void createObjectOwnership(Context context, String fromId, String toId, String relId, String relType, String fromType, String toType) throws Exception {
		...
		// Cloud Indexing policy 
		//commented against IR-834338-3DEXPERIENCER2021x
		//MqlUtil.mqlCommand(context,String.format("modify Connection %s add access bus %s as read,show",relId, fromId));
